...
2. **Judge.** Fix what's real and in scope: bugs in code this PR touched, [`code-review-rules.md`](../../docs/architecture/code-review-rules.md) / hard-rules violations, drift from the linked issue's spec. Decline what isn't: unrelated to this PR (if real, open an issue or a debt-ledger entry instead) · speculative / never-observed hypotheticals · contradicts project posture (scale/pagination advice, concurrency tokens, emailless users, anonymous-endpoint perf guards — see the false-positive atoms in `INDEX.md`) · pure style preference.
   - **Lower the bar for "never observed."** A finding that has never actually happened, self-heals within minutes, and costs nothing when it does fire is a decline, even if it's cheap to fix — cheap-to-fix is not a reason to fix. Ask "has this ever actually happened?" before acting.
   - **Findings on a dead path are WONTFIX, not engineering.** A defect in code that will never execute — e.g. the `Down()` of an approved, never-to-be-rolled-back drop migration — doesn't get a contorted fix; a null/nullable default is a fine restore value. Never chain a second migration/file/layer just to perfect a rollback shape nothing will run.
3. **Rounds are judgment, not a fixed count.** No hard one-cycle cap — sometimes five rounds are justified, sometimes one. Continue while re-reviews surface real new problems; stop when a reviewer just re-flags a fresh hypothetical against code that already changed (decline the Nth variation of the same theme instead of hardening further). Bar stays "would this actually bite in production," not "is the reviewer technically correct." No Needs-Peter gating step either — this triage is the agent's job, not a manual checkpoint.
