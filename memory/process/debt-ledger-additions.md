...
| One-off whose fix is inside a section's own test project (`tests/Humans.<X>.Tests`) — section-owned, same as its `src/` | that section's `Docs/debt.yml` |
| One-off spanning sections, or in `Humans.Base` / `Humans.Web` / shared `tests/` (`Humans.Testing`, `Humans.Integration.Tests`, `Humans.Web.Tests`, `Humans.Application.Tests`) / infrastructure | `inbox:` in [`docs/architecture/debt-ledger.yml`](../../docs/architecture/debt-ledger.yml) |
