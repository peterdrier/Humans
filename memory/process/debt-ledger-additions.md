...
| One-off whose fix is inside a section's own test project, `tests/Humans.<X>.Tests/` — that project is section-owned, so it routes like the section | that section's `Docs/debt.yml`, same as above |
| One-off spanning sections, or in `Humans.Base` / `Humans.Web` / shared test infrastructure (`tests/Humans.Testing`, `tests/Humans.Application.Tests`, `tests/Humans.Integration.Tests`) / infrastructure | `inbox:` in [`docs/architecture/debt-ledger.yml`](../../docs/architecture/debt-ledger.yml) |
