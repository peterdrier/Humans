...
| Debt | Goes to |
|---|---|
| One-off whose fix is inside a single `src/Sections/Humans.<X>/` | that section's [`src/Sections/Humans.<X>/Docs/debt.yml`](../../src/Sections) — create it if absent |
...
- A `/section-doctor` run writes its own section's file directly and routes off-section finds through its sweep queue — a run never writes another section's files mid-assessment.
