...
**How to apply:**
- `git fetch origin main && git worktree add -b <branch> .claude/worktrees/<branch> origin/main` — default flow. Cloud run: `git fetch origin main && git checkout -b <branch> origin/main`, in the repo root.
- `git fetch upstream main && git worktree add -b <branch> .claude/worktrees/<branch> upstream/main` — only when explicitly creating a hotfix/cherry-pick branch destined directly for upstream. Cloud run: `git checkout -b <branch> upstream/main`.
