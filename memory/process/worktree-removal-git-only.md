---
name: Worktree removal — git only, then rmdir the empty husk
description: HARD RULE. To remove a worktree, only `git worktree remove [--force]` is allowed. If git emptied the contents but left the empty parent dir, `rmdir` (non-recursive, no force flags) it — that's the normal second step, not an exception. If anything remains, report and stop. No PowerShell `Remove-Item -Recurse`, no `rm -rf`, no process kills, no `dotnet build-server shutdown`, no retries from a different cwd, no "let me try X to release the lock first." For anything else, tell Peter and wait.
---

**HARD RULE.** Worktree cleanup is git-only; an empty leftover dir gets a plain `rmdir`.
