...
3. **It false-positives on merge commits that absorb main's migrations.** It diffs `--cached` with no provenance check, so a branch merging a main that contains a legitimate data-touching migration file gets blocked once, even though nothing on the branch authored the file. Confirm with `git diff --cached --quiet origin/main -- '*Migrations*'` — silent means every staged migration is byte-identical to main.
...
**How to apply:** verify it's a false positive first (case 2 or 3 above, with the `--cached` diff as evidence), then ask Peter to temporarily neutralize the main checkout's copy via `!` (Git Bash syntax, forward slashes — a backslash path silently creates a junk file in cwd), commit without staging the hook, then restore both trees with `git checkout -- .claude/block-data-migration-hook.sh` and verify each is back to the real script.
