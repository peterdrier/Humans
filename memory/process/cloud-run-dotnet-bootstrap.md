...
A Claude Code cloud container starts with **no `dotnet` on PATH**. A local checkout does not have this problem, so the bootstrap runs where the problem lives: a **SessionStart hook in `.claude/settings.json`** (added 2026-08-22) fires the script automatically in cloud sessions — gated on `CLAUDE_CODE_REMOTE=true`, so local sessions never see it — and its summary lands in the session's starting context. A run can also tell which it is: `CLAUDE_CODE_ENTRYPOINT=remote_trigger` marks a scheduled firing, and `CCR_AGENT_PROXY_ENABLED` / `CLAUDE_CODE_CONTAINER_ID` mark a remote container generally.

**A cloud session should find the bootstrap summary already in its context; read its `Full build` line before building anything.** Calling the script again is harmless (idempotent, ~0.5s warm) — do it if the summary is absent. Skills, `CLAUDE.md` and section docs still should not instruct the call — a local session reading them would be told to go installing SDKs it already has.
