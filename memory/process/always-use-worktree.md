...
ALL branch work — including work on `main` itself — happens in a git worktree under `.claude/worktrees/<name>`. The main checkout is read-only: never edit files, stage, commit, reset, or checkout there.
...
**How to apply:**
- New work: `git worktree add -b <slug> .claude/worktrees/<slug> origin/main`, commit on that branch, push it, open a PR ([[no-direct-to-main]] — the standalone `memory/**` direct-main case is defined there, and even that is pushed from the worktree, never from the main checkout).
...
**How to apply:** step 1 of any feature task — after the issue-authorization preflight in [[issue-fetch-protocol]] for issue-driven work — is `git worktree add -b <name> .claude/worktrees/<name> origin/main`. From then on, every Glob/Grep carries an explicit `path` under that worktree; never omit `path` at the repo root.
