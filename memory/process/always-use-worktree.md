...
- The one case that still wants worktrees in the cloud: a skill running genuinely parallel lanes that must not share a checkout (`/refactor-swarm`). Isolation there is between this session's own lanes, not against other sessions, so the reason survives.
