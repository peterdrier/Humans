...
- [`dont-grow-bloated-service-for-bot-warnings`](architecture/dont-grow-bloated-service-for-bot-warnings.md) — when a service already has many same-domain methods (e.g. ~20 email lookups on `IUserEmailService`), don't add another to silence a "logic in controller" bot warning; inline a short LINQ chain on an existing list-returning method (view-model assembly is excluded from that rule)
