...
- [`google-resources-teams-owned`](architecture/google-resources-teams-owned.md) — `TeamResourceService` directly injecting `IGoogleResourceRepository` is correct: `google_resources` is Teams-owned per design-rules and architecture-test ownership map; the repo's `[Section("GoogleIntegration")]` is a stale label. Suppress HUM0017, do not invent an `IGoogleResourceService`.
