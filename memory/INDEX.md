...
- [`no-displayname-for-people`](architecture/no-displayname-for-people.md) — HARD RULE. In code we own, person identity is `BurnerName`, never `DisplayName`. Canonical: `BurnerName => !IsNullOrWhiteSpace(profile.BurnerName) ? profile.BurnerName : user.DisplayName`. Only carve-outs: `User.DisplayName` (domain entity), the `Name` claim, and non-person labels (groups, locations, resources).
