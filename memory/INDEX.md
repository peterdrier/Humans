...
- [`no-pre-existing-failures`](process/no-pre-existing-failures.md) — HARD RULE. A red test on `main` blocks the merge; "pre-existing, not my PR" is dead. Repair it or `Skip = "tracked-by: nobodies-collective/Humans#NNN"`. CI fails any `Skip=` string without an issue ref.
