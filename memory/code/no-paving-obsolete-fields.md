...
- `Profile.State` / `ProfileState` (`[Obsolete]`, `HUM_PROFILE_STATE`; the C# property is gone and `profiles.state` was dropped in `DropProfileState`) → `User.State` (`UserState`) for suspension, `UserInfo.IsSuspended` / `IsStub` for the predicates.
