...
description: Adding or editing an <img alt="..."> in a Razor view — alt text is a user-facing string and needs a resx key in all six cultures. The admin exemption covers only pages that admin or operator roles alone can reach, not a member-facing modal opened from an admin page. Triggers on any <img> in a .cshtml.
---
...
**The admin exemption does not stretch to cover it.** [`localization-admin-exempt`](localization-admin-exempt.md) exempts a page only when admin or operator roles alone can reach it. A member-facing partial or modal is not exempt merely because the page that opens it is an admin page — the reader is a member either way, and the reader is what the rule is about.
