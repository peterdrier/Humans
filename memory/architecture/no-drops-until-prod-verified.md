...
- **Dead `profiles.State` column (2026-08-21, nobodies-collective/Humans#844)** — `DropColumn State` on `profiles` in `20260820215726_DropProfileState`. The `ProfileState` column was introduced by #405 and every writer (profile save, stub creation, the SetSuspension mutation, `SuspendManyAsync`, the lazy `WriteBackStateIfNullAsync` seed) was deleted by #1436 when suspension moved to `users.State`. Verified by pickaxe over full history across all four paths `Profile` has lived at; no migration or raw SQL ever wrote it. Peter's approval, 2026-08-20: "drop the column, but be careful since we already have 2 migrations in user section today."
