...
- **Per-occurrence event favourites (2026-06-11)** — `DropIndex IX_event_favourites_UserId_GuideEventId` is authorized in the same migration (`20260611203803_AddEventFavouriteDayOffset`) that creates the widened unique index `(UserId, GuideEventId, DayOffset)`. The old unique index actively forbids the per-occurrence rows the feature writes, so no two-PR sequencing is possible; index-only, no data touched, rebuildable from its definition. Do not flag again.
