...
- **`FullProfile.BurnerName`** resolves to `Profile.BurnerName` when present, falling back to `User.DisplayName` only for Stub-state pre-onboarding rows. It is the canonical "what name do we show this human as" lookup.
- **`<vc:human>`** view component already calls `FullProfile.BurnerName`, so views just emit `<vc:human user-id="@x" />` with no name override.
...
- **Email / notification recipient labels** call `IProfileService.GetFullProfileAsync(userId)` and read `.BurnerName`.

## Legitimate reads of `User.DisplayName` (the only four)

1. **`HumanViewComponent`** no-Profile fallback for pre-onboarding humans.
