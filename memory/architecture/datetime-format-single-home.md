...
- Need a date rendered? Call a named method on the home (`value.ToDate()`, `.ToWeekdayDayMonth()` for culture display, `.ToInvariantDate()`/`.ToIso8601()` for machine, …). For an `Instant` in a request/view, the ambient overloads in `Humans.UI.Extensions.DateTimeDisplayExtensions` resolve the user's timezone from session.
