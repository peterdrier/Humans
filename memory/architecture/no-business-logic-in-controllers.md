...
**FROZEN — do not ratchet (Peter, 2026-08-07).** Both dials are off: do **not** lower `MaxStatements`/`MaxCyclomaticComplexity` in `ControllerBusinessLogicAnalyzer`, and do **not** burn down any `[Grandfathered("HUM0031", …)]` method (peterdrier/Humans#1156 cleared all 15; the freeze covers any that reappear). Leave both alone — unfreezing either dial is Peter's explicit call, never a follow-up from a sweep or cleanup pass. nobodies-collective/Humans#857 (the burn-down issue) is closed; the debt-ledger theme `grandfathered-hum0031-controller-logic` is parked. The rule itself stays live at Error for **new** breaches — that part is unchanged.
