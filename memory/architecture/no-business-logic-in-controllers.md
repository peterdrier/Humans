...
**Enforcement** — analyzer `HUM0031` (`ControllerBusinessLogicAnalyzer`, in-editor + build): every method in a `ControllerBase`-derived type — actions **and** private helpers — is measured semantically. Statements > 40 **or** cyclomatic complexity > 15 → Error; pre-existing offenders carry method-level `[Grandfathered("HUM0031", …)]`. The thresholds are deliberately generous (calibrated to the ~15 worst offenders at introduction). If the action does view-model assembly + dispatch only, statements + complexity stay tiny. If it's running a state machine, it's the wrong file.

**FROZEN — do not ratchet (Peter, 2026-08-07).** Both dials are off: do **not** lower `MaxStatements`/`MaxCyclomaticComplexity` in `ControllerBusinessLogicAnalyzer`, and do **not** burn down any `[Grandfathered("HUM0031", …)]` method (peterdrier/Humans#1156 cleared all 15; the freeze covers any that reappear). Leave both alone until the per-section assembly split (nobodies-collective/Humans#866) lands — the controllers move then, so doing this work first is churn against code that is about to relocate. nobodies-collective/Humans#857 (the burn-down issue) is closed for this reason; the debt-ledger theme `grandfathered-hum0031-controller-logic` is parked. The rule itself stays live at Error for **new** breaches — that part is unchanged.
