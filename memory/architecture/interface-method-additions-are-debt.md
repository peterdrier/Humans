...
**Scope:** All interfaces. Applies to every interface in the codebase — `IUserService`, `IUserEmailService`, `IProfileService`, `ITeamService`, `ICampService`, `IShiftManagementService`, `IAuditLogService`, every `I…Repository`, every cross-section interface. Adding a method to any of them is debt and requires the same skepticism. The `[SurfaceBudget(N)]` attribute + HUM0015/HUM0016 analyzer (per-interface caps, declared at the interface) is the in-code enforcement surface; this atom is the principle.
