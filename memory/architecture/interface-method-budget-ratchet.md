...
- 2026-05-23 (issue #279, Frank-authorized — to be confirmed by Peter): +1 on `IShiftManagementService` for `HasQualifyingCantinaSignupAsync`. Same-shape precedent as PR #602's `GetDepartmentCoveragePiesAsync` (single-bool dashboard view-gate helper). Inlining at the call site was considered but would duplicate the active-event + signup + qualifying-shift composition across the dashboard and future consumers. If Peter prefers refactoring to inline, the change is mechanical: drop the method, have ThingsToDoViewComponent call `GetUserActiveSignupsForCantinaGateAsync` directly and apply the same LINQ chain in-line.
