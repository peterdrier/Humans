...
- 2026-05-13 (PR #521, Peter explicit sign-off): +1 on `IUserRepository` for `GetEventParticipationsByUserIdsAsync`. CachingUserService warmup needs all-users bulk EventParticipations to match the bulk shape of the other 7 contributing tables; per-user fetch was N+1. Approved by Peter 2026-05-13.
- 2026-05-13 (PR #521, Peter explicit sign-off): +1 on `IUserService` from 31→32 for canonical `GetUserInfoAsync(Guid)` read-model method enabling the `CachingUserService` decorator (issue #703). Matches the PR #474 ITeamService pattern: read-model entry point that establishes the consolidation target for removing narrower `Get*` helpers once consumers migrate. Approved by Peter 2026-05-13.
