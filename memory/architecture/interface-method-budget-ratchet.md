...
- 2026-05-13 (PR #529, Peter explicit sign-off): `IProfileService` bumped 31 → 32 to add `GetProfilePictureMigrationSnapshotAsync` for the issue #702 admin verification page that gates Phase 2 (#528, drop `Profile.ProfilePictureData`). Reason: counts + DB-only row list both require iterating profile rows against `IFileStorage` with the section-internal `ProfilePictureKey` helper — assembling the snapshot in the controller would leak the key helper out of Profile. Approved by Peter 2026-05-13.
