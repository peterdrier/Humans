...
- 2026-05-10 (PR #474, Peter explicit sign-off): +1 on `ITeamRepository` for `GetAllWithMembersAsync`, needed by the `CachingTeamService` canonical team index because `GetAllActiveWithMembersAsync` excludes inactive teams and `GetAllAsync` does not include members.
