...
- Before acting on any CI or review event on a PR you opened, load the `pd:steward` skill and this repo's overlay [`.claude/steward.md`](.claude/steward.md). Unattended review rounds are capped at five review-round commits — bot/CI response commits only, not the PR's own deliverable ([`review-round-budget`](memory/process/review-round-budget.md)) — past that, stop and surface it. The session that built a PR hands it to a thin steward session at ready-for-review and stops ([`steward-thin-session`](memory/process/steward-thin-session.md)); waiting on a PR means that session's event subscription and nothing else; never schedule periodic self check-ins ([`no-scheduled-pr-checkins`](memory/process/no-scheduled-pr-checkins.md)).
...
- `src/Sections/Humans.<Section>[.Contracts]` — the sections. `ls src/Sections` is the list. Each carries `Docs/<Section>.md` (its invariants, following [`docs/sections/SECTION-TEMPLATE.md`](docs/sections/SECTION-TEMPLATE.md)), and where present `Docs/features/` (its feature specs; cross-section specs live in [`docs/features/global/`](docs/features/global/)) and `Docs/data-access.md` — grep the latter to find which section owns a service, repository, or table.
