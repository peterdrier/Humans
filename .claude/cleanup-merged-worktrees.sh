...
usage() {
  cat <<'EOF'
Usage: .claude/cleanup-merged-worktrees.sh [options]

Finds autonomous-run worktrees created by .claude/run-autonomous.sh and removes
...
Options:
  ...
  --branch-prefix <prefix>     Branch prefix to match (default: claude/weekly-)
  --worktrees-dir <path>       Worktree directory root
  --apply                      Remove merged worktrees and delete their branches
  --help                       Show this help

Examples:
  .claude/cleanup-merged-worktrees.sh
  .claude/cleanup-merged-worktrees.sh --into develop
  .claude/cleanup-merged-worktrees.sh --apply
EOF
}
...
if ((${#candidates[@]} == 0)); then
  echo "No matching worktrees found under $worktrees_root"
  exit 0
fi
