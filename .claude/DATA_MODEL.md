...
| BudgetLineItem | Detail row within a category (free-text description + amount). `IsAutoGenerated` for ticketing sync items, `IsCashflowOnly` for items excluded from budget totals (e.g. donations) |
| BudgetAuditLog | **APPEND-ONLY** field-level change tracking for budget entities (old/new values) |
| TicketingProjection | Projection parameters for ticketing budget group (1:1 with BudgetGroup where IsTicketingGroup=true) |
...
BudgetGroup 1──0..1 TicketingProjection (only where IsTicketingGroup=true)
