...
- **Worktrees here, always — including in a cloud run.** [`always-use-worktree`](../../../memory/process/always-use-worktree.md) sends a cloud session to the repo root because the container is single-session, but this skill's lanes are concurrent writers on separate branches within *one* session: they need separate checkouts from each other, not from other sessions. That reason survives the cloud. This is the documented exception; every other worktree-opening skill branches on the environment.
...
5. Create one worktree+branch per lane off the baseline sha: `refactor/<YYYY-MM-DD>-<section>-workflow-N` at `.claude/worktrees/refactor-<YYYY-MM-DD>-<section>-workflow-N`. **Verify each worktree's branch + base sha before fan-out.**
...
- A **wave-end docs commit** (one PR, by the coordinator — not per-lane, to avoid conflicting edits) updating the **Section Refactor History** table in `docs/architecture/maintenance-log.md`: Last Lane (date + PR) and Post-Lane Score (final built section score) for each served section, plus a re-snapshot of the Score column for all sections.
