...
| 2 | **Reachable?** Write the concrete path: entry point → inputs → state → wrong outcome, using the app's real UI/routes/jobs. | The path needs a state the app never produces: violates an invariant enforced elsewhere, emailless user, concurrent writers at ~3000 users/single server, malformed rows the app never writes, a `null` the constructor/EF/`[Required]` already excludes, a rollback path that never runs. | `IMPOSSIBLE` 👎 |
