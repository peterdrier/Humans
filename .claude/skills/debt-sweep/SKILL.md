...
4. **EF model-drift gate** (themes touching entities, navs, or `Data/Configurations`): the sweep never creates migrations, so verify the fix didn't silently change the model. More than one `DbContext` is in play (nobodies-collective/Humans#858, #866), so the bare command errors — run `has-pending-model-changes --context <C>` for the touched entity's owning context per `memory/process/ef-multi-context-commands.md`, e.g. `dotnet ef migrations has-pending-model-changes --context HumansDbContext --project src/Humans.Infrastructure --startup-project src/Humans.Web`; pending changes → revert the item, classify it as schema work, record it (Phase 7 question or inbox).
