...
2. **Staleness check (every run, cheap):**
   - Distinct `HUM####` ids in real `[Grandfathered(` attribute usages vs. ledger theme ids → each missing rule becomes a new theme (`last_swept: never`, `review: light` unless it is plainly a structural/judgment rule — then `panel`). **Anchor the grep to attribute syntax** — an unanchored `[Grandfathered(` also matches analyzer doc-comments and message strings, which seeds phantom themes:
     ...
     ```
   - Files in `tests/Humans.Web.Tests/Architecture/Baselines/` with >0 non-comment entries vs. ledger → same.
...
4. **EF verification** (items changing the persistent model): follow [section-migrations-in-maintenance](../../../memory/process/section-migrations-in-maintenance.md). Generate the owning context's migration with `dotnet ef`, inspect its snapshot diff, run `has-pending-model-changes` with that context and project, and pass the EF migration review gate before commit. Use `memory/process/ef-multi-context-commands.md` for the context/project pair. Unexpected model changes require investigation; a needed migration alone is not a reason to skip the item. Code-only changes that cannot alter the schema need no EF tooling.
...
- **Stop-and-ask classes are skip-and-ask classes here:** interface/public-surface additions (`interface-method-additions-are-debt`), schema changes requiring specific approval (such as storage drops or required columns), privilege changes → skip the item, queue a Phase 7 question. Substantial architecture transitions also go to explicit planning and dedicated PRs. Never block the loop waiting.
...
| Unexpected EF model drift | Investigate; generate/review a justified section migration, or queue work needing approval/design |
