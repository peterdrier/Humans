...
public sealed class ExpenseReportService(
    ...
    IOptions<SepaConfig> sepaConfig) : IExpenseReportService,
        IExpenseReportBackgroundProcessor, IUserDataContributor
{
    ...
    private readonly SepaConfig _sepa = sepaConfig.Value;
    ...
    public async Task<SepaPayoutResult> GenerateSepaPayoutAsync(
        ...
        CancellationToken ct = default)
    {
        // Re-check eligibility on fresh state: only Approved reports enter the batch.
        var eligible = new List<ExpenseReportDto>();
        foreach (var id in reportIds.Distinct())
        {
            var report = await repo.GetByIdAsync(id, ct);
            if (report is { Status: ExpenseReportStatus.Approved })
                eligible.Add(report);
        }
        ...
        if (eligible.Count == 0)
        {
            return SepaPayoutResult.Failure(
                "None of the selected reports could be included in the SEPA payout. Reports must be in Approved status.");
        }

        // Retry-safety invariant: build the XML BEFORE flipping to SepaSent, so a
        // failure at either step leaves every report Approved and regenerable.
        var now = clock.GetCurrentInstant();
        var xml = sepaBuilder.BuildPain001(_sepa, now, eligible);

        var flippedIds = await MarkSepaSentAsync(eligible.Select(r => r.Id).ToList(), actorUserId, ct);
        if (flippedIds.Count == 0)
        {
            return SepaPayoutResult.Failure(
                "No reports were transitioned to SEPA Sent. They may have already been processed.");
        }
        ...
        return SepaPayoutResult.Success(xml, fileName, flippedIds);
    }

    internal async Task<IReadOnlyList<Guid>> MarkSepaSentAsync(
        IReadOnlyCollection<Guid> reportIds, Guid actorUserId,
        CancellationToken ct = default)
    {
