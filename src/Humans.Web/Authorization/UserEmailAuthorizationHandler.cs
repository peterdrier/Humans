...
/// - Actor in the <see cref="RoleNames.Admin"/>, <see cref="RoleNames.HumanAdmin"/>,
///   or <see cref="RoleNames.Board"/> role: allow
/// - Everyone else: deny
///
/// Self/admin distinction lives in one handler so the self routes and the
/// admin routes share authorization. The HumanAdmin/Board roles match the
/// Profiles section invariants — those actors "Manage humans via admin pages"
/// (see <c>docs/sections/Profiles.md</c>); admin email management is part of
/// that surface. Service signatures stay auth-free per design-rules.md.
/// </summary>
    ...
    : AuthorizationHandler<UserEmailOperationRequirement, Guid>
{
        ...
        Guid targetUserId)
    {
        ...
        if (context.User.IsInRole(RoleNames.Admin)
            || context.User.IsInRole(RoleNames.HumanAdmin)
            || context.User.IsInRole(RoleNames.Board))
        {
            context.Succeed(requirement);
        }
