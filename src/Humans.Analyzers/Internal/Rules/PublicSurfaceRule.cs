...
/// exception: a deliberate surface (<c>Contracts/</c>, the <c>ISection</c> entry point,
/// the <c>&lt;Section&gt;Resource</c> marker) and types the framework silently drops when
/// they are internal (view components, tag helpers, EF migrations).
/// </summary>
/// <remarks>
...
internal static class PublicSurfaceRule
{
    ...
    public static readonly DiagnosticDescriptor Rule = new(
        ...
        messageFormat:
            ...
            + "surface is Contracts/, its Section entry point, its <Section>Resource marker, EF "
            + "migrations, and types the framework needs public (view components, tag helpers). "
            + "Make '{0}' internal, or move it under Contracts/ if another section needs it.",
        ...
        description:
            ...
            + "belongs under Contracts/ instead (design §10, §6a).");
    ...
    /// sources too.
    /// </summary>
    public static bool IsUnderContracts(INamedTypeSymbol type)
    {
        var ns = type.ContainingNamespace?.ToDisplayString();
        if (ns is not null && ns.Split('.').Any(s => string.Equals(s, ContractsSegment, System.StringComparison.Ordinal)))
            return true;
        ...
        foreach (var syntaxRef in type.DeclaringSyntaxReferences)
        {
            ...
            if (normalized.IndexOf("/" + ContractsSegment + "/", System.StringComparison.Ordinal) >= 0)
                return true;
        }
        return false;
    }
