...
public sealed class SectionPublicSurfaceAnalyzer : DiagnosticAnalyzer
{
    ...
    /// </summary>
    /// <remarks>
    /// Namespace first: folder <c>Contracts/</c> maps to a namespace segment
    /// named <c>Contracts</c> by convention, and it needs no file path to check
    /// (real code or test). Falls back to the declaring file's path for the case
    /// that convention misses — the same dual check <c>ConcurrencyTokenAnalyzer</c>'s
