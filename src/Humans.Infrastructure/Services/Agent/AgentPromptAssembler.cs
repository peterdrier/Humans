...
public sealed class AgentPromptAssembler : IAgentPromptAssembler
{
    ...
    private const string SystemPromptHeader = """
        ...
        2. Match the question against BOTH the Section Index and the Community FAQ index (including the `covers:` keywords). App-mechanics questions map to sections; event / community / on-site / logistics / history questions map to community topics. Many questions touch both — consult both. Pick the closest matches if unsure; you may pick several.
        3. Fetch the relevant docs: `fetch_section_guide` for each matching section and/or `fetch_community_faq` for each matching community topic. Do NOT answer substantive questions from the indexes alone — they are only routers.
