...
- **Users/Identity**: `IUserService.GetUserInfoAsync` / `GetUserInfosAsync` — display names for the Holded contact, and the submitter's profile, which is where the IBAN and legal name are read from at submit time and masked for the GDPR export. `SetProfileIbanAsync` writes the member's profile IBAN from `/Expenses/{id}/Iban`. `UserInfo.MergedUserIds` off `GetUserInfoAsync` — GDPR merge-tombstone chain-follow. There is no `IProfileService` dependency.
