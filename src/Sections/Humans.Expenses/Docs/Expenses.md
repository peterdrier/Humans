...
| Note | string? | optional submitter-written **Subject**; stays in Humans — the Holded docs' `Description` is `ER: ` plus each line's description, with the report reference in the doc's `Notes` |
...
| `/Expenses/{id}/Endorse` | POST | Authenticated (coordinator or FinanceAdmin, resource-based) | Endorse |
...
- **The push books one Holded purchase document per bookable line**, in `SortOrder`, each carrying that line's description (as `ER: ` + description, with the report reference in the doc's notes), amount, category account, and its one attachment. Proof rows get neither doc lines nor uploads. Reports pushed before per-line docs keep their single report-level `HoldedDocId`; a re-queue of one resumes onto that doc (remaining uploads + approve) and never mints per-line docs beside it.
...
- On **any successful edit to another member's report**: audit entry `ExpenseEditedOnBehalf` written per change — header update, line add (proof rows included), line update, line remove — with a description saying what changed. A combined line-and-attachment add writes the line audit only after the attachment is stored; a failed upload rolls the line back without claiming the edit happened. Self-edits stay unaudited, exactly as before.
...
- **Users/Identity**: `IUserService.GetUserInfoAsync` / `GetUserInfosAsync`, display names for the Holded contact, and the submitter's profile, which is where the IBAN and legal name are read from at submit time and masked for the GDPR export. `SetProfileIbanAsync` writes the member's profile IBAN from `/Expenses/{id}/Iban`. `UserInfo.AllUserIds` off `GetUserInfoAsync`, GDPR merge-tombstone chain-follow, and the Holded push binds the creditor contact to the resolved submitter (`UserInfo.Id`) and seeds it from reports under any id in that list. There is no `IProfileService` dependency.
- **Email**: `IEmailService.SendAsync` for the approval email. The template, its resx keys and its `/Email/EmailPreview` sample are this section's (`ExpensesEmails`, `ExpensesEmailPreviews`); Email keeps the mechanics (memory/architecture/email-templates-live-in-sender.md). `IUserEmailService.GetNotificationTargetEmailsAsync` resolves the submitter's address.
...
- `ExpenseReportService` lives in `Humans.Expenses.Services` and depends only on Application-layer abstractions. `IExpenseReportService` (internal) carries its reads and mutations; there is no cross-section read interface because no other section reads a claim — add one when a reader appears. The public surface is `Section` plus everything under `Contracts/`: `IExpenseReportBackgroundProcessor` (`DrainHoldedOutboxAsync`, how `HoldedExpenseOutboxJob` reaches the section) and the DTO/enum graph. The job lives in this project's `Jobs/` folder; the section registers and schedules it itself (`Section.cs`, `SectionJobs.cs`).
