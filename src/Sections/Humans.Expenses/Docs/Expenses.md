...
- **Only approved purchase documents can be matched.** Drafts are excluded from the pool via `IHoldedClient.ListDraftPurchaseIdsAsync` before matching — a draft books nothing, so linking one would mark the commitment Invoiced and drop it off the liability list with no real invoice behind it.
- A commitment that was written but whose quote failed to store is reported as a **partial failure carrying its id**, never as an outright failure: the screen sends the operator to the commitment to re-upload the quote, so a storage fault cannot turn into a duplicate liability.
