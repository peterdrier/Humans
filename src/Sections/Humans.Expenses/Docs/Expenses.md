...
- Only the deciders set `MaxAmount` — the coordinator on Endorse, a finance admin on Approve (the approve-time value overrides the endorsement's). The submitter can never set or see a cap input, and neither decider path may lower it below 0.01 or above 1,000,000. A cap is recorded in the decision's own `ExpenseEndorse` / `ExpenseApprove` audit entry, not a separate action.
