...
- `IbanAccessRequirement` / `IbanAccessHandler` encode a self / FinanceAdmin-with-report-context / Admin-on-admin-page matrix, but **nothing constructs the requirement today** — the handler is registered in DI and reached only by its tests. The live gates are `[Authorize(Policy = AdminOnly)]` on `/Users/Admin/{id}/RevealIban` and `CanSetReportIbanAsync` on `/Expenses/{id}/Iban`. See `authorization.md`.
...
- **Admin (Users section)**: `/Users/Admin/{id}/RevealIban` lives in `UsersAdminController` (`Humans.Users`) and is gated by `[Authorize(Policy = AdminOnly)]` — not by this section's `IbanAccessHandler`.
