...
Implements `IRideshareService`, `IUserDataContributor` and `IUserMerge` (both delegate to the
inner service, then clear the cache), and exposes `ICacheStats SnapshotCacheStats`. No warmup —
