...
- **Rosters are admin-and-driver only.** A trip's accepted riders are visible to that trip's driver (via `Mine`) and to Admins (Day roster); never on the public board.
...
- **ExpressInterest requires capacity and consent.** The trip must be `Active` with `SeatsRemaining ≥ seats` (seats ≥ 1); a human cannot express interest in their own trip; when `requestId` is given, the request must be `Active` and the caller must be the trip's owner (driver answering a pin); a duplicate `Pending` interest by the same user on the same trip+request pair is rejected.
- **Accept requires ownership, pending state, and capacity.** The actor must be the posting owner (`requestId != null ? request.UserId : trip.UserId`); the interest must be `Pending`; the trip must still have `SeatsRemaining ≥ interest.Seats`.
