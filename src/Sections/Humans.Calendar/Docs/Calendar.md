...
**Owning services:** `CalendarService` (keyed inner: mutations plus the two row loads the cache warms and refreshes from), `CachingCalendarService` (decorator exposing `ICalendarService` and `ICalendarServiceRead`), `ICalFeedService` (personal iCal feed orchestrator — owns no tables, injects no repository)
