...
- A title, description, or location override applies only to an occurrence generated by the current rule. Shortening or changing a series does not resurrect removed occurrences from text-only overrides; explicitly rescheduled occurrences can still appear outside the series window.
