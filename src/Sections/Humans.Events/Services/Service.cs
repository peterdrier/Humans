    ...
    : IEventService, ICalendarFeedContributor
{
    ...
    {
        // TimeZoneId is stitched in from the Settings-owned event settings row via
        // ISettingsService (cross-section supplier API, nobodies-collective/Humans#1629).
        var burn = await eventSettings.GetEventSettingsByIdAsync(settings.EventSettingsId, ct);
    ...
    {
        var burn = await eventSettings.GetEventSettingsByIdAsync(eventSettingsId, ct)
    ...
    public async Task<IReadOnlyList<CalendarFeedItem>> GetCalendarItemsForUserAsync(Guid userId, CancellationToken ct)
    {
        ...
        var burn = guideSettings is null ? null : await eventSettings.GetEventSettingsByIdAsync(guideSettings.EventSettingsId, ct);
