...
- **Users**: controllers call `IUserServiceRead.GetUserInfoAsync(userId)` for submitter and moderator display names (email as fallback); the current user comes from `HumansControllerBase` over the same interface.
