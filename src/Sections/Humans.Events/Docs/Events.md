...
- **Settings (event settings)** — `EventGuideSettings.EventSettings` navigation was dropped along with the cross-section FK. The Events section reads the linked event (`app_event_settings` row owned by Settings since nobodies-collective/Humans#1104) via `ISettingsServiceRead.GetEventSettingsByIdAsync(EventGuideSettings.EventSettingsId)`, which returns an `EventSettingsInfo` DTO (identity, timezone, gate-opening date, build-calendar offsets, EE capacity) — the Settings-internal entity never crosses the section boundary. Issue [#719](https://github.com/nobodies-collective/Humans/issues/719).
...
**Status:** (A) Migrated — PR peterdrier#374, 2026-04-30. Moved into its own project `src/Sections/Humans.Events` at G5 (nobodies-collective/Humans#866), with the cross-section leaf `Humans.Events.Contracts`. Everything but `Section`, `EventsResource` and the migrations is `internal` — HUM0034 enforces it, including controller-nested request/view-model types.
...
- **DbContext** — `EventGuideDbContext` (`src/Sections/Humans.Events/Data/EventGuideDbContext.cs`, `internal sealed`) is the section's own per-section EF model (nobodies-collective/Humans#858 split): maps `event_guide_settings`, `event_categories`, `event_venues`, `events`, `event_moderation_actions`, `event_preferences`, `event_favourites`, with its own `__EFMigrationsHistory_EventGuide` table and migrations under `Data/Migrations/` (baseline `20260715112050_BaselineEventGuide`). The similarly-named `app_event_settings` (Settings) and `event_settings` / `event_participations` (Shifts) tables live on their own sections' contexts — hence `EventGuideSettings.EventSettingsId` remains a bare cross-context FK read via `ISettingsServiceRead`.
...
- **Cross-domain navs** — Stripped (PR #539, Stage 3). `Event.CampId`, `Event.SubmitterUserId`, `EventModerationAction.ActorUserId`, `EventFavourite.UserId`, `EventPreference.UserId`, and `EventGuideSettings.EventSettingsId` are bare FK columns — no navigation properties, no DB-level FK constraints, no cross-section `.Include()` chains. Camp / User / event-settings data is fetched via supplier services (`ICampService`, `IUserService`, `ISettingsServiceRead`). `TimeZoneId` is cached at warm time on `EventGuideSettingsView` and stays stale on direct event-settings edits until the next event-section write or process restart — invalidation hook still pending under [#719](https://github.com/nobodies-collective/Humans/issues/719).
- **Cross-section calls** — `UserManager<User>` (Identity, in controllers only), `ICampService.GetCampsForYearAsync`, `IUserService.GetUserInfoAsync`, `IEmailService` (in `EventService` — submission confirmation and moderation-outcome emails). `EventService` implements `Humans.Calendar.Contracts.ICalendarFeedContributor` to supply a user's favourited approved events to Calendar's personal iCal feed fanout (PR peterdrier#931; the fan-out moved from Base into `Humans.Calendar` at G5 lane 4b-2c, so `Humans.Events` references `Humans.Calendar`).
...
- **Read/write interface split (PR peterdrier#915).** `IEventServiceRead` (`Humans.Events.Contracts`, the section's leaf project) is the cross-section read surface — 4 methods: `GetApprovedEventsAsync`, `GetApprovedEventByIdAsync`, `GetGuideSettingsAsync`, `GetFavouriteEventIdsAsync` — returning only the cached projections (`ApprovedEventView`, `EventGuideSettingsView`, favourite-id set); no EF entities. `IEventService : IEventServiceRead` adds submission/moderation/settings writes and Events-internal reads. `IEventServiceRead` is registered as a forward to the `CachingEventService` Singleton (`Section.Register`), so cross-section reads are served from the T-03 approved-only cache — interface segregation only, no new cache layer. External consumers: the Events-owned `EventsCardViewComponent` composed onto the Camp detail page and Profile pages (invoked by name from Camps' and Users' views; see Cross-Section Dependencies). External sections inject `IEventServiceRead`. See `memory/architecture/section-read-write-split.md`.
