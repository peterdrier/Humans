...
- **Users**: controllers call `IUserServiceRead.GetUserInfoAsync(userId)` for submitter display name and email; the current user comes from `HumansControllerBase` over the same interface.
