...
- **Camps and Profiles (downstream consumers):** the Camp detail page (`/Camps/{slug}`) renders the Events-owned `EventsCardViewComponent` (`src/Sections/Humans.Events/ViewComponents/EventsCardViewComponent.cs`) through Camps' `ICampPart` seam (`Section.cs` contributes `new CampPart(typeof(EventsCardViewComponent))`; Camps' `CampPartsViewComponent` invokes it by `Type` with `CampPartArgs(CampId)`, nobodies-collective/Humans#1815), which reads `IEventServiceRead` to list the camp's approved events — title, category, description, schedule, venue, host — with per-row favourite hearts that toggle in place via the JSON favourites API (`POST /api/events/favourites/{eventId}`, no page reload). Events contributes the same component to profile pages through `IUserPart`; Users and Camps each compose their own descriptor and supply the target id, so neither view names Events. The component auto-hides when there are no approved events or the Events feature is off. Profile cards show only the user's personal (non-camp) submitted events — events submitted under a camp appear on the camp's page.
