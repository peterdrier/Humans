...
- **Camps (downstream consumer):** the Camp detail page (`/Camps/{slug}`) embeds the Events-owned `EventsCardViewComponent` (`src/Sections/Humans.Events/ViewComponents/EventsCardViewComponent.cs`), which reads `IEventServiceRead` to list the camp's approved events — title, category, description, schedule, venue, host — with per-row favourite hearts that toggle in place via the JSON favourites API (`POST /api/events/favourites/{eventId}`, no page reload). The component lives in the Events section itself and is invoked by name (`Component.InvokeAsync("EventsCard", …)`) from Camps' and Users' views — Camps/Users services do not depend on Events, and no Event types cross into their views (the component is invoked with the camp's or user's id only). Auth-gated at the call site; auto-hides when the camp has no approved events or the Events feature is off. Profile pages also embed the card scoped to `userId`, showing only the user's personal (non-camp) submitted events — events they submitted under a camp appear on the camp's page, not the profile card.
...
- **Settings (event settings)** — there is no `EventSettings` navigation. The Events section reads the linked event cycle via `ISettingsService.GetEventSettingsByIdAsync(EventGuideSettings.EventSettingsId)`, which returns an `EventSettingsInfo` DTO (identity, timezone, gate-opening date, build-calendar offsets, EE capacity) — the Settings-internal entity never crosses the section boundary. Issue [#719](https://github.com/nobodies-collective/Humans/issues/719).
