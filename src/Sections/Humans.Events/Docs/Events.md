...
| DurationMinutes | int | 15–1440 on the individual and admin forms (1440 = the all-day encoding); 15–480 in 15-minute steps on the barrio form and bulk CSV |
| IsRecurring | bool | |
...
- **Users**: controllers call `IUserServiceRead.GetUserInfoAsync(userId)` for submitter display names (email as fallback) — moderation actors render through `<vc:human>` from `ActorUserId`; the current user comes from `HumansControllerBase` over the same interface.
- **Camps**: controllers call `ICampServiceRead.GetCampsForYearAsync(year)` to resolve camp display data per event. `Event.CampId` remains a bare FK column. Camp-event submission authority on `EventsController` barrio actions is sourced from `CampOperationRequirement.SubmitEvent` resource authorization (backed by `CampInfo.IsEventManager`) — the Lead OR Workshop OR-check that consumes `CampRoleAssignment` rows whose `CampRoleDefinition.SpecialRole` is `Lead` or `Workshop` (issue nobodies-collective/Humans#753). Moderation authority remains global (EventsAdmin / Admin) — no camp-scoped moderation.
- **Camps and Profiles (downstream consumers):** the Camp detail page (`/Camps/{slug}`) embeds the Events-owned `EventsCardViewComponent` (`src/Sections/Humans.Events/ViewComponents/EventsCardViewComponent.cs`), which reads `IEventServiceRead` to list the camp's approved events — title, category, description, schedule, venue, host — with per-row favourite hearts that toggle in place via the JSON favourites API (`POST /api/events/favourites/{eventId}`, no page reload). Events contributes the same component to profile pages through `IUserPart`; Users composes the descriptor and supplies the target `userId`, so its view no longer names Events. The component auto-hides when there are no approved events or the Events feature is off. Profile cards show only the user's personal (non-camp) submitted events — events submitted under a camp appear on the camp's page.
- **Search (downstream consumer):** the global `/Search` page renders every event hit through this section's own public `<vc:events-search-result event-id>`, which reads the row via `IEventServiceRead.GetApprovedEventByIdAsync` (an O(1) hit on the approved-only cache) and builds the `/Events/Browse?q=` link itself. Search passes the id and no display fields (nobodies-collective/Humans#1062). No `Features:Events` check on the component: the flag already gates the only producer of those ids. Events does not depend on Search.
- **Shifts (burn settings)** — there is no `EventSettings` navigation. The Events section reads the linked burn (`event_settings` row owned by Shifts) via `ISettingsService.GetEventSettingsByIdAsync(EventGuideSettings.EventSettingsId)`, which returns a `EventSettingsInfo` DTO (identity, timezone, gate-opening date, build-calendar offsets, EE capacity) — the Shifts-internal entity never crosses the section boundary. Issue [#719](https://github.com/nobodies-collective/Humans/issues/719).
- **Email**: `IEmailService` (injected into `EventService`) for submission confirmation and moderation outcome notifications.

## Architecture

**Owning services:** `EventService` (`Humans.Events.Services`), `EventRepository` (`Humans.Events.Data`)
...
**Status:** (A) Migrated — own project `src/Sections/Humans.Events` with the cross-section leaf `Humans.Events.Contracts`. Everything but `Section`, `EventsResource` and the migrations is `internal` — HUM0034 enforces it, including controller-nested request/view-model types.

### For (A) Migrated sections

- `EventService` lives in `src/Sections/Humans.Events/Services/` and never imports `Microsoft.EntityFrameworkCore`.
- `IEventRepository` (impl `EventRepository` in `src/Sections/Humans.Events/Data/`, `IDbContextFactory<EventGuideDbContext>`) is the only code path that touches this section's tables via `DbContext`.
- **DbContext** — `EventGuideDbContext` (`src/Sections/Humans.Events/Data/EventGuideDbContext.cs`, `internal sealed`) is the section's own per-section EF model: maps `event_guide_settings`, `event_categories`, `event_venues`, `events`, `event_moderation_actions`, `event_preferences`, `event_favourites`, with its own `__EFMigrationsHistory_EventGuide` table and migrations under `Data/Migrations/` (baseline `20260715112050_BaselineEventGuide`). The app-wide event calendar lives in Settings' `settings_event` (`SettingsDbContext`), the similarly-named `event_settings` table is Shifts' own knobs row (`ShiftsDbContext`), and `EventParticipations` is Users-owned (`UsersDbContext`) — hence `EventGuideSettings.EventSettingsId` remains a bare cross-context id read via `ISettingsService`. Same database and connection as every other section context.
- **Decorator** — **§15 caching decorator**. Only approved events are cacheable and every write routes through the decorator, so post-moderation invalidation is inline. The public CORS API (`/api/events/events`) is the read path the cache absorbs; the moderation dashboard (`GetAllEventsForDashboardAsync`) stays direct-DB because it needs a fresh pending count the approved-only cache cannot answer. Split projections: `ApprovedEventView` (per-event dict keyed by id, pre-stitched with `Category.Slug/Name/IsSensitive` and `Venue.Name`), flat `EventCategoryView[]`, flat `EventVenueView[]`, and the `EventGuideSettingsView` singleton (pre-stitched with foreign `EventSettings.TimeZoneId`). In-memory filtering in C# handles the `(campId, venueId, categoryId, q)` browse params against the cached snapshot. No `SaveChangesInterceptor` — every event_* write flows through `IEventService` by design (enforced by the universal `HUM0025` analyzer — only `EventRepository` references the Event DbSets), so the decorator handles invalidation inline after each delegated write. `IEventViewInvalidator` exposes the in-section hook for guide-settings edits; the decorator also implements Settings' `IEventSettingsChangeListener`, so an event-settings save marks the `EventGuideSettingsView` projection stale and the next read reloads it (issue [#719](https://github.com/nobodies-collective/Humans/issues/719), peterdrier/Humans#1627). Warmed eagerly at startup (`CachingEventService` registers itself as the `IHostedService` and runs `WarmAllAsync`); warmup failures are logged and swallowed (lazy population on miss still works).
- **Cross-domain navs** — none. `Event.CampId`, `Event.SubmitterUserId`, `EventModerationAction.ActorUserId`, `EventFavourite.UserId`, `EventPreference.UserId`, and `EventGuideSettings.EventSettingsId` are bare FK columns — no navigation properties, no DB-level FK constraints, no cross-section `.Include()` chains. Camp / User / burn-settings data is fetched via supplier services (`ICampServiceRead`, `IUserServiceRead`, `ISettingsService`). `TimeZoneId` is cached at warm time on `EventGuideSettingsView`; an event-settings save now reaches this section through `IEventSettingsChangeListener`, which marks that projection stale so the next read reloads it. Before that it stayed stale until the next event-section write or a process restart ([#719](https://github.com/nobodies-collective/Humans/issues/719), peterdrier/Humans#1627).
- **Cross-section calls** — `ICampServiceRead.GetCampsForYearAsync`, `IUserServiceRead.GetUserInfoAsync`, `ISettingsService.GetEventSettingsByIdAsync`, `IEmailService` (in `EventService` — submission confirmation and moderation-outcome emails). `EventService` implements `Humans.Calendar.Contracts.ICalendarFeedContributor` to supply a user's favourited approved events to Calendar's personal iCal feed fanout (so `Humans.Events` references `Humans.Calendar`).
- **Architecture test/analyzer** — `tests/Humans.Events.Tests/EventsArchitectureTests.cs` pins the service/repository split, the canonical `Events` / `Events/Dashboard` / `Events/Export` / `Events/Moderate` / `api/events` route names, and the caching invariants (decorator wraps `IEventService`, `IEventViewInvalidator` shares the decorator's Singleton, inner `EventService` injects no caching abstraction). The universal `HUM0025` (a DbSet table must be referenced by exactly one repository) enforces that only `EventRepository` touches the Event Guide DbSets.
- **Read/write interface split.** `IEventServiceRead` (`Humans.Events.Contracts`, the section's leaf project) is the cross-section read surface — `GetApprovedEventsAsync`, `GetApprovedEventByIdAsync`, `GetGuideSettingsAsync`, `GetFavouriteEventIdsAsync` and `SearchAsync` — returning only the cached projections (`ApprovedEventView`, `EventGuideSettingsView`, favourite-id set, `EventSearchHit`); no EF entities. `IEventService : IEventServiceRead` adds submission/moderation/settings writes and Events-internal reads. `IEventServiceRead` is registered as a forward to the `CachingEventService` Singleton (`Section.Register`), so cross-section reads are served from the approved-only cache — interface segregation only, no new cache layer. External consumers: the Events-owned `EventsCardViewComponent`, composed directly onto the Camp detail page and contributed to Profile pages through `IUserPart`, and `SearchService.SearchAsync` (nobodies-collective/Humans#1062). External sections inject `IEventServiceRead`. See `memory/architecture/section-read-write-split.md`.
