...
- **Resource set — 12 keys of 17, and the split is by renderer.** `SearchResource` owns the six `Search_Filter*` and six `Search_Global*` keys, i.e. all of `/Search`'s own copy. Five keys keep the `Search_` prefix and stayed in `SharedResource` because their renderers are elsewhere: `Search_Title` / `Search_Placeholder` belong to Shell's `/Profile/Search` person-search page, `Search_NoResults` / `Search_MatchedIn` to the `Humans.UI` `_HumanSearchResults` partial that four pages render, and `Search_MinChars` is read by `/Profile/Search` **and** by this section's `Index.cshtml` — so the section binds `SharedLocalizer` for that one call site rather than splitting the person-search page's four-key message set. A prefix is not an owner.
- **Presentation shared with Users/Profiles lives in `Humans.UI`, not here.** `_HumanSearchResults.cshtml`, `HumanSearchResultViewModel` and `SearchResultMappingExtensions.ToHumanSearchViewModel` moved down from `Humans.Web` at the G5 move: the Humans bucket renders through the same partial as `/Profile/Search`, `/Users/Admin` and the widget gallery, and a section cannot name a `Humans.Web` type. None of the three carries any section's vocabulary, which is the test. Taking them into `Humans.Search` would have broken those three Shell pages; duplicating them would have forked the shape the shared partial exists to keep.
