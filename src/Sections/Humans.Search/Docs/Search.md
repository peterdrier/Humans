...
- `SearchService` lives in `Humans.Search.Services` and the section project takes no `Humans.Infrastructure` reference at all — Scanner's and Cantina's table-less shape. `SearchArchitectureTests.NoTypeInTheSectionTouchesDataAccess` sweeps every constructor in the assembly for a `DbContext`, an `IDbContextFactory<>`, a repository or a `Stores` type, which is the orchestrator claim stated over the whole section rather than over one constructor.
...
- **Resource set — 12 keys of 17, and the split is by renderer.** `SearchResource` owns the six `Search_Filter*` and six `Search_Global*` keys, i.e. all of `/Search`'s own copy. Five keys keep the `Search_` prefix and stayed in `SharedResource` because their renderers are elsewhere: `Search_Title` / `Search_Placeholder` belong to Shell's `/Profile/Search` person-search page, `Search_NoResults` / `Search_MatchedIn` to the `Humans.Users` `_HumanSearchResults` partial that four pages render, and `Search_MinChars` is read by `/Profile/Search` **and** by this section's `Index.cshtml` — so the section binds `SharedLocalizer` for that one call site rather than splitting the person-search page's four-key message set. A prefix is not an owner.
- **Presentation shared with Users/Profiles is not this section's.** `HumanSearchResultViewModel` and `SearchResultMappingExtensions.ToHumanSearchViewModel` moved down from `Humans.Web` into `Humans.UI` at the G5 move; `_HumanSearchResults.cshtml` went with them and then moved on to `Humans.Users` at G5 lane 4b-i (nobodies-collective/Humans#866) — it was Base's last `Humans.Governance.Contracts` site, via `StatusBadgeExtensions`. The Humans bucket renders through the same partial as `/Profile/Search`, `/Users/Admin` and the widget gallery; partial lookup is by name across application parts, so this section renders a partial another section owns and needs **no** reference to do it — the model it binds is still Base's. Taking the partial into `Humans.Search` would have broken those three Shell pages; duplicating it would have forked the shape it exists to keep. nobodies-collective/Humans#1061 replaces the arrangement with a view component per section.
...
- **Test coverage — structure and behaviour pinned as of #1197/#1198, with one hole left.** That hole is the Events cache filter (last bullet below), and it is what still holds G3 predicate 3 at **FAIL** in the audit. Structure: `tests/Humans.Search.Tests/Architecture/SearchArchitectureTests.cs` (#1197, moved and extended at G5) — `ISearchService_ImplementsOrchestratorNotApplicationService`, `SearchService_HasNoRepositoryDependency`, `SearchService_DependsOnlyOnServiceInterfaces`, `NoTypeInTheSectionTouchesDataAccess`, `TheSectionExportsOnlyItsSectionAndResourceMarkers`. Rendering: `tests/Humans.Integration.Tests/Controllers/SearchPageRenderTests.cs` (7 tests) — the page in English and Spanish, both empty-query branches, the no-results path, the `filter=` bind, the anonymous redirect, and no raw `Search_` key anywhere. Behaviour (#1198):
  ...
  - **Controller** — `tests/Humans.Search.Tests/Controllers/SearchControllerTests.cs`, 7 tests: non-human buckets sorted score-desc-then-title-asc, humans by relevance then burner name, view-model projection, and the shell-not-500 path on a service throw.
