...
- `SearchService` lives in `Humans.Search.Services` and the section project has no `Data/` folder, no `DbContext` and no repository reference at all — Scanner's and Cantina's table-less shape. `SearchArchitectureTests.NoTypeInTheSectionTouchesDataAccess` sweeps every constructor in the assembly for a `DbContext`, an `IDbContextFactory<>`, a repository or a `Stores` type, which is the orchestrator claim stated over the whole section rather than over one constructor.
...
- **Presentation shared with Users/Profiles is not this section's.** `HumanSearchResultViewModel` and `SearchResultMappingExtensions.ToHumanSearchViewModel` moved down from `Humans.Web` into `Humans.UI` at the G5 move, then on into `Humans.Interfaces` once `Humans.UI` itself was deleted (nobodies-collective/Humans#866), and now live in `Humans.Base` since the Interfaces→Base rename; `_HumanSearchResults.cshtml` moved separately, on to `Humans.Users` at G5 lane 4b-i — it was Base's last `Humans.Governance.Contracts` site, via `StatusBadgeExtensions`. The Humans bucket renders through the same partial as `/Profile/Search`, `/Users/Admin` and the widget gallery; partial lookup is by name across application parts, so this section renders a partial another section owns and needs **no** reference to do it — the model it binds is `Humans.Base`'. Taking the partial into `Humans.Search` would have broken those three Shell pages; duplicating it would have forked the shape it exists to keep. nobodies-collective/Humans#1061 replaces the arrangement with a view component per section.
