...
- **Teams / Camps / Shifts / Events are still projected here.** `_GlobalSearchSection` binds `GlobalSearchResult (Type, Title, Subtitle, Url, Score)`, built in `SearchService` from each section's search-hit contract. Giving those four the same treatment needs an id on `TeamSearchHit` / `CampSearchHit` (neither carries one — only `Name` and `Slug`), a rota id on `RotaSearchHit`, and a section-supplied score, since ranking currently happens here off the hit's `Name`. That is new public surface on four Contracts assemblies and waits for owner approval.
