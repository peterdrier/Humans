...
- **Presentation shared with Users/Profiles is not this section's.** `HumanSearchResultViewModel` and `SearchResultMappingExtensions.ToHumanSearchViewModel` moved down from `Humans.Web` into `Humans.UI` at the G5 move, then on into `Humans.Interfaces` once `Humans.UI` itself was deleted (nobodies-collective/Humans#866); `_HumanSearchResults.cshtml` moved separately, on to `Humans.Users` at G5 lane 4b-i — it was Base's last `Humans.Governance.Contracts` site, via `StatusBadgeExtensions`. The Humans bucket renders through the same partial as `/Profile/Search`, `/Users/Admin` and the widget gallery; partial lookup is by name across application parts, so this section renders a partial another section owns and needs **no** reference to do it — the model it binds is `Humans.Interfaces`'. Taking the partial into `Humans.Search` would have broken those three Shell pages; duplicating it would have forked the shape it exists to keep. nobodies-collective/Humans#1061 replaces the arrangement with a view component per section.
