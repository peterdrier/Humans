...
- **An unreadable payment response is permanent, not transient.** Holded accepted the response before the body is parsed, so `PayPurchaseDocumentAsync` throws `HoldedPermanentException` when the body is empty or carries no id. Reading it as transient would say "not paid, retry" about money that is already gone. The zero-payment failure message says so: it asks the admin to check Holded rather than asserting nothing was posted.
