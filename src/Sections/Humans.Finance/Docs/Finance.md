...
| `GET /Finance/Creditors/{accountNum:int}` | Per-account creditor statement (positive balance means the organisation owes the member, using Finance's derived `OwedToMember` amount; plus itemized journal lines) |
