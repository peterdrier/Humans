...
- **Booking** a transfer settles it in Holded: the bound member's open purchase documents are paid oldest first from the configured treasury account, and whatever they do not cover is one journal entry, debit the creditor account, credit the bank's ledger account. It stamps `BookedAt` / the acting admin / the Holded refs onto the transfer row. It is the only thing that moves a transfer out of `Generated`, and it cannot be done twice.
...
- **A booking that fails mid-allocation is terminal** — whether Holded refused a document payment or the journal entry. Holded has already taken the money for the postings it accepted, so their ids are persisted; `BookedAt` stays null so nothing claims the transfer settled. **`HoldedPaymentRefs` with a null `BookedAt` refuses re-booking outright** — nothing else could tell a retry from a first attempt, and a retry would post the full amount a second time. `/Finance/Sepa` shows the row as partially booked, with the ids and no button; the remainder is finished in Holded by hand, and there is no affordance to mark a transfer booked without paying through it.
...
- One `AuditAction.SepaPayoutTransferBooked` entry per booking, written after the save, carrying the amount, the **masked** IBAN, the creditor account and the Holded refs — payment ids, plus `entry:<id>` for a journal entry. **A partial booking gets one too**, labelled `PARTIAL` — postings an admin caused are never invisible to the Board.
- **An accepted posting is never thrown away over an unreadable response.** A 2xx means it posted, so `PayPurchaseDocumentAsync` returns `"unconfirmed:{documentId}"` (and `PostLedgerEntryAsync` `"unconfirmed:entry"`) instead of throwing when Holded gives back no readable id. The allocation continues and the transfer books normally; the sentinel lands in `HoldedPaymentRefs` and in the audit entry, naming the document a human must eyeball in Holded. Throwing would have lost the payment from the record entirely and left the transfer retryable.
