...
- **A booking that fails mid-allocation keeps the payments and stays unbooked.** Holded has already taken the money for the payments it accepted, so their ids are persisted; `BookedAt` stays null so nothing claims the transfer settled. A retry cannot succeed — the remaining pending no longer covers the full amount — which is deliberate: the remainder is finished in Holded by hand, and there is no affordance to mark a transfer booked without paying through it.
...
- One `AuditAction.SepaPayoutTransferBooked` entry per booking, written after the save, carrying the amount, the **masked** IBAN, the creditor account and the Holded payment ids.
