...
- **The bank line drives booking, not a click.** `BookSepaTransferAsync` takes the bank movement id and re-validates the pairing server-side — amount, account, not already reconciled, not already booked to another transfer, and exactly one unbooked transfer matches — never trusting what was posted. No unique DB index enforces "one line, one transfer": the in-memory check plus the sweep's `[DisableConcurrentExecution]` is the whole guard, so a race is caught before Holded is called rather than surfacing as a `DbUpdateException` after the money already moved ([`audit-log-as-concurrency-safety-net`](../../../../memory/architecture/audit-log-as-concurrency-safety-net.md)).
