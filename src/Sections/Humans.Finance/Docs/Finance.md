...
> - `../Humans.Finance.Contracts/HoldedPaymentInfo.cs` — read-only row DTO (date / amount / document type) surfaced on `HoldedCreditorStatus.Payments` (derived from debit lines)
