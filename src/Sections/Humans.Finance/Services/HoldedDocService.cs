...
/// <summary>
/// The purchase-document half of Finance: Holded account provisioning, the nightly doc pull,
/// attribution to budget categories, and the actuals those docs add up to.
/// </summary>
internal sealed class HoldedDocService(
    ...
    IClock clock,
    ILogger<HoldedDocService> logger) : IHoldedDocAdminService
{
    // ─── Provisioning ───────────────────────────────────────────────────────────
    ...
    private static HoldedExpenseDoc MapDoc(
        ...
        Instant now)
    {
        // The whole doc is attributed by its first line's account plus the union of doc and line
        // tags, and its full Total lands on that one category. Splitting a mixed-account doc across
        // categories is a deliberate later refinement — see the attribution invariant in Docs/Finance.md.
