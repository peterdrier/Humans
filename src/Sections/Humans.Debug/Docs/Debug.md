...
- The section references `Humans.Base` despite owning no tables: it names `QueryStatistics` (`Humans.Base.Data`) and `InMemoryLogSink` (`Humans.Base.Logging`; lives there rather than in either section because Shell writes to it from `Program.cs` and Backdoor's `BackdoorLogsController` reads it too — `/api/backdoor/logs`). `TrackingMemoryCache` is no longer named directly — `DebugController` reaches active cache-entry counts through `ICacheStatsProvider.GetActiveEntryCounts()` instead of downcasting to the concrete type.
