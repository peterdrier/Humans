...
The widget gallery and the dashboard card read other sections through their contracts: `ITeamServiceRead`, `ICampServiceRead`, `IShiftManagementServiceRead`, `IEventServiceRead`, `IUserServiceRead`, `IShiftView`, and `ISettingsService` (Shifts' read-only supplier of the active event). The gallery also references the section assemblies whose public view components it renders as `<vc:>` tag helpers (Camps, Users, Tickets, Shifts, Calendar, AuditLog, Teams, Events); that fan-in is the page's job and is opened in `Views/_ViewImports.cshtml`. A second group of widgets — Governance's, GoogleIntegration's, Notifications' — needs no reference at all: those sections contribute their own gallery sample card into `ChromeSlots.WidgetGallery` through their existing `SectionChrome` (`ISectionChrome`), and the page renders the slot with `<vc:chrome-slot name="@ChromeSlots.WidgetGallery">`, same as any other chrome slot.
...
- `/Debug/WidgetGallery` itself is a chrome-slot *consumer*: Base-hosted widgets render through `<vc:chrome-slot>`, `<vc:admin-breadcrumb>`, `<vc:section-nav>`, `<vc:things-to-do>`, and each other section's own gallery samples render through one `<vc:chrome-slot name="@ChromeSlots.WidgetGallery">` call, fed by that section's `ISectionChrome` contribution (`UsersGalleryViewComponent`, `ShiftsGalleryViewComponent`, and one-off components in Governance/GoogleIntegration/Notifications) — the page holds no per-section state for those cards and needs no reference to the sections that contribute nothing else it uses.
