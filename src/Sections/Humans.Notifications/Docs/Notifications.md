...
| Any service / job | Emit notifications via `INotificationEmitter.SendAsync(source, class, priority, title, recipientUserIds, …)` for known recipient lists, or `INotificationService.SendToRoleAsync(…, roleName, …)` for role-group dispatch |
...
- When a section's state changes in a way that should surface to users, that section calls `INotificationEmitter.SendAsync` (known recipients) or `INotificationService.SendToRoleAsync` inline in the write path (after the business save — same ordering as audit, design-rules §7a).
- When a user click-throughs a notification, the per-recipient `ReadAt` is set (if not already). When a user resolves one, the shared `Notification.ResolvedAt` + `ResolvedByUserId` are set and `ReadAt` is left alone — every unread query also filters `ResolvedAt == null`, so resolving already removes it from the unread counts. Subsequent mutations are no-ops (idempotent).
- After every successful send, resolve, dismiss, mark-read, mark-all-read, or click-through, the per-user `CacheKeys.NotificationBadgeCounts(userId)` entry is removed for each affected user (the `NotificationBellViewComponent` re-computes on next render via the cached `GetUnreadBadgeCountsAsync`). Dispatch and inbox services hold `IMemoryCache` directly for this — they do not route through `INavBadgeCacheInvalidator`.
...
- `INotificationInboxService.ResolveBySourceAsync(userId, source)` exists for sections that need to auto-resolve all open notifications of a given source for a user when the underlying condition is fixed (e.g. resolving `AccessSuspended` when consents are completed). `INotificationInboxService.ResolveBySourceKeyAsync(source, sourceKey, resolvedByUserId)` resolves all open notifications across all recipients for a specific source + entity key (e.g. auto-resolving an `IssueSubmitted` alert when the issue is closed).
- When an account merge accepts, `NotificationService` participates as an `IUserMerge` implementation: `IUserMerge.ReassignAsync` delegates to `INotificationRepository.ReassignRecipientsToUserAsync`, which re-FKs `NotificationRecipient.UserId` from source to target (collapsing duplicates where the target already has a recipient row for the same notification) and also re-FKs `Notification.ResolvedByUserId` so shared-resolution attribution points at the surviving user. Driven by `IAccountMergeService.AcceptAsync` (Users section) fanning out across registered `IUserMerge` implementations.

## Cross-Section Dependencies

- **Auth:** `IRoleAssignmentService.GetActiveUserIdsInRoleAsync` — role-scoped fan-out, injected by `NotificationService`.
...
- **Users:** `IUserServiceRead.GetUserInfosAsync` — display data for resolver + recipient rendering (stitched in memory). `IUserServiceRead.GetAllUserInfosAsync` — consent-review, onboarding-pending, and admin pending-deletions meters (all counts derived in-memory from the loaded `UserInfo` snapshot).
- **Governance:** `IApplicationServiceRead.GetUnvotedApplicationCountAsync(boardMemberUserId)` — per-board-member voting meter.
- **Tickets:** `ITicketSync.IsInErrorStateAsync` — admin ticket-sync-error meter.
...
- **Users (communication preferences):** `ICommunicationPreferenceService.GetUsersWithInboxDisabledAsync` — filters out informational notifications for users who suppressed the source's `MessageCategory`.
...
- **Page-load badge refresh, not real-time push.** With a small user base and a 2-minute cache, a WebSocket / SSE channel is overengineered. The badge re-computes when the user navigates. Every other section's nav badge (e.g. Issues' own `IssuesUserMenuViewComponent`, since Shell's `NavBadgesViewComponent` was dissolved into per-section chrome at nobodies-collective/Humans#1091) works this way too.
...
**Owning services:** `NotificationService`, `NotificationEmitter`, `NotificationInboxService`, `NotificationMeterProvider`
**Owned tables:** `notifications`, `notification_recipients`
**Status:** (A) Migrated (peterdrier/Humans PR for issue nobodies-collective/Humans#550, 2026-04-22). Moved into its own project at G5 (nobodies-collective/Humans#866).

- The section is `src/Sections/Humans.Notifications` — everything in it is `internal` except `Section`, `NotificationsResource`, and `CleanupNotificationsJob` under `Jobs/` (Shell names the job's concrete type at registration; HUM0034's carve-out for the job's own folder is what lets it be public). Its cross-section surface is the leaf project `Humans.Notifications.Contracts`: `INotificationEmitter`, `INotificationService`, `INotificationAutoResolve` (the two `ResolveBySource*` calls other sections make), `INotificationRetention` (the nightly purge `CleanupNotificationsJob` drives), and the enums those signatures name — `NotificationSource`, `NotificationClass`, `NotificationPriority`. It is a project rather than a `Contracts/` folder because its consumers are other section projects and the Shell: a folder would make every one of them reference the whole Notifications section. The inbox read models, the meter surface and the repository have no consumer outside the section and are internal. The one exception is the in-project `Contracts/INotificationInboxRead.cs`: the unread-inbox-plus-meters snapshot Backdoor serves at `/api/backdoor/notifications`. It sits in a folder, not the leaf, because Backdoor is its only consumer and already references whole section projects for the same purpose (Store, Agent); `NotificationInboxRead` (`Services/`) implements it over `INotificationInboxService` and `NotificationMeterProvider`.
- `INotificationRepository` (impl `src/Sections/Humans.Notifications/Data/NotificationRepository.cs`) is the only non-test file that touches `notifications` / `notification_recipients` via `DbContext`. The repository uses `IDbContextFactory<NotificationsDbContext>` (peeled out of `HumansDbContext` in #858) so it can be registered as **Singleton** while `NotificationsDbContext` remains Scoped.
- The notification bell is the section's own view component (`ViewComponents/NotificationBellViewComponent`, `internal`). It is contributed by Type into the `ChromeSlots.HeaderRight` chrome slot (`SectionChrome`); Shell's `_Layout` and `_AdminLayout` render the slot with `<vc:chrome-slot name="header-right" />`. `Humans.Web/Hosting/SectionViewComponentFeatureProvider` is what lets MVC discover a non-public one, the counterpart of `SectionControllerFeatureProvider` for controllers.
- **DI cycle break — `INotificationEmitter` vs `INotificationService`.** `INotificationEmitter` is the narrow outbound interface (`SendAsync` to a known recipient list). `INotificationService` extends it with `SendToRoleAsync` and injects `IRoleAssignmentService` to resolve the role's holders. Auth and Teams send notifications in the other direction, so they depend on `INotificationEmitter` — implemented by a separate `NotificationEmitter` type, **not** `NotificationService`, and injecting neither `IRoleAssignmentService` nor `ITeamService`. That is what keeps the graph acyclic. It is enforced by `ValidateOnBuild` / `ValidateScopes` at host startup (`Humans.Web/Program.cs`), not by a test — pinning it in a test would mean asserting the absence of constructor parameters, which [`no-tests-for-absences`](../../../../memory/architecture/no-tests-for-absences.md) rejects.
...
The section has no key-authed route of its own. Its machine read surface is `INotificationInboxRead`
(`Contracts/`), served by Backdoor at `GET /api/backdoor/notifications` for the key's owner; see
[`features/notification-api.md`](features/notification-api.md).
...
- `HUM0025` enforces that only `NotificationRepository` writes `DbContext.Notifications` / `DbContext.NotificationRecipients`. (The per-section `HUM0022` that used to carry this rule was retired once the universal `HUM0025` subsumed it.)
