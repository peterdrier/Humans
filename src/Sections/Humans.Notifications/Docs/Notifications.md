...
- The notification bell is the section's own view component (`ViewComponents/NotificationBellViewComponent`, `internal`). Shell's `_Layout` and `_AdminLayout` render it with `@await Component.InvokeAsync("NotificationBell")` — by name, across application parts. `Humans.Web/Hosting/SectionViewComponentFeatureProvider` is what lets MVC discover a non-public one, the counterpart of `SectionControllerFeatureProvider` for controllers.
