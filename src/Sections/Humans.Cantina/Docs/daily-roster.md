...
- [35 — Dietary & Medical Nudge](../../Humans.Users/Docs/dietary-medical-nudge.md): the data-collection counterpart. This feature is its read consumer — the nudge collects `DietaryPreference` / `Allergies` / `Intolerances` / `*OtherText`, and the roster aggregates them.
- [25 — Shift Management](../../Humans.Shifts/Docs/shift-management.md): supplies the `ShiftSignup` rows and `Shift.DayOffset` that define the per-day cohort.
