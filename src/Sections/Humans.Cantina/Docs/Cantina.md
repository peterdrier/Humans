...
- **Shifts:** `IShiftManagementServiceRead.GetOnSiteUserIdsForDayAsync` (on-site cohort). Service-layer reads only; the cantina never touches the Shifts repository.
- **Settings:** `ISettingsService.GetActiveEventSettingsAsync` (active event).
...
- **Cross-section calls** — `IShiftManagementServiceRead` (on-site cohort), `ISettingsService` (active event), `IUserServiceRead` (burner names + dietary, from the cached `UserInfo`).
