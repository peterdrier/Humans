...
- **One sync code path for preview / scheduled / manual.** Diff computation is shared; the `SyncAction` parameter (`Preview` / `Execute`) selects diff-only vs. apply, and the stored `SyncMode` is what determines which mutations execute on `Execute`.
...
Both previously-flagged cross-domain gaps are resolved:

- **Teams** — `GoogleResource.Team` was a live cross-domain nav owned by the Teams section. #992 cut it along with every other cross-section FK constraint: `GoogleResource.TeamId` is now a bare Guid column with no nav and no FK (`GoogleResourceConfiguration.cs`), guarded instead by a pre-check in `TeamService.PermanentlyDeleteTeamAsync`.
- **Users/Profiles** — `IMemoryCache.InvalidateNobodiesTeamEmails()` no longer exists and none of `GoogleController`, `ProfileController`, or `TeamAdminController` inject `IMemoryCache` for this purpose.
...
- **Cross-section calls** — `ITeamService`, `ITeamResourceService`, `IUserService`, `IUserEmailService`, `IEmailService`, `ISettingsService` (Drive monitor `DriveActivityMonitor:LastRunAt` marker in `system_settings`), `IAuditLogService` (write path — `GoogleController` logs requeue/link/remediate actions; `IAuditLogRepository` itself is `internal` to the AuditLog section and not reachable cross-section). `IProfileService` no longer exists — its surface was folded into `IUserService`.
