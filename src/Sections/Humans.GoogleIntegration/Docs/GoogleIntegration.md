...
- **Architecture tests** (`tests/Humans.GoogleIntegration.Tests/Architecture/`) — `GoogleWorkspaceSyncBridgeArchitectureTests.cs` sweeps every type in `Humans.GoogleIntegration.Services` for Google SDK types in its signatures and pins the four sync bridge interfaces' namespace and shape-neutrality; `GoogleIntegrationArchitectureTests.cs` and `GoogleWorkspaceUserArchitectureTests.cs` pin the same containment per type on `GoogleGroupSyncService` and `GoogleWorkspaceUserService` (subsumed by the sweep; retirement awaits Peter's go), and the latter also pins `IWorkspaceUserDirectoryClient`; `TeamResourceArchitectureTests.cs` pins `ITeamResourceGoogleClient`.
