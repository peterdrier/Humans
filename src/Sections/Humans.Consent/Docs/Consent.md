...
Cross-aggregate nav `ConsentRecord.DocumentVersion` — still declared (`ConsentRecord.cs:29`) and walked by `ConsentRepository.GetAllForUserIdsAsync` (`.Include(c => c.DocumentVersion).ThenInclude(v => v.LegalDocument)`, `ConsentRepository.cs:71–72`) to surface document name + version number on the user's consent-history view.
...
- **Users/Identity:** `IUserServiceRead.GetMergedSourceIdsAsync` — chain-follow merge tombstones on every per-user consent read so consents signed under a source id surface for the fold target. Consent records are immutable per §12 and stay at source.
...
- **Cross-section calls:** `ITeamServiceRead`, `INotificationEmitter`, `INotificationAutoResolve`, `IHumanLifecycleService`, `IMembershipCalculatorRead`, `IUserServiceRead`.
