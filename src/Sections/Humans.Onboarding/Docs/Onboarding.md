...
- When a human completes their profile and signs all required documents: the profile-save controller (`ProfileController.Edit` or `OnboardingWidgetController.Names`) calls `IProfileEditorService.SaveProfileAsync` then peer-calls `IOnboardingIntake.SetConsentCheckPendingIfEligibleAsync`. If the predicate holds (profile eligible for review, no existing `ConsentCheckStatus`, `IMembershipCalculatorRead.HasAllRequiredConsentsForTeamAsync(Volunteers)` returns true), it flips `Profile.ConsentCheckStatus` to `Pending` and notifies the ConsentCoordinator role. This is the audit/annotation track — admission to Volunteers happens automatically and does not depend on this flow.
