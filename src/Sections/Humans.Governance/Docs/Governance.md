...
- **Email:** Governance owns its six templates — `GovernanceEmails` (internal) builds each `EmailMessage` from Governance's own `Email_*` keys in `GovernanceResource`, rendered in the recipient's culture via `CultureScope`; `GovernanceEmailPreviews` (`IEmailPreviewContributor`, registered in `Section.Register`) lists all six at `/Email/EmailPreview`. Email supplies transport only — `IEmailService.SendAsync` (`memory/architecture/email-templates-live-in-sender.md`, peterdrier/Humans#1651).
