...
- **Account merge (`IUserMerge`):** roster rows and ballots re-FK from source to target; on a vote still open, when both accounts are on the same roster the target's row wins — the source row's ballot moves onto it when it holds none, and is dropped with the row only when both accounts voted. On a closed vote nothing is dropped: the stored result was counted from the rows as they stood at close, so both rows are kept and pointed at the surviving account. Each of the three outcomes — moved, dropped, or no ballot to begin with — is audited in those terms. The vote actor columns (`CreatedByUserId`, `OpenedByUserId`, `ClosedByUserId`, `AssemblyVotePeek.AdminUserId`) are re-FK'd as well.
