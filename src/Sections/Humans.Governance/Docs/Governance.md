...
- **Account merge (`IUserMerge`):** roster rows and ballots re-FK from source to target; when both accounts are on the same roster the target's row wins, in whatever state the vote is in — `(VoteId, UserId)` is unique, so both rows cannot survive. The source row's ballot moves onto the target when it holds none, and is dropped with the row only when both accounts voted. Each outcome — moved, dropped, or no ballot to begin with — is audited in those terms. The vote actor columns (`CreatedByUserId`, `OpenedByUserId`, `ClosedByUserId`, `AssemblyVotePeek.AdminUserId`) are re-FK'd as well.
