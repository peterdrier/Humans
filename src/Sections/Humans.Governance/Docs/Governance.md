...
- **The result** is computed once, at close, and stored in `ResultJson`; the results page renders the stored result rather than recounting. Closure persists the closed status before it reads the ballots, and `UpsertBallotAsync` re-reads the vote under its row lock, so no ballot is ever told "recorded" and then left out of the tally. Every write to a vote row — update, draft edit, open, delete — re-reads the row's state under the same lock and refuses a stale snapshot, so a transition that committed first is never undone by a form loaded before it.
