...
**Status:** (A) Migrated (peterdrier/Humans PR #503, 2026-04-15). Store/decorator layer subsequently removed under issue nobodies-collective/Humans#533. Moved into its own project `src/Sections/Humans.Governance` at G5 (nobodies-collective/Humans#866); the cross-section surface — `IApplicationServiceRead`, `IMembershipCalculatorRead`, the six-member `IApplicationDecisionService` and `ApplicationStatus` — sits on the `Humans.Governance.Contracts` leaf, everything else is `internal`.
