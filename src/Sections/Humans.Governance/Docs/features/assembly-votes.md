...
- Closed and Cancelled are terminal, enforced at the write and not just at the read: every write to a vote row takes that row's lock, re-reads its persisted state, and refuses a snapshot that has been overtaken — reopening a terminal vote, putting an open vote back to Draft, deleting or re-opening a vote that has already opened, or letting an automatic close overwrite an Extend committed since it read the deadline. Opening writes only the lifecycle fields, so a draft edit that committed while the roster was being built is what opens. The caller reports the refusal as `WrongState`. A closed vote can never reopen; to redo, create a new vote.
...
- Closed and Cancelled are terminal, enforced at the write and not just at the read: every write to a vote row takes that row's lock, re-reads its persisted state, and refuses a snapshot that has been overtaken — reopening a terminal vote, putting an open vote back to Draft, deleting or re-opening a vote that has already opened, or letting an automatic close overwrite an Extend committed since it read the deadline. Opening writes only the lifecycle fields, so a draft edit that committed while the roster was being built is what opens. The caller reports the refusal as `WrongState`.
...
- **Account merge (`IUserMerge`):** roster and ballots re-FK from source to target; if both accounts are on the same roster, the target's row wins and the source's ballot is dropped with an audit entry. The vote's own actor columns (`CreatedByUserId`, `OpenedByUserId`, `ClosedByUserId`) and `assembly_vote_peeks.AdminUserId` move too, so the acta and the published peek list keep naming the surviving human instead of a tombstone.
