...
- **Duplicate accounts:** a merge moves the merged-from account's roster rows onto the surviving account. On a vote still open, two rows for one human collapse into one — entitlements are OR-ed onto the survivor, and a sole ballot is re-parented so it is not lost, while two ballots means one is dropped, because one person holds one ballot per vote. On a vote that has closed, nothing collapses: the stored result was counted from the rows as they stood at close, so both rows are kept and simply pointed at the surviving account, and the disclosure list, the participation figures and the export keep agreeing with the acta.
