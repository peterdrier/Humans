...
- **Duplicate accounts:** a merge moves the merged-from account's roster rows onto the surviving account, and two rows for one human collapse into one in whatever state the vote is in — `(VoteId, UserId)` is unique, so keeping both is not available. Entitlements are OR-ed onto the survivor, a sole ballot is re-parented so it is not lost, and two ballots means one is dropped, because one person holds one ballot per vote. On a closed vote that leaves a narrow, accepted inconsistency: the published numbers come from the stored result and do not move, but the disclosure list and the export read live rows, so they show one ballot where the stored tally counted two, for the one human who voted twice through two accounts.
