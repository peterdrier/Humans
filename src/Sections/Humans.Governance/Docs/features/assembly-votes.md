...
- Closed and Cancelled are terminal, enforced at the write and not just at the read: every lifecycle write goes through `UpdateAsync` / `ReplaceOptionsAsync`, which take the vote row's lock, re-read its state, and refuse a snapshot that would reopen a terminal vote, put an open vote back to Draft, or let an automatic close overwrite an Extend committed since it read the deadline. The caller reports that refusal as `WrongState`. A closed vote can never reopen; to redo, create a new vote.
...
- Closed and Cancelled are terminal, enforced at the write and not just at the read: every lifecycle write goes through `UpdateAsync` / `ReplaceOptionsAsync`, which take the vote row's lock, re-read its state, and refuse a snapshot that would reopen a terminal vote, put an open vote back to Draft, or let an automatic close overwrite an Extend committed since it read the deadline. The caller reports that refusal as `WrongState`.
