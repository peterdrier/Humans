...
- **Org-level containers** (not tied to any barrio) are owned by an ordinary, production-created camp like any other; admin-only access falls out naturally because that camp has no assigned leads. (Rejected alternative: a virtual `SystemCampIds.Organization` sentinel camp — implemented and removed pre-merge 2026-05-14 because it forced special-case branches in the auth handlers.) <!-- wheat: docs/superpowers/specs/2026-04-26-containers-design.md §"Decisions during implementation" -->
...
- **Resources.** The 20 `Container_*` keys are `ContainersResource`. The 9 `ContainerMap_*` keys stay in `SharedResource` — they belong to City Planning's map page, which has not moved; section views reach them through `SharedLocalizer`, and Shell's City Planning views reach `Container_*` through an injected `IStringLocalizer<ContainersResource>`.
