<!-- freshness:triggers
  ...
  src/Sections/Humans.CityPlanning/Views/_ViewImports.cshtml
-->
...
- **Org-level containers** (not tied to any barrio) are owned by an ordinary, production-created camp like any other; admin-only access falls out naturally because that camp has no assigned leads.
...
| ImageStoragePath | string? | **Legacy**, max 512; relative path from `wwwroot/`. The pre-nobodies-collective/Humans#797 single image. Write path is gone — the columns are read-only and drain as images are removed. |
...
- **Audit Log:** `IAuditLogService.LogAsync` on every write, under the literal entity types in `Services/AuditEntityTypes.cs`.
- **Users:** `IUserServiceRead`, only through `HumansControllerBase` (current-user resolution in `ContainerController`).
...
- **DbContext** — `ContainersDbContext` (`Data/ContainersDbContext.cs`, `internal sealed`) is the section's own per-section EF model: maps only `containers`, `container_images` and `container_placements`, with its own `__EFMigrationsHistory_Containers` table and migrations under `Data/Migrations/`. Same database and connection as every other section context — the split partitions the EF model, not the database.
...
- **`Contracts/` is a folder, not a project.** Every consumer outside the section — `CityPlanningController` and `CityPlanningApiController` — sits in `Humans.CityPlanning`, which references this project directly, so no downward carve is needed. The reverse edge is the leaf only: this project references `Humans.CityPlanning.Contracts` (the authorization handler and the list page gate on the placement phase), and that leaf references `Humans.Base` alone, so the pair stays acyclic. The folder is unusually wide for the same reason: City Planning drives container CRUD from its own controllers and views, so the service interface, its DTOs, the authorization requirement/target, the form model and the card model are all cross-section surface. Narrowing it means moving the `/CityPlanning/BarrioMap/Admin/Containers` pages into this section, which is a URL change and out of scope.
- **Shared card partials live here.** `Views/Shared/_ContainerCardRow.cshtml`, `_ContainerCardModals.cshtml` and `_ContainerFormFields.cshtml` are rendered by both this section's `Container/Index` and City Planning's `CityPlanning/Containers`, over `ContainerCardModel` (the DTOs plus the host's form routing). They stay in the section rather than in Base precisely so they can localize from `ContainersResource`; a Base partial cannot see a section's resource set and would have to take every label in on its model.
- **Resources.** The `Containers_*` keys, including `Containers_Map*`, are all in `ContainersResource` — the map page is City Planning's URL over Containers' data, so the vocabulary stays in this set. `Humans.CityPlanning`'s `_ViewImports` binds an `IStringLocalizer<ContainersResource>` alongside its own, so which set a key comes from is visible at the call site. `ContainerController` injects the same localizer for its flash messages.
- **Audit discriminators are literals** (`Services/AuditEntityTypes.cs`), not `nameof`. `Camp` in particular is not nameable from this assembly.
