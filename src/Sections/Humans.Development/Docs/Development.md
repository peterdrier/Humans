...
- **The dashboard seed creates a valid event calendar.** Its build start is no later than the first-crew boundary, so the seeded fixture satisfies Settings' build-window partition invariant.
...
- **Gap #6 from the G0 audit is closed.** Shifts, Teams and Settings carried out their read-splits: `DevelopmentDashboardSeeder` now creates/deletes the seeded event through `IEventSettingsSeeding.CreateActiveEventAsync`/`DeleteEventAsync` (input records, not a mutated `EventSettings` entity), opens shift browsing via `IShiftSeeding.SetShiftBrowsingOpenAsync`, resolves the event via `ISettingsService.GetEventSettingsByIdAsync`, and resolves teams via `ITeamServiceRead.GetTeamBySlugAsync` (`TeamInfo`, not the `Team` entity). No baselined entity-returning read remains in this section.
