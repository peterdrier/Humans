...
- **Every write goes through the owning section's service.** No `DbContext`, no repository, no cross-section table access - held today by the section's csproj (no `Humans.Infrastructure` reference, no EF Core package) and by every seeder taking only application-service dependencies. A pinning `DevelopmentArchitectureTests` on the constructors is queued for a follow-up (`docs/health/runs/2026-08-24-Development.md` Needs Peter #1).
