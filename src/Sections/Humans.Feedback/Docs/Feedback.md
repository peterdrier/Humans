<!-- freshness:triggers
  ...
  src/Humans.Base/Caching/CacheKeys.cs
-->
...
| ScreenshotStoragePath | string? | `IFileStorage` key of the screenshot blob (max 512); historical uploads landed under `uploads/feedback/` |
...
- `FeedbackService` caches the feedback nav-badge actionable count inline via `IMemoryCache` (`CacheKeys.FeedbackBadgeCount`, 2-min TTL) — it is allowlisted in `ApplicationServicesTakeNoMemoryCacheRule` for exactly this short-TTL request-acceleration cache (the badge count is the only thing it caches). Cache-staleness signaling still routes through `INavBadgeCacheInvalidator`, never a raw `IMemoryCache.Remove` from outside the service.
