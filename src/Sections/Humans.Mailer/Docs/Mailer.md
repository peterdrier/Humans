...
- **Cross-section surface** — `IMailerAudienceSync` (`Contracts/`), one method returning `int`. `MailerAudienceSyncJob` stays in `Humans.Infrastructure/Jobs/` (no `ISection` seam for jobs yet, design §15 step 6b) and is the only consumer; Infrastructure references `Humans.Mailer` directly — no cycle, since Mailer never references Infrastructure. Nothing else — the dashboard stats, the import plan/apply pair and the whole `IMailerLiteService` surface are internal.
