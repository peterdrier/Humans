...
- **Cross-section surface** — `IMailerAudienceSync` (`Contracts/`), one method returning `int`. `MailerAudienceSyncJob` — its only consumer — moved into this project's `Contracts/` folder at G5 lane 5b-5, leaving only its DI registration and roll-call entry in Shell (no `ISection` seam for jobs yet, design §15 step 6b). With no Base consumer left, the `Humans.Mailer.Contracts` leaf was no longer forced and folded in beside it. Nothing else — the dashboard stats, the import plan/apply pair and the whole `IMailerLiteService` surface are internal.
