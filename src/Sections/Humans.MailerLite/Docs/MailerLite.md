...
- **Decorator decision** — no caching decorator. Rationale: admin-only, sequential, runs by hand; one DB count per dashboard load is fine at 3000 users. `MailerLiteClient` is a Singleton holding its own subscriber/group snapshot, refreshed only on demand.
