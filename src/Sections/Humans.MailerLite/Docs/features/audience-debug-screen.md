...
# MailerLite Audience Debug Screen
...
A per-audience debug screen on the existing MailerLite admin section that previews exactly what the next `Sync` would apply, so the admin can spot anomalies before pulling the trigger.
...
Bottom-right of the page; JS `confirm()` with the two counts. POSTs to the existing `/MailerLite/Admin/Audiences/{key}/Sync` action — that already does fresh-recompute + diff + apply + audit. Redirects with the existing `TempData["Banner"]` summary.
...
- [`src/Sections/Humans.MailerLite/Docs/MailerLite.md`](../MailerLite.md) — section invariants, including the new route.
