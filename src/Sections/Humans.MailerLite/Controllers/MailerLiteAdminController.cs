...
[Route("MailerLite/Admin")]
internal sealed class MailerLiteAdminController(
    IMailerLiteService ml,
    IMailerLiteImportService import,
    IMailerLiteAudienceSyncService audienceSync,
    ...
    ILogger<MailerLiteAdminController> logger) : HumansControllerBase(users)
{
        ...
        CancellationToken ct = default)
    {
        ...
        var available = _audiences
            .Select(a => new MailerLiteAudienceListItem(a.Key, a.DisplayName))
            .ToList();

        var snapshot = await MailerLiteAudienceDebugSnapshotBuilder.BuildAsync(audience, ml, _users, logger, ct);

        var options = new DebugTableOptions(PageSizes: [20, 50, 100, 200], DefaultPageSize: 20);
        var vm = new MailerLiteAudienceDebugViewModel(
