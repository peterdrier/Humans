...
- **GDPR export** (`IUserDataContributor` — `StoreGdprContributor`): `ContributeForUserAsync` always returns no rows — `IssuedByUserId`/`AddedByUserId`/`RecordedByUserId` identify the volunteer who issued an invoice, added a line, or recorded a payment, not the buyer, and no read path exists for "rows attributed to this user" today. An order's counterparty fields carry no FK to `Users` at all (an order is owned by exactly one `CampSeason` or `Team`, never a person), so they are out of reach of a per-user contributor by construction.
- **Article 17 erasure retains everything, by design.** `EraseForUserAsync` is a no-op; `ErasureDeclaration` maps `StoreRecords` to a fiscal-retention reason — an invoice, order line and payment are accounting vouchers, and Spanish law requires the books and their supporting documents be kept 6 years (Código de Comercio Art. 30) / 4 years for tax purposes (Ley 58/2003 Art. 66), GDPR Art. 17(3)(b). A factura's own counterparty fields (name, tax id, address) are themselves required invoice content. The bare `IssuedByUserId`/`AddedByUserId`/`RecordedByUserId` attribution FKs are left pointing at Users — Users' own Article 17 erasure anonymizes the account they point at.
...
- **Backdoor** (consumer): `/api/backdoor/store/*` reads the accounting export through `IStoreAccountingRead` (`Contracts/`) — order lines with effective price / VAT / revenue account / invoice number, and `Paid` payments with their Stripe payment intent ids, per year. Amounts come from the same `BalanceCalculator` pass as `/Store/Admin/Summary`, so an order's `LineGross` rows sum to the summary's total. Unlike the summary, the export selects orders by their persisted `Year` (`GetOrdersForYearWithLinesAndPaymentsAsync`, plus legacy `Year = 0` rows via the year's season ids), not through the counterparty — an order whose camp was deleted since, or whose team was reparented, still exports, labelled `(unknown camp)` / `(unknown team)`, because the money on it is still in the books (peterdrier/Humans#1719).
- **GDPR** (`Humans.Gdpr.Contracts`): Store implements `IUserDataContributor` (`StoreGdprContributor`) for the Article 15/17 account of the operator-attribution columns — see "Invariants" above.
...
**Project:** `src/Sections/Humans.Store` — the G5 pilot (nobodies-collective/Humans#866). The whole vertical is one assembly: `Domain/ Data/ Services/ Controllers/ Models/ Views/ Authorization/ Docs/ Contracts/` plus `Section.cs`. Everything is `internal` except `Section`, `StoreResource`, and the accounting read contract in `Contracts/` (`IStoreAccountingRead`, its two DTOs and the `OrderCounterpartyType` / `OrderState` enums they carry).

- `Service` (`Services/Service.cs`) depends only on Base abstractions — nothing in Store reaches another section's internals.
- `Repository` (`Data/Repository.cs`, §15b Singleton + `IDbContextFactory<StoreDbContext>`) is the only type that touches Store tables. `IStoreRepository` keeps its prefix where the rest of the internals drop theirs, because it derives from `IRepository` and cannot itself be called that (#866 design §6a). Store publishes **one cross-section read contract**, `IStoreAccountingRead` (`Contracts/IStoreAccountingRead.cs`), implemented by `Service` for Backdoor's `/api/backdoor/store` export (peterdrier/Humans#1719) — two year-filtered reads returning DTOs, never entities. An earlier `IStoreServiceRead` (added for the admin dashboard tile, nobodies-collective/Humans#1264) was retired because its sole caller was ever Store's own `SectionAdminTiles`, which resolves `Service` directly; the summary DTO graph it returned stays `internal`. Store still has no caching decorator (see below).
