...
- `/Store` — Camp Lead and department-coordinator order browse + create + line edit. Each counterparty (camp-you-lead or department-you-coordinate) is rendered as its own card; when there are none, the page renders its localized read-only catalog alert. A privileged reader (StoreAdmin/FinanceAdmin/Admin **or** TeamsAdmin) sees every camp season and department for the year, not just the ones they lead/coordinate; per-row Create/Delete affordances are resolved against `OrderAuthorizationHandler` rather than a blanket admin flag.
...
- **Shifts:** `ISettingsService.GetActiveEventSettingsAsync()` for the active event's `Year` and `TimeZoneId` — used to (a) resolve the active catalog year on `/Store` and `/Store/Admin/Catalog`, (b) populate `Year` on new team orders, and (c) compute "today in event time zone" for the `OrderableUntil` deadline gate.
...
- **Cross-section calls** route through `ICampServiceRead` (camp / camp-season lookups), `ISettingsService` (active event year + time-zone), `IAuditLogService`, `IHoldedClient`, `IStripeService`.
