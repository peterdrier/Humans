...
- The active year is derived via `ISettingsService.GetActiveEventSettingsAsync()` (see Cross-Section Dependencies in `Store.md`).
...
- Year selector at the top defaults to the active event year via `ISettingsService.GetActiveEventSettingsAsync()`, falling back to the clock's current year if there is no active event. `?year=N` overrides the default.
...
See [`Store.md`](../Store.md) for the full table schema. Summary:
...
The Store section reads several environment variables — see `Store.md` *Stripe Configuration* for the canonical contract. Summary:
...
- [`Store.md`](../Store.md) — section invariants (canonical reference)
