...
- "Create order" creates a new `Order` in `Open` state attached to the camp season; `CreateOrderAsync` rejects a second order for a season that already has one for the same year, so a camp season carries at most one. (The order `Label` was removed from the UI in #816 — the column is retained but unused.)
