...
- **Users/Identity:** `IUserServiceRead.GetUserInfoAsync` — the returned `UserInfo.MergedUserIds` chain-follows merge tombstones on every per-user audit read so source-attributed entries surface for the fold target. The read also resolves a tombstone id forward to its survivor, so a per-user audit read keyed on a merged-away id returns the survivor's history.
