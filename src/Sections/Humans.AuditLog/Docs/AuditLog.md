...
- **Workgroups:** `WorkgroupRegistered`, `WorkgroupReferred`, `WorkgroupRefused`, `WorkgroupWithdrawn`, `WorkgroupClosed`, `WorkgroupReactivated`, `WorkgroupCoordinatorsChanged`, `WorkgroupDispositionRecorded`, `WorkgroupRegisteredExisting`, `WorkgroupApplicationOverdue`, `WorkgroupUpdateDueNotified`, `WorkgroupLogEntryDeleted`, `WorkgroupLogEntryUpdated`, `WorkgroupMeetingUpdated`, `WorkgroupMeetingDeleted`, `WorkgroupCommentHidden`, `WorkgroupsRootFolderUpdated` — the register's lifecycle, written by `WorkgroupService`. Registration is administrative recognition (Board Resolution clause 4), so every step is a human action the Board must be able to see; the edit and delete actions matter because each overwrites a row that keeps no history of its own. `WorkgroupApplicationOverdue` and `WorkgroupUpdateDueNotified` are the daily pass's two notices, written under the job actor. `WorkgroupDormancyFlagged` and `WorkgroupCloseCandidateFlagged` are reserved — Peter struck the silence machinery on 2026-09-16 and nothing writes them now, but rows an earlier run wrote still read back through them; do not remove.
