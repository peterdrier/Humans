...
- **Governance (assembly votes):** `AssemblyVoteOpened`, `AssemblyVoteStopped`, `AssemblyVoteExtended`, `AssemblyVoteCancelled`, `AssemblyVoteClosed`, `AssemblyVotePeeked`, `AssemblyBallotCast`, `AssemblyBallotChanged`, `AssemblyBallotsViewed`, `AssemblyVoteRosterMerged` — lifecycle of a binding Asociado vote plus its ballot events (nobodies-collective/Humans#86). The lapse close is written under a job actor; ballot entries name the voter and never the choice; `AssemblyVotePeeked` records an Admin reading the tally before close and is published on the results page; `AssemblyVoteRosterMerged` is written when an account merge moves a roster row.
<!-- /freshness:auto -->
