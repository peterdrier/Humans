    ...
    ITeamManagementService, ITeamSeeding, IUserMerge
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var snapshot = AsReadOnlyDictionary;
        var result = new List<UserTeamMembershipInfo>(teamIds.Count);
        foreach (var teamId in teamIds)
        {
            if (!snapshot.TryGetValue(teamId, out var team))
                continue;
            ...
            result.Add(new UserTeamMembershipInfo(
                membership.TeamMemberId, team.Id, team.Name, team.Slug, team.IsSystemTeam,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var teamId in teamIds)
        {
            ...
            if (team.ParentTeamId is { } parentTeamId
                && snapshot.TryGetValue(parentTeamId, out var parent))
            {
                result[parentTeamId] = parent;
            }
        }
        return result;
    }
    ...
    // ==========================================================================
    // ITeamSeeding — the dev/demo fixture surface. Explicit implementations: the
    // Teams-internal members of the same names return this section's entities, which
    // cannot leave the assembly, so these overloads differ only in their projection
    // (design §15 step 5b).
    // ==========================================================================

    async Task<TeamInfo> ITeamSeeding.CreateTeamAsync(
        string name,
        string? description,
        bool requiresApproval,
        Guid? parentTeamId,
        ...
        CancellationToken cancellationToken)
    {
        var team = await CreateTeamAsync(
            name, description, requiresApproval, parentTeamId, googleGroupPrefix, isHidden,
            cancellationToken);
    ...
    private async Task<TResult> WithInner<TResult>(Func<ITeamManagementService, Task<TResult>> action)
    {
        await using var scope = scopeFactory.CreateAsyncScope();
        var inner = scope.ServiceProvider.GetRequiredKeyedService<ITeamManagementService>(InnerServiceKey);
