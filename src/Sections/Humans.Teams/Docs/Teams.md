...
- **Search (downstream consumer):** the global `/Search` page renders every team hit through this section's own public `<vc:teams-search-result slug>`, which fetches its display fields from `ITeamServiceRead`. Search passes the slug and no display fields (nobodies-collective/Humans#1062); `TeamSearchHit` carries the section's own `Score`. Teams does not depend on Search.
