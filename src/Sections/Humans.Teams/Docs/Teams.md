...
- `TeamResourceService` (in `Humans.GoogleIntegration.Services`) uses `IGoogleResourceRepository` + the `ITeamResourceGoogleClient` connector (PR #274). `IGoogleResourceRepository` lives in `Humans.GoogleIntegration.Data` alongside its EF impl `GoogleResourceRepository`.
...
- **Read/write interface split, three levels since the G5 move.** `ITeamServiceRead` (methods: `GetTeamsAsync`, `GetTeamAsync`, `GetTeamBySlugAsync`, `SearchAsync`, `GetUserCoordinatedTeamIdsAsync`, `GetEffectiveBudgetCoordinatorTeamIdsAsync`, `IsUserCoordinatorOfTeamAsync`, `GetUserTeamMembershipsAsync`, `GetTeamsWithParentsAsync`) is the cross-section read surface — only `TeamInfo` / `TeamSearchHit` projections, no EF entities. `ITeamService : ITeamServiceRead, IApplicationService` on the `Humans.Teams.Contracts` leaf adds the 13 members Base, Shell and the Budget / Consent / Development / Expenses sections actually call, all in flat projections. The internal `ITeamManagementService : ITeamService` in `Humans.Teams.Services` carries the remaining ~40 — the directory, detail, admin list, roster, join-request, role-definition, role-assignment, team-page and early-entry surface, plus the entity-returning `GetTeamEntityBySlugAsync` / `GetTeamByIdAsync` reads that can no longer leave the assembly. `GetAllTeamsAsync`, `GetUserTeamsAsync` and `GetByIdsWithParentsAsync` (entity-returning, once on this surface) are gone — their cross-section replacements are `GetTeamBySlugAsync` / `GetTeamAsync`, `GetUserTeamMembershipsAsync` and `GetTeamsWithParentsAsync`. See [`memory/architecture/section-read-write-split.md`](../../../../memory/architecture/section-read-write-split.md).
...
- **`SystemTeamSyncJob` is Teams' own** (G5 lane 4b-2e, nobodies-collective/Humans#866): `Services/SystemTeamSyncJob.cs`, `internal sealed`, registered in `Section.cs`. Its interface `ISystemTeamSync` joined it at G5 lane 5c and now lives on `Humans.Teams.Contracts`, which is what the six consuming sections reference. It had stayed in `Humans.Application.Interfaces.GoogleIntegration` on the belief that Hangfire pinned its assembly-qualified name; `RecurringJob.AddOrUpdate<T>(id, …)` is keyed on the id and rewrites the stored type string at every startup, so the pin was never real.
