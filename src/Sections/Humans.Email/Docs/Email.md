...
- **`Humans.Email.Contracts` — everything consumed from outside the section:**
  - `IEmailService` + `EmailMessage` — the one transport entry point, consumed from 15 other section projects (Auth, Camps, Campaigns, Consent, Events, Feedback, GoogleIntegration, Governance, Issues, Onboarding, Shifts, Surveys, Teams, Tickets, Users) plus the `Humans.Web` shell.
