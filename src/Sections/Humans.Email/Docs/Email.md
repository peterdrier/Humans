...
- **Resource set:** the 78 `Email_*` keys moved into `Humans.Email/EmailResource.{resx,es,ca,de,fr,it}` with `EmailRenderer`, their one and only renderer. They are *not* this section's page copy — the two admin views carry no localized string at all.
