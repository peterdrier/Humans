...
- **Resource set:** the `Email_*` keys that remain live in `Humans.Email/EmailResource.{resx,es,ca,de,fr,it}` beside `EmailRenderer`, their one and only renderer. They are *not* this section's page copy — the two admin views carry no localized string at all. Sections that own their templates keep their own `<Section>_Email_*` keys in their own resx set; Governance's six templates (`application_approved`, `application_rejected`, `term_renewal_reminder`, `assembly_vote_opened`, `assembly_vote_reminder`, `assembly_vote_cancelled`) moved to `GovernanceResource` with `GovernanceEmails`, Onboarding's `signup_rejected` moved to `OnboardingResource` with `OnboardingEmails`, Teams' `added_to_team` moved to `TeamsResource` with `TeamsEmails` (taking `Email_ResourcesSection`, its only reader, with it), Feedback's `feedback_response` moved to `FeedbackResource` with `FeedbackEmails`, Issues' `issue_comment` moved to `IssuesResource` with `IssuesEmails`, Campaigns' `campaign_code` moved to `CampaignsEmails` (its copy is the campaign's own, so Campaigns gains no resx set), Events' four `event_*` templates moved to `EventsEmails` (hardcoded English, unchanged by the move), Auth's `magic_link_login`/`magic_link_signup` moved to the new `AuthResource` set with `AuthEmails`, Consent's `reconsents_required`/`reconsent_reminder` moved to `ConsentResource` with `ConsentEmails`, Surveys' `survey_invitation`/`survey_reminder` moved to `SurveysResource` with `SurveysEmails`, and Shifts' `coordinator_rota_message`/`coordinator_team_rotas_message` moved to `ShiftsResource` with `ShiftsEmails` (taking the three coordinator request records with them); their renderer and factory methods are gone from here (peterdrier/Humans#1651). `Email_ReasonLine` stays: the workgroup notices still read it, and each moving section copies it.
