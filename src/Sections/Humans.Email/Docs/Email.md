...
| `POST /Email/PreviewMarkdown` | Any authenticated human | `EmailPreviewController.PreviewMarkdown` — renders the caller's own typed subject/Markdown through `IEmailPreviewServiceRead.RenderMarkdown`, for the `_EmailComposer` "Preview" button. Separate controller from `EmailController` because that one is `AdminOnly` at the class level and every compose form (not just admin ones) needs this. |
...
- `IEmailPreviewServiceRead` is the cross-section seam for side-effect-free final-body rendering, with two methods: `RenderSystemMessage(EmailMessage)` accepts only always-send system messages (opt-outable messages need recipient-specific unsubscribe policy and are rejected) for previewing a fully-built factory message; `RenderMarkdown(subject, markdownBody, category)` renders arbitrary human-typed Markdown for the shared composer's "Preview" button and accepts any category, substituting a placeholder unsubscribe footer when it is opt-outable. Both delegate to the same internal `IEmailBodyComposer`.
