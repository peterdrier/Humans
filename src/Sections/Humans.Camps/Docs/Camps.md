...
- Filesystem I/O for camp images is abstracted behind the shared `IFileStorage` abstraction (`Humans.Interfaces` interface + `FileSystemFileStorage` implementation in `Humans.Web`, rooted at `wwwroot/`); the service never touches `System.IO`.
