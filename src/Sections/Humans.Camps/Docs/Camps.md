...
- **Early Entry contributor:** the `CachingCampService` decorator implements `IEarlyEntryProvider` — emits one grant per Active `HasEarlyEntry` member (entry date = global `CampSettings.EeStartDate`, source = "Camp: {name}"). `SetEarlyEntryAsync` and the member-removal cascade evict the per-user EE cache via `IEarlyEntryInvalidator`.
