...
Implements `ICampService` (hence `ICampServiceRead`), `ICampLeadDirectory`,
`ICampSeeding`, `ICampRoleCampAccess`, `IUserMerge`, `ICampInfoInvalidator`,
`IEarlyEntryProvider`. `SearchAsync` is served from the cached `CampInfo`
