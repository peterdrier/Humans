...
- **Magic Link Auth** is the email-based login/signup flow handled by `MagicLinkService` + `AccountController`. Users enter an email address; the system sends a Data-Protection-token-protected link. Login links verify a user id; signup links carry the email address. Both token types are single-use, consumed into the same replay-protected 15-minute cache: **login** tokens on `VerifyLoginTokenAsync`, **signup** tokens on `VerifyAndConsumeSignupTokenAsync` at the POST that redeems them. The signup GET reads the token without consuming it (form prefill, so a scanner following the link cannot burn it). This is the guarantee the shipped signup email makes in all six cultures. Signup sends are rate-limited to 1 per 60 seconds per address; login-link sends are rate-limited to 1 per 60 seconds per user via `User.MagicLinkSentAt`. Token generation/validation/URL building sits behind `IMagicLinkUrlBuilder` and rate-limit/replay state behind `IMagicLinkRateLimiter` so `MagicLinkService` itself depends on no `HumansDbContext`, `IDataProtectionProvider`, or `IMemoryCache`.
- **Access gating** — `MembershipRequiredFilter` (a global `IAsyncActionFilter`) routes authenticated users by their stored `UserState`: only `Active` reaches the app; `Bare` → `/OnboardingWidget`, `DeletePending` → `/User/Deletion`, and `Suspended`/`AdminSuspended`/`Rejected` → `/User/Status`. `Deleted`/`Merged` accounts are walled off before the exemptions below apply — only `Account`, `Language`, and `User/Status` reach them. It passes through endpoints carrying `[AllowAnonymous]` and the exempt controllers a non-Active recoverable user must still reach wholesale (`Account`, `OnboardingWidget`, `Consent`, `User`, `Language`, `Guest`, `GovernanceApplications`, `Issues`, `Notifications`, `Survey`), plus specific `Profile`/`ProfileEmails` actions (own-profile editing, privacy/download-data, email management) — not those controllers wholesale. Role-gated app controllers require `UserState == Active` before their role policies matter. It also passes a Backdoor-API-key-authenticated request straight through — a machine principal never carries the `UserState` claim this gate reads, and there is no onboarding page to redirect a JSON client to (see `src/Sections/Humans.Backdoor/Docs/Backdoor.md`).
...
| GET | `MagicLinkSignup` | anonymous | Reads the signup token (without consuming it) and renders the complete-signup form |
| POST | `CompleteSignup` | anonymous | Redeems the signup token — consumed once, after field validation — then creates the `User` + `UserEmail` and signs the new user in. A duplicate submit from the now-signed-in user redirects onward; any other replay gets `MagicLinkError` |
...
|----------|------|---------|
| Id | Guid | PK |
| UserId | Guid | The assignee's user id. No FK constraint and no nav property — `users` is Users' table in another context; never `.Include()`d |
| CreatedByUserId | Guid | Who created the row (required). Same shape: bare id, no FK, no nav property |
...
The auth surface is mid-transition. Phase by phase:
...
- Magic-link tokens are single-use: once a token is redeemed — a login token on `VerifyLoginTokenAsync`, a signup token on `VerifyAndConsumeSignupTokenAsync` — it is reserved in the replay cache for the remainder of its 15-minute lifetime and cannot be used again. The two token strings come from different DataProtection purposes, so they never collide in that cache. Survey and unsubscribe links use their own token providers and do not redeem through here — they stay reusable by design.
...
- Verified-email resolution goes through `IUserEmailService.FindByAddressAsync` (aliased, verified only) — never through raw `DbContext.UserEmails` queries. Unverified rows are ignored (their auto-link would bypass the merge-request review gate).
...
- **On a magic-link token redeemed:** the token is reserved in the replay cache for the remainder of its 15-minute lifetime via `IMagicLinkRateLimiter.TryConsumeTokenAsync` so it cannot be redeemed twice. `AccountController.MagicLink` stamps `User.LastLoginAt` after the verified sign-in. On the signup path the reservation is taken in `CompleteSignup` *after* field validation, so a re-rendered form does not burn the link; a replay that follows finds the reservation held and gets `MagicLinkError`, unless the caller is the user the first POST created and signed in, who is redirected onward. If provisioning fails or throws, the reservation is released (`ReleaseSignupToken`) so the link survives to be retried. The reservation itself is exclusive under concurrent requests: `IMemoryCache` has no atomic add, so `MemoryCacheExtensions.TryReserveAsync` takes the read and the write under one lock.
- **On account merge:** `AccountMergeService.MergeAsync` (Users section) calls every registered `IUserMerge`, including `RoleAssignmentService.ReassignAsync(archivedId, survivorId, …)`, which re-FKs `role_assignments` rows from the archived user to the survivor. AspNetUserLogins re-FK is Users' own merge step (`UserRepository.ReassignLoginsToUserAsync`).
...
- **Users (account merge):** `RoleAssignmentService` implements `IUserMerge`; Users' `AccountMergeService` calls `ReassignAsync` to re-FK `role_assignments` from the archived user to the survivor during the merge fold.
...
- **Caching decorator.** `CachingRoleAssignmentService` (Singleton, `TrackedCache<Guid, RoleAssignmentRow>`) wraps the keyed Scoped inner `IRoleAssignmentService` and resolves it per call through `IServiceScopeFactory` — the decorator wraps the service interface, never the repository. It holds every `role_assignments` row, so `GetActiveCountsByRoleAsync` and `GetActiveForUserAsync` derive "active at now" in memory at any clock instant; invalidation is wholesale, driven from the inner service's writes via `IRoleAssignmentCacheInvalidator.InvalidateAll()`. Warm-up runs from a hosted service at startup. See `Docs/health.md` §3 for the warm path's known cost.
- **Section boundary shape** — no `IStringLocalizer`, no `DbContext`/store in `Services/`, framework-free `Contracts` leaf. `MagicLinkService` takes no `EmailSettings`/`IDataProtectionProvider` parameter and no repository parameter (URL construction sits behind `IMagicLinkUrlBuilder`; it is nearly an orchestrator per peters-hard-rules.md — see `Docs/health.md` §3 for what disqualifies it today). These are documentation, not pinned tests — a test asserting a type lacks a constructor parameter is forbidden by [`no-tests-for-absences`](../../../../memory/architecture/no-tests-for-absences.md).
- **Cross-domain navs removed, not just `[Obsolete]`-marked:** `RoleAssignment.User`, `RoleAssignment.CreatedByUser` no longer exist as nav properties. The repository does not `.Include()` them; the service stitches display data in memory from `IUserServiceRead.GetUserInfosAsync` (§6b).
- `MagicLinkService` owns no tables. Its persistent state is `User.MagicLinkSentAt`, mutated via `UserManager<User>`. Verified-email lookup goes through `IUserEmailService.FindByAddressAsync`. Data-Protection token generation/validation and URL construction sit behind `IMagicLinkUrlBuilder`; replay-protection and signup rate-limit state sit behind `IMagicLinkRateLimiter`. That arrangement keeps `MagicLinkService` free of `HumansDbContext`, `EmailSettings`, `IDataProtectionProvider`, and `IMemoryCache`.
...
- Do **not** reintroduce `.Include(ra => ra.User | ra.CreatedByUser)` anywhere. New read paths go through `IRoleAssignmentRepository` (or extend the repository with a new narrowly-shaped query) and stitch display data in `RoleAssignmentService` via `IUserServiceRead`.
