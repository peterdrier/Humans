...
`AuthArchitectureTests.SectionReferencesNoVerticalSection` pins the boundary as an exact list — `Humans.AuditLog.Contracts`, `Humans.Gdpr.Contracts`, `Humans.Notifications.Contracts` (horizontal leaves), plus `Humans.Users.Contracts` (display stitching, `UserManager<User>`, `IUserEmailService`) and `Humans.Email.Contracts` (the magic-link send). A reference to a vertical *section project*, or to a third vertical leaf, still fails it.
