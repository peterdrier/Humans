...
- **On a magic-link token redeemed:** the token is reserved in the replay cache for the remainder of its 15-minute lifetime via `IMagicLinkRateLimiter.TryConsumeTokenAsync` so it cannot be redeemed twice. `AccountController.MagicLink` stamps `User.LastLoginAt` after the verified sign-in. On the signup path the reservation is taken in `CompleteSignup` *after* field validation, so a re-rendered form does not burn the link; a replay that follows finds the reservation held and gets `MagicLinkError`, unless the caller is the user the first POST created and signed in, who is redirected onward. If provisioning fails, the reservation is released (`ReleaseSignupToken`) — the failure paths roll themselves back, so the link must survive to be retried.
