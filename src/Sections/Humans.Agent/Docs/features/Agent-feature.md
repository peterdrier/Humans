...
- Issues system (`src/Sections/Humans.Issues/Docs/features/issues-system.md`) — handoff target via client-side modal pre-fill.
- Legal documents (`src/Sections/Humans.Consent/Docs/features/legal-documents-consent.md`) — `agent-chat` slug renders the AI Terms at `/Legal/agent-chat`; linked from the Assistant panel composer footer.
