...
- `AgentSettings.PreloadConfig` defaults to `Tier1` (8 highest-signal sections in the index). If non-admin users start asking about sections outside that set and the model can't help, an admin can flip the live setting to `Tier2` at `/Settings#agent` (peterdrier/Humans#1634; `/Agent/Admin/Settings` redirects there) — both tiers fit Anthropic ITPM caps because section bodies route through tool calls, not preload.
