...
| Admin | View operational status at `/Agent/Admin/Status` (usage / spend / refusals / top docs / top users / retention job / Anthropic balance), configure settings at `/Settings#agent` (peterdrier/Humans#1634), view all conversations at `/Agent/Conversations` (Human column + filters, Older/Newer paging), drill into the diagnostic view at `/Agent/Conversations/{id}` (token counts, tool-call args, prompt preview), disable globally |
...
**Owning services:** `AgentService` (orchestrator), `AgentSettingsService`, `AgentToolDispatcher`, `AgentUserSnapshotProvider`, `AgentAbuseDetector`, `AgentPromptAssembler`, `AgentPreloadCorpusBuilder`, `AgentPreloadAugmentor`, `AnthropicClient`. `AgentPreloadAugmentor` collects each section's `ISectionHelp` glossary contribution (only `SectionHelpContent.Faq` still lives in `Humans.Base`) and implements `IAgentPreloadAugmentor` from this project's `Contracts/` folder; `AgentConversationRetentionJob` lives under `Jobs/` and calls `IAgentConversationRetention`, its registration and schedule contributed via `SectionJobs.cs`.
...
- `AgentSettings.PreloadConfig` defaults to `Tier1` (8 highest-signal sections in the index). If non-admin users start asking about sections outside that set and the model can't help, an admin can flip the live setting to `Tier2` at `/Settings#agent` (peterdrier/Humans#1634; `/Agent/Admin/Settings` GET is removed) — both tiers fit Anthropic ITPM caps because section bodies route through tool calls, not preload.
