...
- [`src/Sections/Humans.Shifts/Docs/shift-management.md`](shift-management.md) — base rotas / shifts / signups model; the gap algorithm reads `ShiftSignup` rows it produces.
- [`src/Sections/Humans.Shifts/Docs/shift-signup-visibility.md`](shift-signup-visibility.md) — site-wide signup-visibility policy; the tracking page does not reuse that policy (its access is the new `VolunteerTrackingWrite` gate, not the public-signup-list gate).
- [`src/Sections/Humans.Tickets/Docs/event-participation.md`](../../Humans.Tickets/Docs/event-participation.md) — `EventParticipation.Status = Attending` is the pre-filter for the declared-but-unbooked cohort.
