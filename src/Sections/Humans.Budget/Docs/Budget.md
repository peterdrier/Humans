...
- Everything but `Section`, `BudgetResource` and the migrations is `internal` — HUM0034 enforces it. The cross-section surface is the leaf project `Humans.Budget.Contracts`: `IBudgetServiceRead` (five methods — `GetYearByIdAsync` moved up from `IBudgetService` so Expenses can offer the categories of the year a pending report is already booked to), `IBudgetDemoSeeder` (one), the DTOs those name, and the `BudgetYearStatus` / `ExpenditureType` enums. `ITicketingBudgetService` moved to `Humans.Budget.Services` and went internal (Peter's ruling 43) — its only consumer, `TicketingBudgetSyncJob`, is section-internal. The leaf is a project rather than a `Contracts/` folder because one consumer is still in Base — `TicketQueryService`. `TicketingBudgetSyncJob` was the second until G5 lane 5b-3 moved it into this project's `Jobs/` folder.
