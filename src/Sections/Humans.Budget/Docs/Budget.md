...
- Everything but `Section`, `BudgetResource` and the migrations is `internal` — HUM0034 enforces it. The cross-section surface is the leaf project `Humans.Budget.Contracts`: `IBudgetServiceRead` (four methods), `ITicketingBudgetService` (one), `IBudgetDemoSeeder` (one), the DTOs those name, and the `BudgetYearStatus` / `ExpenditureType` enums. The leaf is a project rather than a `Contracts/` folder because one consumer is still in Base — `TicketQueryService`. `TicketingBudgetSyncJob` was the second until G5 lane 5b-3 moved it into this project's `Contracts/` folder.
