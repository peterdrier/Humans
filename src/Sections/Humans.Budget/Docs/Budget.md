...
- `BudgetService.ContributeForUserAsync` (GDPR contributor) chain-follows merge tombstones via `UserInfo.MergedUserIds` so `BudgetAuditLog` entries written under a now-merged source `ActorUserId` surface for the fold target. `BudgetAuditLog` is append-only (§12) and stays attributed to the source User row by design.
...
- **Users/Identity:** `IUserServiceRead.GetUserInfosAsync` — actor display names for audit log. `IUserServiceRead.GetUserInfoAsync` — the returned `UserInfo.MergedUserIds` chain-follows merge tombstones on the `BudgetAuditLog` GDPR export so source-attributed entries surface for the fold target.
