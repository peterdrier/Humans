...
**Cross-section FKs:** `ResponsibleTeamId` → Teams domain — **FK only, no nav property**. `BudgetLineItem.ResponsibleTeam` no longer exists on the entity; the Finance CategoryDetail view reads a stitched-in `ResponsibleTeamName` instead.
...
**Cross-section FKs:** `ActorUserId` → Users/Identity — **FK only**, no navigation property (the `[Obsolete]`-marked `BudgetAuditLog.ActorUser` nav was deleted in nobodies-collective/Humans#1188; callers resolve display names via `IUserService`).
...
- **Tickets:** none inbound. The Tickets→Budget bridge (`TicketingBudgetService`) is Budget's own service — it reads paid orders via `ITicketServiceRead` and calls the section's internal `IBudgetService` (`SyncTicketingActualsAsync` / `RefreshTicketingProjectionsAsync` / `UpdateTicketingProjectionAsync` / `GetTicketingProjectionEntriesAsync`). Budget has no code path that reads Tickets tables directly. Only `SyncActualsAsync` is public, on the contracts leaf, for the nightly `TicketingBudgetSyncJob` (`Humans.Budget/Contracts/` since G5 lane 5b-3, so that member is now section-internal in fact). (The dedicated `ITicketingBudgetRepository` added for PR #545b was removed in #815.)
...
**Status:** (A) Migrated (peterdrier/Humans PR for issue nobodies-collective/Humans#544, 2026-04-22). Moved into its own project `src/Sections/Humans.Budget` at G5 (nobodies-collective/Humans#866).

- Everything but `Section`, `BudgetResource` and the migrations is `internal` — HUM0034 enforces it. The cross-section surface is the leaf project `Humans.Budget.Contracts`: `IBudgetServiceRead` (four methods), `IBudgetDemoSeeder` (one), the DTOs those name, and the `BudgetYearStatus` / `ExpenditureType` enums. `ITicketingBudgetService` moved to `Humans.Budget.Services` and went internal (Peter's ruling 43) — its only consumer, `TicketingBudgetSyncJob`, is section-internal. The leaf is a project rather than a `Contracts/` folder because one consumer is still in Base — `TicketQueryService`. `TicketingBudgetSyncJob` was the second until G5 lane 5b-3 moved it into this project's `Contracts/` folder.
