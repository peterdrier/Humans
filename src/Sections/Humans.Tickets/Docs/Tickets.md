...
- `EventParticipation` derivation (Ticket Tailor's active vendor event only, scoped to `EventSettingsInfo.Year` from `ISettingsService.GetActiveEventSettingsAsync`):
...
- **Settings:** `ISettingsService.GetActiveEventSettingsAsync` — the narrow read-only supplier of the active year (`EventSettingsInfo`) for `EventParticipation` derivation, the backfill page, the who-hasn't-bought report and the onsite roster; do not reach for `IShiftManagementService`.
...
- No Tickets service holds an `IMemoryCache` directly; the per-event `TicketEventSummary:{eventId}` entry and both tracked ticket slices live in `TrackedCache` instances on the two caching decorators. The inner `TicketQueryService` and write-side `TicketSyncService` are cache-free; sync invalidates the per-event summary via `ITicketVendorCacheInvalidator.InvalidateEventSummary` and clears tracked ticket slices via `ITicketCacheInvalidator.InvalidateAll`. Other Tickets-section services (e.g. `TicketTransferService`) that need to invalidate after a write call `ITicketCacheInvalidator.InvalidateAfterTransfer(senderUserId, receiverUserId)` instead of touching a cache directly. Do not push `IMemoryCache` into controllers, view components, or other domain services. New invalidation seams go on `ITicketCacheInvalidator` (not `ITicketServiceRead`) so the budgeted query surface doesn't grow each time a new write site is added.
