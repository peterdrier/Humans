...
**Aggregate-local navs:** `CampaignGrant.Campaign`, `CampaignGrant.Code`. No cross-domain navs — Email outbox rows reference the grant by bare FK only, resolved through the Email section's services.
...
- Campaign template substitution (`{{Code}}`, `{{Name}}`) HTML-encodes both values before insertion into `Campaign.EmailBodyTemplate` — recipient display names are user-controlled and would otherwise be an HTML-injection vector. `EmailSubject` substitutes unencoded: subjects are plain text, never rendered as HTML (`CampaignsEmails.CampaignCode`). Placeholder matching uses `StringComparison.Ordinal`. The full substitution vocabulary is `{{Code}}` and `{{Name}}`; new body placeholders must keep the encoding guard. The body is edited through the shared `_EmailComposer` (Base) and rendered through `SanitizedMarkdownRenderer` — `<img>` sources are restricted to `https://`.
