...
- **Cross-section surface** — `Humans.Issues.Contracts` carries `IIssuesRetention.PurgeExpiredAsync` (driven by `CleanupIssuesJob`, which moved from `Humans.Infrastructure/Jobs` into this section's `Jobs/` at G5 lane 5b-1, then out of `Contracts/` entirely in the HUM0034 Jobs carve-out), `IssueCategory` (named by `Humans.Agent`'s `route_to_issue` proposal), `IssueStatus`, and `IIssueTriage` with its `IssueReadModels` (the Backdoor machine API, nobodies-collective/Humans#1128). `IIssuesServiceRead.GetActionableCountForViewerAsync` was removed (nobodies-collective/Humans#1091): its sole caller, Shell's `NavBadgesViewComponent`, dissolved into this section's own `IssuesUserMenuViewComponent` chrome contribution, so the method moved onto the internal-only `IIssuesService`. Everything else is internal to the section.
