...
- **A queue exists because a section declared it, so the table cannot drift from the section list.** `IIssueQueueOwner` derives from `ISectionAnnotations` and publishes its own "Issue queue" annotation, so `/Debug/Sections` still shows where a reported issue lands — contributed by the owning section, not by Issues. `Profiles` (merged into Users) and `Legal` (renamed Consent) are no longer declared by anybody: rows still carrying those strings keep resolving through the same fall-through an unknown value takes, to the Admin-only queue, with no migration. Validation still keys off `IssueSectionRouting`, not `ISectionCatalog`: the catalog is the set of sections that exist, the routing lookup is the set of queues they declared.
