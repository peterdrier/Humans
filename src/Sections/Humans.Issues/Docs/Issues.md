...
- **Section** is a string drawn from the queues sections declare through `IIssueQueueOwner` (Budget, Camps, CityPlanning, Governance, Onboarding, Scanner, Shifts, Teams, Tickets) or `null`, enforced on both write paths. Stored as a string so the set of queues can change without migrations. Anything outside the set is coerced to `null`; null-section issues fall to the Admin queue only.
