...
- **Section** is a string drawn from `IssueSectionRouting.AllKnownSections` (Tickets, Camps, Teams, Shifts, Onboarding, Profiles, Budget, Governance, Legal, CityPlanning, Scanner) or `null`, enforced on both write paths. Stored as a string so the routing table can change without migrations. Anything outside the set is coerced to `null`; null-section issues fall to the Admin queue only.
...
- **The routing table is checked against the published section list, not driven by it.** `SectionAnnotations` (`ISectionAnnotations`) publishes one "Issue queue" annotation per `AllKnownSections` entry; anything naming no discovered section surfaces on `/Debug/Sections` and in a startup warning. `Profiles` (merged into Users) and `Legal` (renamed Consent) are drifted today, and stay routable until the stored rows carrying those strings are migrated. Validation therefore keys off `IssueSectionRouting`, not `ISectionCatalog`: the catalog is the set of sections that exist, the routing table is the set of queues that exist, and these are not the same set.
