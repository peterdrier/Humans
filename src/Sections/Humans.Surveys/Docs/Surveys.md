...
**Status:** (A) Migrated — born §15-compliant (peterdrier/Humans issue nobodies-collective/Humans Survey section, 2026-06-04). Moved into its own project `src/Sections/Humans.Surveys` at G5 (nobodies-collective/Humans#866); everything but `Section`, `SurveysResource`, `Contracts/` and `Jobs/` is `internal` (HUM0034). `Contracts/` is entirely public: the enums, the `SurveyDefinitionSnapshot`/`SurveyReadModels` DTOs, `SurveyResponsesMarkdownBuilder`, `ISurveyReminderSender.SendDueRemindersAsync` (driven by `Jobs/SendSurveyReminderJob.cs`, public because Shell names the concrete type when it registers and schedules it), and `ISurveyAnalysisRead` (the Backdoor API's read surface, nobodies-collective/Humans#1128 — see Cross-section read interface below). The reminder job came home from `Humans.Infrastructure/Jobs` to this project at G5 lane 5b-5, briefly into `Contracts/`, then out to its own `Jobs/` folder at nobodies-collective/Humans#1353 (HUM0034 Jobs carve-out).
...
`ISurveyAnalysisRead` (public, in `Contracts/`): survey list, one survey's resolved question graph (`GetDefinitionAsync` → `SurveyDefinitionSnapshot`), the raw per-response export, and the per-question aggregates. Read-only — a survey is authored only in the admin UI, never over the API. Its sole consumer is `Humans.Backdoor`'s `BackdoorSurveysController` behind `/api/backdoor/surveys` (nobodies-collective/Humans#1128). The reminder job in Base is the other outside consumer, seeing only `ISurveyReminderSender`. `ISurveyServiceRead` shipped empty in v1 and no other section consumed it, so it was deleted at G5; `ISurveyAnalysisRead` is unrelated and newer.
