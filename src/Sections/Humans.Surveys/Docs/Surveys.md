...
- **The invitation email's custom message has a builder-time "Preview" button** (distinct from the send-a-preview-to-self flow below) that renders the typed Markdown through the shared `_EmailComposer`/`IEmailPreviewServiceRead.RenderMarkdown` seam (Email section) — no invitation is sent and no outbox row is created.
