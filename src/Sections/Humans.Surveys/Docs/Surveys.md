...
- **An Identified draft follows its final submitting context and privacy choice.** A wizard start returns its draft id and answer snapshot from the same operation. Resume/autosave updates the persisted `InputMethod` and selected culture, and final submit stamps both again with the answers and `SubmittedAt`. Identified and CompletionTracked finalisation atomically claim the shared participation row in one transaction, so concurrent tabs cannot persist conflicting representations; a winning CompletionTracked claim deletes the personal draft in that same transaction.
