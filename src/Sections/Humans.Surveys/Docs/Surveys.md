...
- **An Identified draft follows its final submitting context and privacy choice.** Resume/autosave updates its persisted `InputMethod` and selected culture, and final submit stamps both again atomically with the answers and `SubmittedAt`, so another open tab cannot misclassify the response's funnel or export language. Choosing CompletionTracked preserves any Identified draft while either wizard remains active, then deletes it only after the unlinkable response completes; a stale Identified session cannot silently complete after its draft is gone.
