...
- **An Identified draft follows its current entry context and privacy choice.** Resuming the same draft updates its persisted `InputMethod` and selected culture, so the eventual response lands in the correct funnel and export language. Switching from Identified to CompletionTracked deletes the personal draft and answers before the unlinkable response begins.
