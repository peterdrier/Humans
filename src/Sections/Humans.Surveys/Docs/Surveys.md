...
- **Shareable slugs support both anonymous and identified surveys.** When `AllowAnonymous` is true, logged-out visitors are Anonymous and logged-in Humans may choose Identified, CompletionTracked, or Anonymous. When it is false, the visitor must sign in, must currently belong to the configured audience (or may be any logged-in Human when no audience is configured), and is forced to Identified. Slug access is rechecked on entry and every wizard GET/POST. All representations use `InputMethod=Slug`. Reserved slugs `admin`/`answer` are rejected by the builder and 404 on the answer path.
