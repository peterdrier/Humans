...
- **Invitation send is idempotent and additive.** Each send resolves the audience, diffs against existing `(SurveyId, UserId)` invitations, then atomically gets-or-creates each net-new participation row before preparing its email; a concurrent public start therefore reuses the same row rather than aborting the wave. Nobody is double-invited and **sends never revoke**. The Send page previews that exact net-new count, not the raw audience size. Requires the survey Open with a valid audience configuration (`Team` requires a team; `LoggedInSince` requires a cutoff date). Invites are operational (`MessageCategory.System`, always-send) — surveys are never marketing.
...
- **An Identified draft follows its final submitting context and privacy choice.** Draft lookup/creation runs while locking the shared participation row and returns its id plus answer snapshot from that same operation, so concurrent starts share one draft. Autosave and final submit stamp the active session's `InputMethod` and selected culture with its answers. Autosave locks the participation row without claiming it; Identified and CompletionTracked finalisation claim that row in their transaction, so a stale autosave cannot write after submission and concurrent tabs cannot persist conflicting representations. A winning CompletionTracked claim deletes the personal draft in that same transaction.
- **Public participation rows are not emailed invitations until email is actually prepared.** A logged-in tracked public start may create a `survey_invitations` ledger row with `SentAt = null`. CompletionTracked uses the shared Unix-epoch `CreatedAt` sentinel (and scrubs an existing unsent row to it), never the respondent's precise start time, so it cannot be correlated with response `SubmittedAt`; Identified may retain its real creation time. Unsent rows are excluded from invited counts/status/reminders. If still incomplete, a later send atomically upgrades that same row by stamping `SentAt` and queue status; if another wave already upgraded it or the Human completed through the public link, the conditional update loses and no duplicate/spent token is emailed.
...
**Status:** (A) Migrated — born §15-compliant (peterdrier/Humans issue nobodies-collective/Humans Survey section, 2026-06-04). Moved into its own project `src/Sections/Humans.Surveys` at G5 (nobodies-collective/Humans#866); everything but `Section`, `SurveysResource`, `Contracts/` and `Jobs/` is `internal` (HUM0034), and the only public `Contracts/` member is `ISurveyReminderSender.SendDueRemindersAsync`, which `Jobs/SendSurveyReminderJob.cs` drives — public because Shell names the concrete type when it registers and schedules it. The job came home from `Humans.Infrastructure/Jobs` to this project at G5 lane 5b-5, briefly into `Contracts/`, then out to its own `Jobs/` folder at nobodies-collective/Humans#1353 (HUM0034 Jobs carve-out); `ISurveyReminderSender` has no consumer outside the section, and the `Humans.Surveys.Contracts` leaf it occupied until 5b-5 folded in at the same time it lost its only outside consumer (ruling 44).
