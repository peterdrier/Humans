...
**Status:** (A) Migrated — born §15-compliant (peterdrier/Humans issue nobodies-collective/Humans Survey section, 2026-06-04). Moved into its own project `src/Sections/Humans.Surveys` at G5 (nobodies-collective/Humans#866); everything but `Section` and `SurveysResource` is `internal` (HUM0034), and the only cross-section surface is a single member in the project's `Contracts/` folder — `ISurveyReminderSender.SendDueRemindersAsync`, which `SendSurveyReminderJob` drives beside it. The job came home from `Humans.Infrastructure/Jobs` to this project's `Contracts/` folder at G5 lane 5b-5, leaving that member with no consumer outside the section; the `Humans.Surveys.Contracts` leaf it occupied until then folded in at the same time it lost its only outside consumer (ruling 44).
