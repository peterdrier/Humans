...
- **An Identified draft follows its current entry path and privacy choice.** Resuming the same draft through a token or slug updates its persisted `InputMethod`, so the eventual response lands in the correct funnel. Switching from Identified to CompletionTracked deletes the personal draft and answers before the unlinkable response begins.
