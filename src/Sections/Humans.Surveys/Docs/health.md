...
| 2026-09-20 | third run, after the §11 self-service authoring lane shipped — target gains the Approve shape and an enforcement cite on every invariant; docs, comments and two debt rows caught up to open authoring; reject modals left the table; `SurveyDetail` can no longer be built without an author; a dead resx key deleted in six cultures. Author-side navigation and preview stay Board-gated — raised for Peter, not struck | peterdrier/Humans#PENDING |
