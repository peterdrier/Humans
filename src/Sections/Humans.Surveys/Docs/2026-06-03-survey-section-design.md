...
Public responses via `PublicSlug` are **Anonymous** when logged out. Logged-in Humans explicitly choose Identified, CompletionTracked, or Anonymous; the first two use an unsent participation-ledger row rather than an emailed invitation.
...
4. **Shareable slug** → **slug answering path is IN v1** (decided 2026-06-04; representation choice amended 2026-08-23; identified access amended 2026-09-02; Asociado privacy amended 2026-09-03). A survey with a `PublicSlug` can be answered at `/Survey/{slug}` without sending an email. Anonymous-enabled surveys retain the public representation choice: logged-out responses are Anonymous, while logged-in Humans choose Identified, CompletionTracked, or Anonymous. Surveys without respondent-selected anonymity require sign-in and current audience access (or any logged-in Human when no audience is configured). Ordinary surveys force Identified; Asociado votes verify current eligibility and force CompletionTracked, so completion is recorded but the submitted ballot has no voter link. Access is rechecked throughout the wizard. All use `InputMethod = Slug`. Public starts are counted by `Survey.PublicStartedCount`; tracked logged-in starts additionally use the per-survey/user participation ledger. (§4, §8)
