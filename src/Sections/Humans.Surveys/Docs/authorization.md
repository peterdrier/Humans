...
| `SurveyAdminController` | Class | `Board, Admin` | `PolicyNames.BoardOrAdmin` (survey authoring at `/Survey/Admin` — `Index`, `Official`, `Create`, `Edit`, `Preview`, `PreviewPage`, `PreviewThankYou`, `SendPreviewEmail`, `PreviewEmail`, `Save`, `Open`, `Close`, `Send` GET/POST, `Results`, `RankedAvailability`, `ExportCsv`, `ExportJson` all inherit) |
