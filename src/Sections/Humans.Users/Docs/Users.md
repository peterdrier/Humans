...
- **Gdpr:** `IGdprService.EraseForUserAsync` — the erasure fan-out itself lives in Gdpr (since this PR); `AccountDeletionService` resolves the merge chain, calls it per id, and invalidates each archived id's cache as its erasure completes. It names no section to erase; Gdpr fans out over its own `IUserDataContributor` roster.
