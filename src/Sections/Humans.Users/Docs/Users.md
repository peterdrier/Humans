...
- **The merge index is derived state, rebuilt on demand.** `CachingUserService` builds a `Dictionary<Guid, Guid[]>` over its warmed snapshot the first time a read needs it, and drops it on every mutation of the dict (`TrackedCache.OnMutated`). It is never persisted and never invalidated by hand; a merge, an erasure or a cache refresh all mark it dirty through the same hook.
...
- **Audit Log / Consent / Budget / Expenses:** read `UserInfo.MergedUserIds` off the row `GetUserInfoAsync` / `GetUserInfosAsync` already returned, to chain-follow merge tombstones on per-user reads of append-only entities.
