...
- The shared own/other profile view renders `<vc:user-parts slot="user-profile" user-id view-mode>` (Base's `UserPartsViewComponent`), which collects every active section's `IUserPart` descriptors for that slot (`UserPartSlots`), orders them by weight, and invokes each component `Type` with a `UserPartArgs(userId, viewMode)` — the profile target and the same audience the profile card gets; a contributor that throws is logged and skipped. Contributing components own their data and any visibility beyond the ordinary authenticated-profile gate; an empty component renders nothing. Users references no contributor: Events is the reference contributor and reaches the page only through this seam.
