...
User-suspension state lives on `User.State` (`UserState.Suspended` / `AdminSuspended`); `UserInfo.IsSuspended` is the canonical predicate. Single-user writes go through `IUserService.ApplyProfileOnboardingMutationAsync` with `UserProfileOnboardingMutation.SetSuspension` (called by `HumanLifecycleService`), which delegates to `IUserRepository.SetSuspensionAsync` — that call writes `users.State` and nothing else. The suspension **reason** is not stored on the row: `HumanLifecycleService.SuspendAsync` puts it in the audit log (`AuditAction.MemberSuspended`) and in the `AccessSuspended` notification body. `Profile.AdminNotes` is a general-purpose admin field and is never written by the suspension path. The bulk path is `IUserServiceInternal.SuspendProfilesForMissingConsentAsync` → `IUserRepository.SuspendManyAsync`, which returns only the users whose state actually moved — a row already `Rejected`/`Merged`/`Deleted` outranks `Suspended` and is left alone. Unsuspending re-classifies from the remaining fields, so a rejected or deletion-pending row does not come back as `Active`. The User entity has no `IsArchived` / `SuspendedAt` / `SuspensionReason` columns; "archive" / "lockout" semantics are achieved by anonymizing identity fields and removing OAuth logins through `IAccountDeletionService.PurgeAsync` / `AnonymizeExpiredAccountAsync`.
...
| Authenticated human | Read own User row. Self-declare `NotAttending` for the active event year (`POST /Profile/Me/DeclareNotAttending` → `IUserServiceInternal.DeclareNotAttendingAsync`; undo via `/Profile/Me/UndoNotAttending`). Request account deletion. |
...
- **On admin participation backfill:** `IUserServiceInternal.BackfillParticipationsAsync` writes records with `Source = AdminBackfill`.
- **On account-deletion request (user-initiated):** `IAccountDeletionService.RequestDeletionAsync` orchestrates the user-initiated path. Internally calls `IUserServiceInternal.SetDeletionPendingAsync` to stamp `DeletionRequestedAt` + `DeletionScheduledFor` on the User row, revokes team memberships and governance roles immediately so the user loses access during the 30-day grace period, and sends a confirmation email.
- **On scheduled deletion expiry:** `ProcessAccountDeletionsJob` calls `IAccountDeletionService.AnonymizeExpiredAccountAsync` per due user. It captures the identity slice the caller needs for the confirmation email, resolves the merge chain, and calls `IGdprService.EraseForUserAsync` per id — the erasure fan-out lives in Gdpr beside the export one, so the loop over `IUserDataContributor`s and the `UserService.Account`-last ordering are Gdpr's concern. A contributor that throws aborts the run with the deletion markers still set, so the whole cascade retries the next day. Each archived id's `UserInfo` cache entry is dropped as its erasure completes (interleaved, so a mid-chain failure still invalidates the ids already erased); then the cross-cutting caches (Teams active-teams + member, role-assignment claims, shift authorization, shift view) — the survivor's `UserInfo` entry refreshes automatically as part of the `IUserService` write path. The job writes the `AuditAction.AccountAnonymized` audit entry and sends the confirmation email.
- **On admin purge (non-Production only):** `UsersAdminController.PurgeHuman` (`POST /Users/Admin/{id}/Purge`) calls `IAccountDeletionService.PurgeAsync`, which runs the same Article 17 fan-out as the scheduled expiry path — `IGdprService.EraseForUserAsync` per merge-chain id, survivor last — so an admin purge never erases less than the scheduled job does; the Account contributor inside that fan-out owns the identity collapse (tombstone name/address, external logins, permanent lockout). The orchestrator then drops the Teams active-teams cache, refreshes the cached `UserInfo` entry, and invalidates per-user role-assignment / shift-authorization / shift-view caches.
- **On account merge accept:** `IAccountMergeService.AcceptAsync` (Users section) resolves the request and delegates to the one merge engine, `MergeAsync(survivor, archived, …)` — the admin picks the survivor, so the request's stored direction can be flipped. `MergeAsync` fans out its `IUserMerge` implementations to re-FK source → target (Users' own `IUserService.ReassignAsync` calls `IUserRepository.ReassignLoginsToUserAsync` for `AspNetUserLogins` and `ReassignEventParticipationToUserAsync` for `event_participations`), then `AnonymizeForMergeAsync` to tombstone the source row by setting `MergedToUserId` + `MergedAt` and bumping `LockoutEnd` far-future so the source can no longer sign in. The source `User` row stays in place as a redirect — it is NOT deleted — so append-only history written under the source id (audit log, consent records, budget audit log) remains attributable.
...
- **`.Contracts` leaves:** `AuditLog` (every write path records an audit entry), `Auth` (role assignment on provisioning and merge), `Camps` (camp name + roles on the admin popover), `Campaigns`, `Consent` (profile completeness gates on the consent state), `Email` (verification, unsubscribe and merge-notification mail — Users owns its four templates (`email_verification`, `deletion_requested`, `account_deleted`, `access_suspended`) via `UsersEmails` + `UsersEmailPreviews`, keys `Users_Email_*`; Email is the transport only), `Gdpr` (`UserService` and `AccountMergeService` are Article 15 contributors), `Governance` (`IMembershipCalculatorRead` for non-compliant suspension), `GoogleIntegration` (`IGoogleSyncService` drops a suspended human from team resources; `ITeamResourceService` verifies a coordinator's team Group before team-reply messaging), `Notifications` (merge requests and email problems notify the human), `Onboarding` (profile editing reports onboarding progress), `Shifts` (participation backfill reads the burn settings), `Teams` (account merge folds team memberships), `Tickets` (participation is derived from ticket sync).
...
- **Account merge (`IAccountMergeService.AcceptAsync`, Users section):** the `IUserMerge` fan-out calls Users' own `IUserService.ReassignAsync` (which re-FKs `AspNetUserLogins` and `event_participations` via `IUserRepository`) and `IUserServiceInternal.AnonymizeForMergeAsync` to fold a source User into a target.
...
- **GDPR:** `UserService` implements `IUserDataContributor` and contributes the `UserService.Account` and `EventParticipations` slices plus the Profiles-half slices (`Profile`, `ContactFields`, `UserEmails`, `VolunteerHistory`, `Languages`, `CommunicationPreferences`); `tests/Humans.Web.Tests/Services/Gdpr/GdprExportDependencyInjectionTests.cs` discovers contributors by reflection — nothing to register in a list (design-rules §8).
...
- Event-participation writes must all go through one of `IUserServiceInternal.DeclareNotAttendingAsync`, `UndoNotAttendingAsync`, `BackfillParticipationsAsync`, or `IUserService.SetParticipationFromTicketSyncAsync`, `RemoveTicketSyncParticipationAsync`. `TicketSyncService` does this; new writers must follow the same pattern. The repository-level `UpsertParticipationAsync` is internal to the section.
...
Sub-aggregate of Profile — no separate service. Written through `IProfileEditorService.SaveProfileAsync` (which calls `IUserServiceInternal.SaveProfileVolunteerHistoryAsync`); read via `UserInfo.VolunteerHistory`.
...
- Profile-less users (legacy contact imports whose Stub Profile has not yet been materialized by the `/Profile/Admin/Backfill` tool) render in `GET /Profile/{id}/Popover` as a sparse card showing display name, best-available verified email, and a localized "imported · no profile yet" badge — the endpoint never returns 404 for a known `User`. Truly unknown user ids still return 404. In the fallback (no-profile) branch the email is derived directly from canonical `UserEmail` rows (verified primary, then any verified) — `User.Email` is not read on that branch because it silently falls back to the legacy Identity column when `UserEmails` is not loaded (see `User.Email` SILENT-FALLBACK FOOTGUN). The full-profile branch still reads `popoverUser.Email` (pre-existing); same footgun applies and is tracked separately.
- Facilitated messaging (`/Profile/{id}/SendMessage`) is a **conduit, not a mailbox** — Humans relays one-off emails between humans and stores nothing: no message persistence, no threading, no attachments. (Deliberate contrast with Feedback, which *does* persist a conversation thread.) The body is edited through the shared `_EmailComposer` (Base) and rendered through `SanitizedMarkdownRenderer` (Markdown, `https://`-only images); max 2000 chars. The recipient's address is never disclosed to the sender. A coordinator may send with `Reply-To` set to a team Google Group only when `TeamMessageOptionsProvider` (`Services/`, an orchestrator over `ITeamServiceRead` and `ITeamResourceService`, no tables) offers the team: the viewer coordinates an active team with a Google Group prefix, and that team has an active `Group` resource with a successful sync, no error, and a `Url` equal to the group's `https://groups.google.com/a/{domain}/g/{prefix}` (the one field both GoogleIntegration link paths write consistently). `ProfileViewController` only looks the requested team up in that list, on GET and on POST alike, and sorts it for display. Personal sends set `Reply-To` to the sender's notification email only when the sender ticks "include my contact info". Every send uses `AuditAction.FacilitatedMessageSent`; team sends name and link the team in the audit entry without copying its address into the human's audit description.
- `<vc:user-parts slot user-id view-mode>` (Base's `UserPartsViewComponent`) collects every active section's `IUserPart` descriptors for a named slot (`UserPartSlots`), orders them by weight, and invokes each component `Type` with a `UserPartArgs(userId, viewMode)`; a contributor that throws is logged and skipped, and contributing components own their data and any visibility beyond the host's gate. Hosted at: `user-profile` (own/other profile main column, Events is the reference contributor); `user-profile-sidebar` (profile right sidebar, own-profile only); `user-admin-detail` (AdminDetail main column — Calendar's `UserCalendarViewComponent`); `user-admin-detail-sidebar` (AdminDetail right sidebar — Tickets' `TicketHoldingsViewComponent`, gated to non-Public view modes); `board-vote-applicant` / `onboarding-review-applicant` (Governance's and Onboarding's applicant detail pages — Users' own `ProfileCardViewComponent`, Admin view mode); `ticket-transfer-party` (Tickets' transfer admin detail, sender and receiver — Users' `HumanSummaryViewComponent`). Users contributes the last three via `Humans.Users/SectionUserParts.cs`. The profile communication-preferences panel uses the Users resource set, including its non-editable "Always on" state.
...
- **Users/Identity:** `IUserServiceRead.GetUserInfosAsync` — display data for cross-domain nav stitching. `IUserServiceInternal.AnonymizeForMergeAsync` — invoked by `AccountMergeService.AcceptAsync` to tombstone the source User during fold; AspNetUserLogins + EventParticipation re-FK is handled by the Users section's own `IUserMerge` implementation (`IUserRepository.ReassignLoginsToUserAsync` / `ReassignEventParticipationToUserAsync`).
...
- **GDPR:** `AccountMergeService` implements `IUserDataContributor` (design-rules §8a) and emits the `AccountMergeRequests` slice. The `Profile`, `ContactFields`, `UserEmails`, `VolunteerHistory`, `Languages` and `CommunicationPreferences` slices are emitted by `UserService` — `ProfileService` implements no contributor interface. Each contributor declares its own export keys as constants on its class (`AccountMergeService`, `UserService`). `tests/Humans.Web.Tests/Services/Gdpr/GdprExportDependencyInjectionTests.cs` discovers contributors by reflection — nothing to register in a list.
