...
- **Architecture tests:**
  - `tests/Humans.Users.Tests/Architecture/UserArchitectureTests.cs` — pins UserService/AccountProvisioningService/UnsubscribeService, no DbContext, IUserRepository required, no outbound edges to higher sections, nav-strip enforcement (`User_HasNoCrossDomainNavigationProperties`), and the two-marker localizer guard.
