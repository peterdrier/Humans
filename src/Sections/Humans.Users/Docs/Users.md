...
- **On scheduled deletion expiry:** `ProcessAccountDeletionsJob` calls `IAccountDeletionService.AnonymizeExpiredAccountAsync` per due user. It captures the identity slice the caller needs for the confirmation email, resolves the merge chain, and hands the id list to `IGdprService.EraseForUsersAsync` — the erasure fan-out lives in Gdpr now (since this PR), beside the export one, so the loop over `IUserDataContributor`s and the `GdprExportSections.Account`-last ordering are Gdpr's concern. A contributor that throws aborts the run with the deletion markers still set, so the whole cascade retries the next day. Gdpr returns the ids it erased; `AccountDeletionService` then drops each archived id's `UserInfo` cache entry and the cross-cutting caches (Teams active-teams + member, role-assignment claims, shift authorization, shift view) — the survivor's `UserInfo` entry refreshes automatically as part of the `IUserService` write path. The job writes the `AuditAction.AccountAnonymized` audit entry and sends the confirmation email.
...
- **Gdpr:** `IGdprService.EraseForUsersAsync` — the erasure fan-out itself lives in Gdpr (since this PR); `AccountDeletionService` resolves the merge chain, passes the id list in, and invalidates the archived ids' caches from the returned list. It names no section to erase; Gdpr fans out over its own `IUserDataContributor` roster.
