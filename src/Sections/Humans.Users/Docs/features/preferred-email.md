...
> **See [Workspace Account Provisioning](../../../Humans.GoogleIntegration/Docs/features/workspace-account-provisioning.md) for full details** including the provisioning flow, credentials email, ordering constraints, and 2FA requirements.
...
### Google Sync
`GoogleWorkspaceSyncService` uses `UserInfo.GoogleEmail` (the `UserEmail` row with `IsGoogle = true`) for:
...
### Background Jobs
These jobs use `UserInfo.PrimaryEmail` (the `UserEmail` row with `IsPrimary = true`) to send to the notification target:
- `SendReConsentReminderJob` - consent reminder emails
- `SuspendNonCompliantMembersJob` - suspension notification emails

## Related Features

- [Authentication](../../../Humans.Auth/Docs/features/authentication.md) - OAuth provides primary email
- [Profiles](profiles.md) - Email visibility on profile
- [Contact Fields](contact-fields.md) - Shares ContactFieldVisibility enum
- [Background Jobs](../../../../../docs/features/global/background-jobs.md) - Jobs send to effective email
