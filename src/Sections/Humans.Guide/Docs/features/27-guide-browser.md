...
- Gated by the `Features:Events` toggle
...
**Acceptance Criteria:**
- Day filter: toggle buttons for each event day (multi-select, derived from the linked burn settings' gate-opening date, end offset, and timezone)
- Category filter: dropdown listing active categories
- Venue filter: dropdown listing active `EventVenue` records (replaces barrio filter; individual events use shared venues)
...
- Detail shows: title, full description, category, date/time, duration, location (camp or venue name + location note), submitter display name
...
- Favourites persist via existing `EventFavourite` records
...
- Opt-out preferences are read from existing `EventPreference` records
...
| Entity | Usage |
|--------|-------|
| `Event` | Source of all approved events |
...
- **Favourites and Schedule** (US-26.8): Reuses `EventFavourite` and the `/Events/Schedule` page
- **Category Opt-Out** (US-26.7): Reuses `EventPreference`
- **Feature Toggle**: Gated by `Features:Events` in appsettings, same as all other guide UI
