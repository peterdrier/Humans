...
public class BaselineSeeder
{
    ...
    private static void WriteBaseline(string repoRoot, string relativePath, IEnumerable<string> violations, string ruleSummary)
    {
        ...
        var sorted = new SortedSet<string>(violations, StringComparer.Ordinal);
        var lines = new List<string>
        {
            "# Baseline: " + ruleSummary,
            "# One locator per line: relative/path.cs:stable-key [# L<line> diagnostics]",
            "# The diff key is everything before ' # ' — the trailing comment is informational",
            "# only (line numbers, per-rule diagnostics) and is ignored when comparing baselines.",
