...
public class GateServiceTests
{
    ...
    private readonly DbContextOptions<GateDbContext> _dbOptions =
    ...
    private readonly GateDbContext GateDb;
    private readonly TestDbContextFactory<GateDbContext> GateDbFactory;
    ...
    public GateServiceTests()
    {
        ...
        _svc = new GateService(new GateRepository(GateDbFactory), _tickets, _earlyEntry, _eventSettings, _shifts, _roles, _users, _pinHasher, _auditLog, NullLogger<GateService>.Instance, Clock);
    ...
    private void StubActiveEvent(int year = 2026) =>
        _eventSettings.GetActiveEventSettingsAsync(Arg.Any<CancellationToken>()).Returns(EventFixtures.Event(
            eventName: "Test",
            year: year,
