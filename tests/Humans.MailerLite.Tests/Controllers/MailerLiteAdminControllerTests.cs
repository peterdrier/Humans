...
/// <summary>
/// Verifies the <see cref="MailerLiteAdminController.Commit"/> action:
/// drift detection against the TempData snapshot redirects back to
/// /MailerLite/Admin/Import when counts shifted more than 10%, and calls
/// <see cref="IMailerLiteImportService.ApplyAsync"/> when within tolerance.
/// </summary>
public class MailerLiteAdminControllerTests
{
    ...
    private MailerLiteAdminController BuildSut(
        ImportPlanCounts? snapshotCounts = null,
        IEnumerable<IMailerLiteAudience>? audiences = null)
    {
    ...
    public async Task Commit_RedirectsToPreview_WhenCountsDriftedMoreThan10Percent()
    {
        ...
        Assert.Equal(nameof(MailerLiteAdminController.Import), redirect.ActionName);
        Assert.Equal(
    ...
    public async Task Debug_RendersFiveSections_AndListsAvailableAudiencesInDropdown()
    {
        ...
        var other = Substitute.For<IMailerLiteAudience>();
        ...
        var view = Assert.IsType<ViewResult>(result);
        var vm = Assert.IsType<MailerLiteAudienceDebugViewModel>(view.Model);
