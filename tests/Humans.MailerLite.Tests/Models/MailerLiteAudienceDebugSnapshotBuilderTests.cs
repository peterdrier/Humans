...
/// </summary>
public class MailerLiteAudienceDebugSnapshotBuilderTests
{
    [HumansFact]
    ...
    {
        // Sanity: the builder only takes IMailerLiteAudience / IMailerLiteService /
        ...
        var method = typeof(MailerLiteAudienceDebugSnapshotBuilder)
            .GetMethod(nameof(MailerLiteAudienceDebugSnapshotBuilder.BuildAsync))!;
        var paramTypes = method.GetParameters().Select(p => p.ParameterType).ToList();

        paramTypes.Should().ContainInOrder(
            typeof(IMailerLiteAudience),
    ...
    public async Task Paging_SlicesPagesIndependentlyPerSection()
    {
        ...
        var page1 = MailerLiteAudienceDebugSnapshotBuilder.PageExpected(rows,
            new DebugTableState("exp", 1, 50, DebugSortColumn.Name, Descending: false), options);
        var page2 = MailerLiteAudienceDebugSnapshotBuilder.PageExpected(rows,
            new DebugTableState("exp", 2, 50, DebugSortColumn.Name, Descending: false), options);
        var page3 = MailerLiteAudienceDebugSnapshotBuilder.PageExpected(rows,
    ...
    public void Paging_SortByEmailDescending_ReversesEmailOrder()
    {
        ...
        var ascending = MailerLiteAudienceDebugSnapshotBuilder.PageExpected(rows,
            new DebugTableState("exp", 1, 50, DebugSortColumn.Email, Descending: false), options);
        var descending = MailerLiteAudienceDebugSnapshotBuilder.PageExpected(rows,
    ...
    public void Paging_InvalidPageSize_FallsBackToDefault()
    {
        ...
        var page = MailerLiteAudienceDebugSnapshotBuilder.PageExpected(rows,
    ...
    private static IMailerLiteAudience StubAudience(string key, string groupName, IEnumerable<Guid> members)
    {
