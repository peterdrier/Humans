...
public class AgentToolDispatcherTests
{
    ...
    public async Task GetShiftDetails_with_block_id_returns_rota_name_dates_and_day_count()
    {
        ...
        eventSettings.GetActiveEventSettingsAsync(Arg.Any<CancellationToken>()).Returns(ev);

        var dispatcher = MakeDispatcher(shiftView: shiftView, eventSettings: eventSettings);
