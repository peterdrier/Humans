...
public class SummaryAggregateTests
{
    ...
    public async Task Single_order_single_line_projects_to_all_three_views()
    {
        ...
        camp.CounterpartyType.Should().Be(OrderCounterpartyType.Camp);
    ...
    public async Task Multiple_camps_and_products_produce_consistent_totals()
    {
        ...
        _repo.GetAllProductsForYearAsync(2026, Arg.Any<CancellationToken>()).Returns([
            new Product { Id = productX, Year = 2026, Name = "X", Description = "x",
                UnitPriceEur = 5m, VatRatePercent = 0m, OrderableUntil = new LocalDate(2026,12,31), IsActive = true },
            new Product { Id = productY, Year = 2026, Name = "Y", Description = "y",
                UnitPriceEur = 7m, VatRatePercent = 0m, OrderableUntil = new LocalDate(2026,12,31), IsActive = true }
        ]);

        Order Order(Guid id, Guid season, params (Guid pid, int qty, decimal price)[] lines)
            => new()
            {
                Id = id,
                CampSeasonId = season,
    ...
    public async Task Deactivated_product_with_lines_still_appears()
    {
        ...
        _repo.GetAllProductsForYearAsync(2026, Arg.Any<CancellationToken>()).Returns([
            new Product { Id = deadProductId, Year = 2026, Name = "Retired", Description = "x",
        ...
        _repo.GetOrdersForCampSeasonsWithLinesAndPaymentsAsync(
            ...
            .Returns([
                new Order
                {
                    Id = orderId, CampSeasonId = seasonId, State = OrderState.Open,
                    Lines =
                    {
                        new OrderLine
                        {
    ...
    public async Task ByCamp_balance_reflects_paid_partial_unpaid_states()
    {
        ...
        _repo.GetAllProductsForYearAsync(2026, Arg.Any<CancellationToken>()).Returns([
            new Product { Id = productId, Year = 2026, Name = "P", Description = "x",
                UnitPriceEur = 10m, VatRatePercent = 0m, OrderableUntil = new LocalDate(2026,12,31), IsActive = true }
        ]);

        Order Order(Guid id, Guid season, int qty, decimal paid) => new()
        {
            Id = id,
            CampSeasonId = season,
            State = OrderState.Open,
            ...
            Payments = paid == 0m
                ...
                : new List<Payment>
                {
                    new() { Id = Guid.NewGuid(), OrderId = id, AmountEur = paid,
                        Method = PaymentMethod.Manual, ReceivedAt = Instant.FromUtc(2026, 5, 1, 0, 0) }
                }
        };
    ...
    public async Task Open_order_reprices_to_current_catalog_like_the_order_page()
    {
        ...
        _repo.GetAllProductsForYearAsync(2026, Arg.Any<CancellationToken>()).Returns([
            new Product { Id = productId, Year = 2026, Name = "Tent", Description = "x",
    ...
    public async Task Order_for_camp_season_not_in_year_is_excluded()
    {
        ...
        _repo.GetOrdersForCampSeasonsWithLinesAndPaymentsAsync(
                Arg.Any<IReadOnlyCollection<Guid>>(), Arg.Any<CancellationToken>())
            .Returns([
                new Order { Id = Guid.NewGuid(), CampSeasonId = outOfYear, State = OrderState.Open }
            ]);
