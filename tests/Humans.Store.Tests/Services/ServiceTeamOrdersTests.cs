...
/// See <c>src/Sections/Humans.Store/Docs/Store.md</c> (Architecture — the schema decision for why
/// team orders reuse <c>Order</c> rather than a separate table).
/// </summary>
public class ServiceTeamOrdersTests
{
    ...
    public ServiceTeamOrdersTests()
    {
        ...
        _service = new Service(_repo, _audit, _campService, _teams, _clock, _eventSettings, _stripe, _holded, Options.Create(_storeOptions), NullLogger<Service>.Instance);
    }
    ...
    public async Task GetIndexDataAsync_privileged_reader_lists_departments_it_does_not_coordinate()
    {
        ...
        _repo.GetOrderForTeamAsync(otherDeptId, 2026, Arg.Any<CancellationToken>()).Returns((Order?)null);
        _repo.GetActiveProductsForYearAsync(2026, Arg.Any<CancellationToken>())
            .Returns(new List<Product>());
        ...
        data.Counterparties.Should().ContainSingle(c =>
            c.CounterpartyType == OrderCounterpartyType.Team && c.CounterpartyId == otherDeptId);
    }
    ...
    public async Task GetStoreSummaryAsync_merges_team_orders_into_by_counterparty_and_cross_tab()
    {
        ...
        _repo.GetOrdersForTeamsWithLinesAsync(
            ...
            .Returns([new Order {
                Id = orderId, TeamId = deptId, Year = 2026, State = OrderState.Open,
                Lines = { new OrderLine {
        ...
        result.ByCounterparty.Should().ContainSingle(r =>
            r.CounterpartyType == OrderCounterpartyType.Team
            && r.OrderId == orderId
    ...
    public async Task DeleteOrderAsync_removes_zero_balance_order_and_audits()
    {
        ...
        _repo.GetOrderWithLinesAndPaymentsAsync(orderId, Arg.Any<CancellationToken>())
            .Returns(new Order { Id = orderId, CampSeasonId = Guid.NewGuid(), Year = 2026 });
        ...
        decimal balanceEur)
    {
        return new OrderDto(
            Id: Guid.NewGuid(),
            CampSeasonId: counterpartyType == OrderCounterpartyType.Camp ? Guid.NewGuid() : null,
            TeamId: counterpartyType == OrderCounterpartyType.Team ? Guid.NewGuid() : null,
