...
public class CreditorServiceTests
{
    ...
    private CreditorService MakeService() => new(
        _repo,
        _client,
        _holded,
        _clock,
        _cache,
        NullLogger<CreditorService>.Instance);
    ...
    public async Task EnsureCreditorContact_ReboundDuringHoldedRoundTrip_DoesNotClobberTheNewBinding()
    {
        ...
        var logger = new CapturingLogger<CreditorService>();
        _repo.GetCreditorContactByUserAsync(userId, Arg.Any<CancellationToken>()).Returns(
            new HoldedCreditorContact
            {
                Id = bindingId,
        ...
        var id = await new CreditorService(_repo, _client, _holded, _clock, _cache, logger)
    ...
    public async Task SetCreditorAccountNum_UnboundAndReboundBetweenReadAndWrite_DoesNotClobberTheNewBinding()
    {
        ...
        await new CreditorService(_repo, _client, _holded, _clock, _cache, logger)
