...
/// <remarks>
/// <para>
/// Mailer has none of the usual halves — no resource set (the two admin pages are English
/// operator copy, so there is no <c>NotContain("Mailer_")</c> and no Spanish request) and no
...
/// </para>
/// </remarks>
public class MailerPageRenderTests(HumansTestDatabase database) : IntegrationTestBase(database)
{
    ...
    public async Task Every_mailer_page_renders_its_own_copy_with_tag_helpers_applied()
    {
        ...
        [
            ("/Mailer/Admin", "Mailer Dashboard"),
            ("/Mailer/Admin/Import", "Import Preview"),
            ("/Mailer/Admin/Audiences/ticket-no-shifts/Debug", "Audience Debug"),
        ];
    ...
    public async Task Import_preview_renders_the_matched_human_through_the_view_component()
    {
        ...
        var response = await Client.GetAsync("/Mailer/Admin/Import", ct);
    ...
    /// <summary>
    /// Every <c>/Mailer/Admin/*</c> route is <c>AdminOnly</c>. The controller is now
    ...
    public async Task Non_admin_cannot_reach_the_mailer_dashboard()
    {
        ...
        var response = await Client.GetAsync("/Mailer/Admin", ct);
