...
Onboarding-specific value types: `OnboardingResult`, `BulkOnboardingResult`. The consent-check threshold (`Profile.ConsentCheckStatus → Pending` + Consent Coordinator notification) is owned by `IProfileService.TrySetConsentCheckPendingIfEligibleAsync` — leaves (`ProfileService.SaveProfileAsync`, `ConsentService.SubmitConsentAsync`) fire it at their natural completion points, never through Onboarding.
...
- **No leaf-to-director callbacks.** Neither `ProfileService` nor `ConsentService` depends on `IOnboardingService`. The consent-check threshold (was `IOnboardingEligibilityQuery.SetConsentCheckPendingIfEligibleAsync`) was relocated to `IProfileService.TrySetConsentCheckPendingIfEligibleAsync` — the field's owner. Both leaves call it at their natural completion points. Director-to-leaf is one-way: `OnboardingService` writes through `IProfileService` during clear/flag/reject/approve, never the reverse.
...
- When a human completes their profile and signs all required documents: `ProfileService.SaveProfileAsync` fires `IProfileService.TrySetConsentCheckPendingIfEligibleAsync` inline. If the predicate holds (profile eligible for review, no existing `ConsentCheckStatus`, `IMembershipCalculator.HasAllRequiredConsentsForTeamAsync(Volunteers)` returns true), it flips `Profile.ConsentCheckStatus` to `Pending` and notifies the ConsentCoordinator role. This is the audit/annotation track — admission to Volunteers happens automatically and does not depend on this flow.
- When a legal document is signed: `ConsentService.SubmitConsentAsync` fires `IProfileService.TrySetConsentCheckPendingIfEligibleAsync` (same threshold check from the Consent side) and `SyncVolunteersMembershipForUserAsync`, which admits the user to the Volunteers system team if `!IsSuspended && ConsentCheckStatus != Flagged && RejectedAt is null && HasAllRequiredConsentsForTeam(Volunteers)`. `Profile.IsApproved` is not consulted.
...
- **DI direction is one-way.** `OnboardingService → IProfileService` (and other leaves) only. No leaf depends on `IOnboardingService`. The historical `IOnboardingEligibilityQuery` narrow-interface band-aid was removed once the threshold check moved to its rightful owner (`IProfileService.TrySetConsentCheckPendingIfEligibleAsync`). Reviewers should reject any new ctor dependency from `ProfileService`/`ConsentService` onto `IOnboardingService` — that's the inversion this PR removed.
