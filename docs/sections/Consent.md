...
- **Own DbContext (#858):** all three tables live in `LegalDbContext` (`src/Humans.Infrastructure/Data/LegalDbContext.cs`), peeled out of `HumansDbContext` on 2026-08-10; both repositories take `IDbContextFactory<LegalDbContext>`. Migrations are under `Migrations/Legal/` against `__EFMigrationsHistory_Legal`, and the baseline `20260810192942_BaselineLegal` re-creates the `consent_records` immutability triggers. The section stayed in Base — the peel was the DbContext, not a G5 project move.
...
- **Cross-section calls:** `IProfileService`, `IOnboardingService`, `ITeamService`, `INotificationService`, `INotificationAutoResolve`, `IHumanLifecycleService`, `IMembershipCalculator`, `IUserService`.
