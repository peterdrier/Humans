...
- **Invalidation** — two paths:
  - Decorator-mediated: every mutating method calls `ICampInfoInvalidator.InvalidateCampAsync(campId)` / `InvalidateSettingsAsync()` after delegating to the inner service.
  - `CampInfoSaveChangesInterceptor` (Infrastructure, registered for both `AddDbContext` and `AddDbContextFactory` pipelines in `Program.cs`) catches every persisted mutation to `camps`, `camp_seasons`, `camp_leads`, `camp_historical_names`, `camp_images`, `camp_settings`, and — critically — `camp_members`. Without the `camp_members` watch, `CampSeasonInfo.EeGrantedCount` would stay stale after grants/revokes until process restart (historical drift bug — pinned by `CampsArchitectureTests`, documented in `CampInfo` `<remarks>`).
...
- **Cache size budget.** ~5 MB at the 100-camp / 500-user steady state — comfortably under the §15 50-MB ceiling. Per-entry footprint dominated by season blurbs (~2 KB/season × ≤6 seasons of history).
