...
8. **Link to specs, do not duplicate.** Workflows belong in the section's own `src/Sections/Humans.<Section>/Docs/<feature>.md`; UI text choices belong in CLAUDE.md. Section docs point to those files, they do not copy from them.
...
- **No workflow diagrams.** State machines live under `src/Sections/Humans.<Section>/Docs/<feature>.md`. The section doc states "Status follows: Submitted → Approved/Rejected" and stops.
