...
4. **Tool whitelist.** Only `fetch_feature_spec`, `fetch_section_guide`, `get_shift_details`, `route_to_issue` are valid tool names. Unknown names return a tool error; filesystem is never touched outside `docs/sections/` and `docs/features/`. `get_shift_details` resolves an id (block or singleton) against the calling user's own signups only — anything that doesn't belong to the caller returns "Shift not found" without leaking other users' shifts.
...
- **Profiles / Users / Auth / Teams / Tickets / Shifts** — `IAgentUserSnapshotProvider` composes the per-turn user context from `IProfileService`, `IUserService`, `IRoleAssignmentService.GetActiveForUserAsync`, `ITeamService.GetActiveTeamMembershipsForUserAsync` (team name + role), `ITicketQueryService.GetOpenTicketIdsForUserAsync`, and `IShiftSignupService.GetByUserAsync` (filtered to upcoming signups in the active event from `IShiftManagementService.GetActiveAsync`, conflated by `SignupBlockId` so multi-day range signups become one entry). The Shifts integration also backs the `get_shift_details` tool, which `IAgentToolDispatcher` resolves against the calling user's signups only (privacy guard).
