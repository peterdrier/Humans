...
- **Test coverage — the orchestration is untested; the per-bucket behaviour largely is not.** The gap is real but narrower than "none": there is no `SearchArchitectureTests`, no `SearchServiceTests` and no `SearchControllerTests`, so **nothing this section itself does is covered** — the query-length gate, the `onlyType` short-circuit, the five-bucket fan-out, the `Features:Events` gate, and the non-human 100/80/60 rubric are all unverified. What *is* covered lives in the owning sections' own tests, because that is where the filters live:
  ...
  - **Humans score tiers** — 18 cases in `PersonSearchMatcherTests.cs`.
