<!-- freshness:triggers
  ...
  src/Humans.Infrastructure/Data/AuthDbContext.cs
  src/Humans.Infrastructure/Data/AuthDbContextFactory.cs
...
- `IRoleAssignmentRepository` (impl `Humans.Infrastructure/Repositories/Auth/RoleAssignmentRepository.cs`) owns the SQL surface for `role_assignments`, reading/writing through `IDbContextFactory<AuthDbContext>` so the repository is registered Singleton while `AuthDbContext` itself stays Scoped. Since nobodies-collective/Humans#858, `role_assignments` lives in its own `Humans.Infrastructure/Data/AuthDbContext.cs` (own `__EFMigrationsHistory_Auth` table, migrations under `Migrations/Auth/`) — not `HumansDbContext`. Same database and connection; the split is code-side only.
