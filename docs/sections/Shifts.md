...
- Browse-page department filter (`GetBrowseShiftsAsync(departmentId)`) mirrors the pie bucketing: it returns rotas owned by the department itself **plus** rotas on non-promoted sub-teams of that department. Promoted sub-teams (`IsPromotedToDirectory = true`) have their own pie and filter separately. Resolution lives in `ShiftManagementService.ResolveDepartmentTeamIdsAsync`; the repo takes a flat `IReadOnlyCollection<Guid>` so the service is the only place that knows about the hierarchy.
