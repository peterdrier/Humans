...
- **`UserEmail.IsPrimary` invariants.** Service-layer guarantee in `UserEmailService.EnsurePrimaryInvariantAsync` (exactly one verified `IsPrimary` per user, recovers from zero/multi states). Account-merge fold preserves target's `IsPrimary` and demotes the source's. Partial unique index `IX_user_emails_UserId_PrimaryVerified` `(UserId, IsPrimary, IsVerified) WHERE IsNotificationTarget = true AND IsVerified = true` is the DB safety net (column kept under legacy name `IsNotificationTarget` per `no-column-drops-for-decoupling.md`).
