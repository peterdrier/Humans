...
- Profile-less users (legacy contact imports whose Stub Profile has not yet been materialized by the `/Profile/Admin/Backfill` tool) render in `GET /Profile/{id}/Popover` as a sparse card showing display name, best-available verified email, and an "imported · no profile yet" badge — the endpoint never returns 404 for a known `User`. Truly unknown user ids still return 404. The fallback email is derived directly from canonical `UserEmail` rows (verified primary, then any verified); `User.Email` is never read here because it silently falls back to the legacy Identity column when `UserEmails` is not loaded (see `User.Email` SILENT-FALLBACK FOOTGUN).
