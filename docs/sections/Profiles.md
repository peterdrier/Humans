...
- **Write-through sync (issue #692).** `IProfileService.SaveProfileAsync` writes `User.DisplayName ← Profile.BurnerName` in the same operator action after every profile save. After the first save, `User.DisplayName` cannot drift from `Profile.BurnerName` on any new write. Rendering surfaces still resolve through `FullProfile.BurnerName` / `Profile.BurnerName ?? User.DisplayName`; direct reads of `User.DisplayName` are limited to the four legitimate categories in `memory/architecture/burnername-is-the-display-name.md`.
