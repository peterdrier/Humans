...
- **Onboarding:** none (one-directional — `OnboardingService` consumes `IProfileService`, never the reverse). The consent-check threshold (`Profile.ConsentCheckStatus → Pending` + Consent Coordinator notification) is owned here as `IProfileService.TrySetConsentCheckPendingIfEligibleAsync` and self-invoked from `SaveProfileAsync`.
