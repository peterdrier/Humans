...
All 6 services live in `Humans.Application/Services/Profile/`. Each has a repository in `Humans.Application/Interfaces/Repositories/` with EF implementation in `Humans.Infrastructure/Repositories/`. The `IProfileStore` provides in-memory caching via the `CachingProfileService` decorator.
