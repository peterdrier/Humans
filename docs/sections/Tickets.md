...
Cross-domain nav `TicketOrder.MatchedUser` stripped (FK `MatchedUserId` retained; nav property removed). No FK-join in `ITicketRepository`; join to User is done in-memory via `IUserService.GetByIdsAsync`.
...
Cross-domain nav `TicketAttendee.MatchedUser` stripped (FK `MatchedUserId` retained; nav property removed). No FK-join in `ITicketRepository`; join to User is done in-memory via `IUserService.GetByIdsAsync`.
...
- Admin decisions: **"Process transfer"** (shown only when `TicketVendorSettings.EnableAutomatedTransferWriteback` is on) runs the automated TicketTailor void(-to-hold)+reissue and, on success, sets `Approved` and writes the swapped local attendee rows; **"Mark successful"** sets `Approved` with no vendor call (team voided+reissued by hand, or to close out a partial automated attempt); **"Cancel transfer"** requires a reason and sets `Rejected`. On automated vendor failure the request stays `Pending` with the diagnostic recorded. The next ticket sync reconciles `ticket_attendees`.
...
- `TicketTransferService` — transfer request lifecycle: `GetMyAttendeesAsync`, `GetConfirmationAsync`, `CreateRequestAsync`, `CancelAsync`, `ApproveAsync` (mark successful — no vendor call), `ProcessTransferAsync` (automated void(-to-hold)+reissue, gated by `EnableAutomatedTransferWriteback`), `RejectAsync` (cancel with reason). Emails Sender + tickets@ on request, Sender + Receiver on decision.
...
- **`ITicketRepository`** (Tickets-owned) — owns reads/writes for `ticket_orders`, `ticket_attendees`, `ticket_sync_states`. Aggregate-local navs kept (`TicketOrder.Attendees`, `TicketAttendee.TicketOrder`). Cross-domain `MatchedUser` nav properties have been stripped from both entities; FK (`MatchedUserId`) is retained and joining to `User` is done in-memory via `IUserService.GetByIdsAsync` after the read.
