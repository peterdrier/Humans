...
- `TicketTransferService` — transfer request lifecycle: `GetMyAttendeesAsync`, `GetConfirmationAsync`, `CreateRequestAsync`, `CancelAsync`, `ApproveAsync` (mark successful — no vendor call), `ProcessTransferAsync` (automated void(-to-hold)+reissue), `RejectAsync` (cancel with reason). Emails Sender + tickets@ on request, Sender + Receiver on decision.
