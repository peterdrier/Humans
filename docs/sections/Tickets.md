...
<!-- wheat: docs/superpowers/specs/2026-05-25-early-entry-roster-design.md §Consumers -->
- A stub's Early Entry pill is **always the viewer's own** earliest entry date, never the attendee's: every surface that renders stubs (homepage strip, holdings list, transfer wizard, Scanner gate card) goes through the single `TicketStubInfo.From(row, holderEarlyEntry)` mapper, which stamps one value supplied by the caller from `IEarlyEntryService` for the *current* viewer. A transfer therefore cannot leak the sender's EE status to the recipient, and the pill can never be present on one surface and missing on another.
