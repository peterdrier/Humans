...
- **Ticket Sync** is a background Hangfire job (`TicketSyncJob`, every 5 min per the shipped `TicketVendor:SyncIntervalMinutes`; code default 15) that pulls order, attendee, and gate check-in data from the vendor through `ITicketVendorService`.
