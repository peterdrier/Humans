...
| ResourceId | Guid? | FK → GoogleResource, `OnDelete: SetNull`. Only set for Google sync entries. No nav property — the `Resource` nav was dropped in nobodies-collective/Humans#1188; EF uses the typed-FK form (`HasOne<GoogleResource>()`). |
