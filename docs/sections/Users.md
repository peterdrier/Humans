...
- **When `AnonymizeExpiredAccountAsync` runs:** one `forgotten_emails` row is written per deleted `UserEmail` via `IForgottenEmailService.RecordForgottenAsync`. Hashes the address before storing so the GDPR-erasure guarantee survives the skip-list.
