...
| Admin | Trigger account merge flows on the unified **Account Merges** page (`/Users/Admin/AccountMerges`, `UsersAdminAccountMergesController`); `AccountMergeService` + `DuplicateAccountService` are Users-section services. Outside Production only: purge a human via `ProfileController.PurgeHuman`. |
...
- An Admin **cannot** purge their own account — `ProfileController.PurgeHuman` returns the user to the admin-detail page with an error when `user.Id == currentUser.Id`.
- Purge **cannot** run in Production — `ProfileController.PurgeHuman` returns `NotFound` when `IWebHostEnvironment.IsProduction()`. Account anonymization (the GDPR-deletion path through `AnonymizeExpiredAccountAsync`) runs in every environment via `ProcessAccountDeletionsJob`.
