...
- A 400000xx account binds to **at most one member**. `SetCreditorContactAsync` refuses a bind whose account already carries another member's binding and writes nothing — only `UserId` is unique in the DB, so nothing else stops two members' payables merging onto one account (nobodies-collective/Humans#974).
