...
- A 400000xx account should bind to **at most one member**, but only the manual path enforces it: `SetCreditorContactAsync` refuses a bind whose account — *or whose resolved Holded contact* — already carries another member's binding, and writes nothing (nobodies-collective/Humans#974). Both checks are needed: after the one-shot number resolution misses, a binding carries a contact id with a null `SupplierAccountNum`, which the account-number check cannot see. The two automatic writers — `SetCreditorAccountNumAsync` (records the number Holded assigned after a push) and the `seedAccountNum` carried through `EnsureCreditorContactAsync` — are **unguarded**, and the DB index on `SupplierAccountNum` is non-unique, so a duplicate can still land. Tracked as nobodies-collective/Humans#975.
