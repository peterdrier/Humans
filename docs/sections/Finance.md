...
- A 400000xx account should bind to **at most one member**, but only the manual path enforces it: `SetCreditorContactAsync` refuses a bind whose account already carries another member's binding and writes nothing (nobodies-collective/Humans#974). The two automatic writers — `SetCreditorAccountNumAsync` (records the number Holded assigned after a push) and the `seedAccountNum` carried through `EnsureCreditorContactAsync` — are **unguarded**, and the DB index on `SupplierAccountNum` is non-unique, so a duplicate can still land. Tracked as nobodies-collective/Humans#975.
- Creditor accounts are the `40000000`–`40000099` block (`CreditorAccountMin`/`Max`). Every read that draws on Holded's contact list must filter to it — Holded assigns a supplier number to *every* supplier contact, so an unfiltered list turns ordinary org vendors into bindable member creditor accounts.
- **Unbound is a valid state, not an error.** A first-time submitter has no creditor contact until their first push; `EnsureCreditorContactAsync` creates it and `SetCreditorAccountNumAsync` records the assigned 400000xx. The bind control exists only for a *pre-existing* Holded contact the auto-create would duplicate.
- `ListCreditorAccountsAsync` is the one creditor read that calls Holded live (`ListContactsAsync`) — the 400000xx account **name** lives only in Holded, and a contact created today would be invisible to a nightly cache. It degrades to blank names when Holded is unreachable rather than failing the page. If it gets slow, cache it behind a service-layer caching decorator (design-rules §15), not a name table.
