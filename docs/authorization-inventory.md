...
These three composite handlers, `AuthorizationPolicyExtensions.cs`, `MembershipRequiredFilter.cs`, `NameRequiredFilter.cs`, and `HangfireAuthorizationFilter.cs` are the only authorization files left directly in `src/Humans.Web/` — every resource-based handler now lives in its owning section, and the framework-facing plumbing (`PolicyNames`, `RoleNames`, `RoleGroups`, `RoleChecks`, `ShiftRoleChecks`, `HumansControllerBase`, `AuthorizeViewTagHelper`) lives in `src/Humans.Interfaces/` (moved off the deleted `Humans.UI`/`Humans.Domain` projects — #8868f53394/#c1d55c4de9-era G5 lanes — while keeping their original namespaces `Humans.UI.Authorization` / `Humans.Domain.Constants` / `Humans.UI.Controllers` / `Humans.UI.TagHelpers`, so `using` directives referencing those namespaces are correct even though the physical files sit under `Humans.Interfaces`).
