...
| File | Line | Call |
|---|---|---|
| `src/Sections/Humans.Teams/Contracts/HumansTeamControllerBase.cs` | 31 | `AuthorizeAsync(User, team, TeamOperationRequirement.ManageCoordinators)` (`ResolveTeamManagementAsync`; moved with the Teams section — #1280, line shifted +7) |
| `src/Sections/Humans.Teams/Contracts/HumansTeamControllerBase.cs` | 44 | `AuthorizeAsync(User, team, TeamOperationRequirement.ManageEarlyEntry)` (`ResolveEarlyEntryManagementAsync`) |
| `src/Sections/Humans.Teams/Controllers/TeamController.cs` | 165 | `AuthorizeAsync(User, teamInfo, TeamOperationRequirement.ManageEarlyEntry)` (drives `CanManageEarlyEntry` view-model flag on team details; moved with the Teams section — #1280, line shifted +3 by added `using`s) |
| `src/Sections/Humans.Teams/Controllers/TeamController.cs` | 684 | `AuthorizeAsync(User, PolicyNames.AdminOnly)` (EditTeam POST — `IsSensitive` leave-unchanged guard for non-Admin editors) |
...
| `src/Humans.Web/Controllers/ProfileController.cs` | 719, 752, 797, 839, 876, 913, 950, 970, 1014, 1093, 1109, 1135, 1168, 1194, 1220, 1395, 1421, 1465 | `AuthorizeAsync(User, userId, UserEmailOperations.Edit)` (18 email-edit endpoints; first 15 sites shifted +1 by an added `using`, last 3 unchanged) |
...
| `src/Humans.Web/Controllers/UsersAdminController.cs` | 304 | `AuthorizeAsync(User, model.RoleName, RoleAssignmentOperationRequirement.Manage)` (AddRole — moved off `ProfileController` in #901) |
| `src/Humans.Web/Controllers/UsersAdminController.cs` | 342 | `AuthorizeAsync(User, roleAssignment.RoleName, RoleAssignmentOperationRequirement.Manage)` (EndRole — moved off `ProfileController` in #901) |
