...
| `src/Humans.Web/Controllers/ExpensesController.cs` | 204, 591, 637, 657, 702, 723 | `AuthorizeAsync(User, report, new ExpenseReportOperationRequirement(ExpenseReportOperation.X))` — `View` (Detail 204, Attachment 591), `Endorse` 637, `CoordinatorReject` 657, `Approve` 702, `FinanceReject` 723 |
...
| `src/Humans.Web/Controllers/StoreController.cs` | 54, 76, 79, 80, 81, 103, 120, 138, 178, 196, 225, 255, 282, 307 | `AuthorizeAsync(User, order/resource, StoreOrderOperationRequirement.X)` (and `StoreOrderCreateContext` / `StoreOrderLineContext` for Create at 103 and line-deadline-aware AddLine/RemoveLine at 255/282) |
...
| `src/Humans.Web/Controllers/ProfileController.cs` | 716, 749, 794, 836, 873, 910, 947, 967, 1011, 1090, 1106, 1132, 1165, 1191, 1217, 1393, 1419, 1463 | `AuthorizeAsync(User, userId, UserEmailOperations.Edit)` (18 email-edit endpoints) |
| `src/Humans.Web/Controllers/ProfileController.cs` | 1875 | `AuthorizeAsync(User, PolicyNames.TicketAdminBoardOrAdmin)` (onsite-chip visibility gate) |
| `src/Humans.Web/Controllers/ProfileController.cs` | 1971 | `AuthorizeAsync(User, PolicyNames.PrivilegedSignupApprover)` (inside `BuildSentMessagesContextAsync` private helper — gates whether a non-own-profile viewer sees the "sent messages" panel on the profile page; admits coordinators or `PrivilegedSignupApprover` role) |
| `src/Humans.Web/Controllers/UsersAdminController.cs` | 302 | `AuthorizeAsync(User, model.RoleName, RoleAssignmentOperationRequirement.Manage)` (AddRole — moved off `ProfileController` in #901) |
| `src/Humans.Web/Controllers/UsersAdminController.cs` | 340 | `AuthorizeAsync(User, roleAssignment.RoleName, RoleAssignmentOperationRequirement.Manage)` (EndRole — moved off `ProfileController` in #901) |
