...
| `BudgetAdminController` (`src/Humans.Web`, `[Route("Finance")]`) | Class | `FinanceAdmin, Admin` | `PolicyNames.FinanceAdminOrAdmin` (the other 23 actions of the old `FinanceController` — Budget years/groups/categories/line-items CRUD, cash flow, audit log, ticketing-budget sync; same `/Finance` route prefix, no URL moved — #1239) |
...
| `src/Sections/Humans.Expenses/Controllers/ExpensesController.cs` | 163, 554, 600, 620, 665, 686 | `AuthorizeAsync(User, report, new ExpenseReportOperationRequirement(ExpenseReportOperation.X))` — `View` (Detail 163, Attachment 554), `Endorse` 600, `CoordinatorReject` 620, `Approve` 665, `FinanceReject` 686 (moved with the Expenses section — #1240; line numbers shifted by the same PR's member-ledger/backdated-sync fix, #1241) |
...
| `src/Humans.Web/Controllers/IssuesController.cs` | 191, 261, 307, 334, 361, 386 | `AuthorizeAsync(User, issue, IssuesOperationRequirement.Handle)` |
| `src/Humans.Web/Controllers/CityPlanningApiController.cs` | 275, 300, 338 | `AuthorizeAsync(User, ...)` (resource-based) |
| `src/Humans.Web/Controllers/ProfileController.cs` | 717, 750, 795, 837, 874, 911, 948, 968, 1012, 1091, 1107, 1133, 1166, 1192, 1218, 1394, 1420, 1464 | `AuthorizeAsync(User, userId, UserEmailOperations.Edit)` (18 email-edit endpoints) |
| `src/Humans.Web/Controllers/ProfileController.cs` | 1876 | `AuthorizeAsync(User, PolicyNames.TicketAdminBoardOrAdmin)` (onsite-chip visibility gate) |
| `src/Humans.Web/Controllers/ProfileController.cs` | 1972 | `AuthorizeAsync(User, PolicyNames.PrivilegedSignupApprover)` (inside `BuildSentMessagesContextAsync` private helper — gates whether a non-own-profile viewer sees the "sent messages" panel on the profile page; admits coordinators or `PrivilegedSignupApprover` role) |
...
| `src/Humans.Web/Controllers/AgentController.cs` | 49 | `AuthorizeAsync(User, user.Id, PolicyNames.AgentRateLimit)` |
