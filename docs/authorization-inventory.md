...
| `ExpensesController.SepaReopen` | Action | `FinanceAdmin, Admin` | `PolicyNames.FinanceAdminOrAdmin` + resource-based `ExpenseReportOperation.ReopenSepa` (reopens a SepaSent report to Approved — #982) |
| `ExpensesController.SepaGenerate` | Action | `FinanceAdmin, Admin` | `PolicyNames.FinanceAdminOrAdmin` |
| `ExpensesController` runtime guards | In-method | `authService.AuthorizeAsync(User, report, new ExpenseReportOperationRequirement(ExpenseReportOperation.X))` — `View` (Detail + Attachment), `Endorse`, `CoordinatorReject`, `Approve`, `FinanceReject`, `IncludeInSepaPayout`, `ReopenSepa` | Resource-based (see §6) |
...
| Resource: expense report | `authService.AuthorizeAsync(User, report, new ExpenseReportOperationRequirement(ExpenseReportOperation.X))` — `View`, `Endorse`, `CoordinatorReject`, `Approve`, `FinanceReject`, `IncludeInSepaPayout`, `ReopenSepa` (submitter-side actions use inline `SubmitterUserId` owner checks instead) | `Model.CanX` (view-model) |
...
| `ExpensesController` | Detail/Attachment view, Endorse, CoordinatorReject, Approve, FinanceReject, SepaReopen, SEPA payout | `authService.AuthorizeAsync(User, report, new ExpenseReportOperationRequirement(ExpenseReportOperation.X))` (resource-based) |
...
| `ExpenseReportAuthorizationHandler` | `ExpenseReportOperationRequirement` (`View`, `Edit`, `Submit`, `Withdraw`, `Endorse`, `CoordinatorReject`, `Approve`, `FinanceReject`, `CategoryOverride`, `IncludeInSepaPayout`, `ReopenSepa`) | `ExpenseReportDto` | `src/Humans.Web/Authorization/Requirements/ExpenseReportAuthorizationHandler.cs` |
...
| `src/Humans.Web/Controllers/ExpensesController.cs` | 183, 573, 619, 642, 690, 714, 715, 801 | `AuthorizeAsync(User, report, new ExpenseReportOperationRequirement(ExpenseReportOperation.X))` — `View` (Detail 183, Attachment 573), `Endorse` 619, `CoordinatorReject` 642, `Approve` 690, `FinanceReject` 714, `ReopenSepa` 715, `IncludeInSepaPayout` 801 |
