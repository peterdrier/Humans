...
**Why:** Violates the "services own their data" rule in [`design-rules.md`](../../architecture/design-rules.md). A view component that reads `IMemoryCache` has reached past the service boundary and is making ownership/eviction decisions about data it doesn't own. This creates cache consistency bugs (the owning service can't invalidate stale entries it didn't write) and hides data access patterns from the service layer. Past incident: PR #222 added `IMemoryCache` directly to `UserAvatarViewComponent` for avatar URL resolution; the fix moved avatar caching into `IProfileService`.
