...
- `features/governance/membership-tiers.md:189` — see Question 1 below.

## Questions

Delivered to Peter inline at the end of this sweep (the report is the record, not the delivery channel):

1. **`membership-tiers.md` Business Rule 2** — "Volunteer doesn't require an Application — **consent check clearance** is sufficient". `sections/Onboarding.md` states the opposite for admission (`HasRequiredNameFields` + consents, with `ConsentCheckStatus` explicitly *not* consulted), and `MembershipCalculator` computes active membership from role assignments + Volunteers-team consents with no `ConsentCheckStatus` anywhere. The phrase is ambiguous between "consented to the legal documents" (true) and "Consent Coordinator cleared" (false), so it was left unedited pending Peter's reading.
2. **Section label vs section-doc filename** — the inventory freeze renamed the section to **Consent**, but `sections/Consent.md`'s H1, the `sections/_Index.md` row, and the `docs/README.md` row all still read "Legal & Consent". The rename PR (#1159) deliberately moved files only. Rename the labels too, or leave the display name as-is?
3. **Homeless wheat from the dietary spec** — one genuine rejected-alternative note has no home in the three allowed destinations: "`MedicalConditions` stays a plain `ProfileInfo` string, made hard to leak by a focused review/test that every serializer either omits it or gates it behind the policy; a wrapper type forcing a policy token to read medical was **rejected as over-engineering at this scale**, but noted as the fallback if leaks surface." Verified still true (`Profile.cs:92`, no wrapper type in the domain). Too narrow for `design-rules.md` as a general pattern, and `sections/*.md` excludes rationale narrative. Give it a sensitive-field subsection in `design-rules.md`, or let it die with the spec (git history keeps it)?
