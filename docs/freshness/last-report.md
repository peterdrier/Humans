...
| 9 | Add `memory/` to the catalog | Added to `editorial_trees`, with `INDEX.md`/`META.md` ignored. The two stale `Humans.Web.Extensions.DateTimeDisplayExtensions` atoms corrected to `Humans.UI.Extensions`. Its 152 rule atoms carried **zero** freshness markers, so adding the tree bare would have flooded the next sweep with ~152 "unmarked" flags — **100 atoms marked in the same pass**, each glob resolved against a real file and verified. The other 52 are pure policy (git etiquette, working habits) that cannot rot against source and were deliberately left bare. |
