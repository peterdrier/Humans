...
3. **Homeless wheat from the dietary spec** — "`MedicalConditions` stays a plain `ProfileInfo` string, made hard to leak by a focused review/test that every serializer either omits it or gates it behind the policy; a wrapper type forcing a policy token to read medical was **rejected as over-engineering at this scale**, but noted as the fallback if leaks surface." Verified still true (`Profile.cs:92`, no wrapper type in the domain). Too narrow for `design-rules.md` as a general pattern; `sections/*.md` excludes rationale narrative. **Still open** — asked twice; the husk is already deleted, so the text survives only in git history and in this report until Peter rules.
