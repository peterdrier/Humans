...
- [ ] **Step 1: Create `_BuildStrikeRotaRow.cshtml`** — a single all-day day-row: the date cell, fill bar, status cell, and avatars cell copied from the per-day (`item`) rendering in `_BuildStrikeRotaTable`. Action/status cell keeps the FormPost read-only status; add the `InstantToggle` TODO placeholder (Chunk 4). Model: `BuildStrikeRotaRowViewModel`.

- [ ] **Step 2: Wire the table — FormPost path unchanged** — In `_BuildStrikeRotaTable.cshtml`, keep the range form + `confirmSignup-*` modal + per-rota `<script>` (still rendered in FormPost). Replace **only the per-day detail/single `<tr>` rendering** inside the `dateRanges` loop with `@await Html.PartialAsync("_BuildStrikeRotaRow", new BuildStrikeRotaRowViewModel { Item = item, Es = es, IsSignedUp = Model.UserSignupShiftIds.Contains(item.Shift.Id), SignupsBlockedByMissingDietary = Model.SignupsBlockedByMissingDietary, Interaction = Model.Interaction })`. The range-header/expand rows stay in the table (FormPost grouping).
