...
2. **Routing-keyword digest in the community index.** `CommunityFaqReader.ExtractKeywords` harvests the bold lead-in of every `- **…**` bullet (Key facts) and every standalone bold line (FAQ questions), de-duplicated, capped at 100/file, rendered after the Overview summary as `— covers: …`. The bold lead-ins / FAQ questions are the KB authors' hand-curated "what matters" markers, so this is a free specificity filter that also preserves multi-word phrases ("vulva urinals", "War zone") and acronyms ("Early Entry (EE)") — and it is bilingual-safe with zero stopword tuning. The Overview paragraph remains the human-readable `Summary`; keywords are additive.

   **Rejected — statistical TF-IDF / inverted term→file index.** At this scale "most specific" terms are dominated by one-off names/dates/URLs (noise, not routing signal) and unigram tokenization loses phrases; it would need df-filtering, n-gram extraction, and EN+ES stopword lists for a worse result than the free bold signal. The LLM is the retriever — it maps "EE"→"early entry" itself when it reads the digest — so a precomputed term→file table is redundant. Consistent with the original non-goal ("no embeddings/vector search; keyword/topic granularity is sufficient").

   **Scale-out path (future, not built).** Keyword generation does not have to live in Humans code. When the corpus grows large enough that inlining a per-file digest strains the prompt budget (the true ~100× case), the offline KB generator can emit the keywords directly — an explicit machine-readable field per file (front-matter / a `## Keywords` block), or a compact global term→file map computed offline (where TF-IDF + n-grams + curation are cheap and reviewable). The runtime change is then small: prefer an explicit keyword field when present, else fall back to the current bold-harvest. Until then, runtime harvest covers it with no KB-side change.
