...
| 14 | `refactor/858-peel-14-teams` | Teams | 7 | Relationship-clean on re-audit: §10's "Teams → Users (7) + 8 inbound" was fully drained by peterdrier#1188/#1225 (navs) and #992's `DropCrossSectionForeignKeys` (constraints, both directions) — no severing PR needed. One §5-class wall surfaced: `IX_team_role_definitions_team_name_unique` physically was a hand-written functional index (`TeamId, lower(Name)` — raw Sql in `AddTeamRoleSlots`) while the model declares plain `(TeamId, Name)`. Peter's 2026-08-11 call: no custom SQL in migrations, ever — `20260811145603_RealignTeamRoleNameIndex` swaps the physical index to the model's plain definition — shipped ahead of the peel in its own PR, peterdrier#1266 (index-only same-name swap per the `no-drops-until-prod-verified` exception pattern; the functional index is strictly stronger, so no data violation is possible). Follow-up nobodies-collective/Humans#1027 tracks keying the uniqueness off a stable identifier instead of mutable `Name`. `teams` carries the 6-row system-team `HasData` seed; sentinel `teams` |
| 15 | `refactor/858-peel-15-users` | Users + Profiles (merged) | 9 + 6 Identity | One `UsersDbContext` per §10.3 — carries the `IdentityDbContext` base, all 8 section DbSets and the 7 Identity tables; sentinel `users`. Raw-SQL audit: every chain `Sql(` against `users`/`profiles`/`user_emails`/`contact_fields`/`communication_preferences` is data-only (backfills/renames), so the baseline carries no Sql blocks. `HumansDbContext`, its factory and the full root chain (289 files) deleted in the same PR; no removal migration — there is no snapshot left to shrink. `__EFMigrationsHistory` left in place (drop is its own PR after prod verification, §10.3 item 2). New Users snapshot is byte-identical to the final root snapshot modulo namespace |
...
- **Gate** — its §5.1 wall is gone (`20260802203816_RealignScaffoldedPhysicalDefaults` dropped all
  ...
  **Peeled in peel 14** (2026-08-11): Teams (`TeamsDbContext`, 7 tables). The §8-row-14 wall
  (`lower(Name)` functional index) fell with Peter's same-day no-custom-SQL call —
  `RealignTeamRoleNameIndex` realigned the physical index to the model before the peel.
  `HumansDbContext` is at 8 DbSets: Profiles (6), Users (2 + Identity) — Profiles blocked by
  its three `→ User` relationships, Users last by design.
  **Peeled in peel 15** (2026-08-12): Users + Profiles together into one `UsersDbContext`
  (§10.3), and `HumansDbContext` deleted, root chain and all. Every live database was verified
  at the root chain tip (`20260811150817_PeelTeams`) before the deletion; the programme's peel
  phase is complete and only the `__EFMigrationsHistory` drop (§10.3 item 2) remains.
