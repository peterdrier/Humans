...
HUM0036 | View component invoked by its string name (`IViewComponentHelper.InvokeAsync(string, …)`, the `InvokeAsync(helper, string)` extension, `Controller.ViewComponent(string, …)`) — invoke by `Type` (design-rules §8b). Analyzes Razor-generated code; skips unmapped `<vc:>` tag-helper internals, reports hand-written `.cshtml` sites at their line | Error
