...
HUM0034 | Public type in a `[assembly: Section("…")]` assembly (nobodies-collective/Humans#866, G5) that is not the section's `ISection` entry point, its `<Section>Resource` localization marker, an EF Core migration, an MVC view component, or declared under `Contracts/` — the #866 keystone, making "internal by default" load-bearing instead of convention-only (`[Grandfathered("HUM0034")]` downgrades to Warning) | Error
