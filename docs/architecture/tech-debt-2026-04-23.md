...
- **Important section:** jobs no longer inject `HumansDbContext` (all 10 cleaned); 3 of 4 controller DbContext leaks cleaned (DevLoginController remains); composer services migrated; HumanRedirectController shim removed; IMemoryCache injection cleaned from jobs (still present in view-components by design).
...
- `UsersAdminController.Audience` direct DbContext reads (§2a) — resolved by `IAdminDatabaseDiagnosticsService`.
...
- `src/Humans.Web/Controllers/AdminController.cs:26` **[DONE 2026-06-06 — AdminController now owns only the dashboard; audience segmentation moved to UsersAdminController and reads through `IAdminDatabaseDiagnosticsService`.]**
