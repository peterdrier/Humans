...
- **Bare `[Fact]` and `[Theory]` are forbidden in test code.** All test methods must use `[HumansFact]` / `[HumansTheory]` from `Humans.Testing` (30s default timeout for both). Enforced via `BannedApiAnalyzers` rule `RS0030`.
...
- **`Timeout = 0` (or negative) on `HumansFact` / `HumansTheory` is forbidden.** The setter throws `ArgumentException` at attribute construction. To allow a longer cap on a slow test, set `Timeout = N` with `N > 30000`. Infinite timeout is not an option.
