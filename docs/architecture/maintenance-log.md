...
| §15i FullProfile foundations (issue #635) | 2026-05-04 | Follow-up | One-time | — | Shipped Phase 2/3/4/5/7/8/11 + arch tests + behavior tests + docs. `Profile.State` enum (lazy-compute write-back), `Profile.IsSuspended` `[Obsolete]`, `User.NormalizedEmail` `[Obsolete]`, `FullProfile.PrimaryEmail`/`AllVerifiedEmails`/`GoogleEmail`, Stub Profile invariant + `/Profile/Admin/Backfill`, `UserEmail.IsPrimary` partial unique index, `IFullProfileInvalidator` dev-mode caller log. Migration is additive only (`AddColumn` + `CreateIndex`). **Deferred to follow-up:** the actual Phase 9 reader migration (~117 cross-domain nav-read sites) and Phase 10 User-side nav strip (`User.Profile`, `User.UserEmails`, etc. deletion). Phase 6 (`HumansUserStore`) intentionally not shipped — the existing `IdentityFindByEmailRestrictionsTests` arch test forbids `UserManager.FindByEmailAsync`/`FindByNameAsync` in Application+Web, making the store dead weight. Three other drifts noted in the PR body. |
