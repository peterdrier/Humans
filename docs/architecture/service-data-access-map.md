...
(providers share the scoped `HumansDbContext` and EF is not thread-safe).
No direct DB access, no cache.
...
> `SyncCreditorLedgerAsync` sweeps full history nightly for creditor-account
> (40000000–40000999) journal lines; `GetCreditorStatusAsync` /
