...
- **Transitional §2c violations from #511 (User migration):**
  ...
  - ~~`AccountMergeService.RejectAsync` pending-email removal via direct `DbContext.UserEmails.Remove`~~ — fixed in #511. A narrow `IUserEmailService.RemoveUnverifiedEmailAsync` method now handles this.
