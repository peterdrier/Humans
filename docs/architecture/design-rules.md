...
- **2 business services** still live in `Humans.Infrastructure/Services/` and inject `HumansDbContext` directly: `GoogleWorkspaceSyncService`, `HumansMetricsService`. All other files in that folder are connectors, stubs, or renderers that correctly stay in Infrastructure. Target: 0. Migration progress by section:
...
- **Cross-domain `.Include()` calls** remain in `GoogleWorkspaceSyncService` (still in Infrastructure). The Application layer is clean — 0 `.Include()` calls across all Application-layer services. Target: 0 everywhere.
...
- **Google Workspace** (pre-§15): resources are on Shared Drives only; all SDK access goes through the dedicated services listed in §13. Extracted connectors so far:
  ...
  - `IGoogleGroupMembershipClient`, `IGoogleGroupProvisioningClient`, `IGoogleDrivePermissionsClient`, `IGoogleDirectoryClient` (§15 Part 2a, issue #574) — SDK bridge surface for the `GoogleWorkspaceSyncService` migration coming in §15 Part 2b (#575). Real implementations in `Humans.Infrastructure/Services/GoogleWorkspace/`; stubs (in-memory fakes) registered when no service-account credentials are configured. `GoogleWorkspaceSyncService` still imports `Google.Apis.*` directly until Part 2b strips the imports.
