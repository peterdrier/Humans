...
- **35 repositories** exist today (the dedicated `TicketingBudgetRepository` was removed in #815 — the Tickets→Budget bridge is now repository-free; `UserEmailRepository` was folded into `UserRepository` as a `.UserEmails` partial; `DriveActivityMonitorRepository` no longer exists; `SurveyRepository` added in #884; `GateRepository` added with the Gate section). Target: one per domain (~20 total, some sections need two):
