...
**`OnboardingService.SetConsentCheckPendingIfEligibleAsync`** does not invalidate the `FullProfile` dict. Pre-existing behavior (it did not invalidate the old cache either). To be addressed after the §15 migration settles. (`OnboardingService.PurgeHumanAsync` was removed in issue #582 and replaced by `IAccountDeletionService.PurgeAsync`, which invokes `IUserService.PurgeOwnDataAsync` — that in turn invalidates `FullProfile`.)
