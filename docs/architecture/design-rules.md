...
**The Web→section decoupling epic (nobodies-collective/Humans#1073) is the general form of this shape.** Where §8a/§8b's three fanouts assemble a per-user *document*, the seam interfaces in `Humans.Base.Interfaces` let a section register itself into Shell chrome — nav, dashboard tiles, jobs, policies, health checks — without Shell naming the section. Each derives from `ISectionContribution`, discovered by `SectionDiscoveryExtensions.DiscoverImplementations<T>()` and registered as a singleton against every seam interface it implements.
...
| `ISectionSettings` | `/Settings` tabs (key, label, view component, policy) | `SettingsTabComposition.Compose` (filters each tab by its `Policy` for the current user; rendered by `SettingsTabsViewComponent`) |
