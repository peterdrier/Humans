...
| `HumanAdminOnlyHandler` (`Humans.Web.Authorization.Requirements` — the only one left in the Shell) | `HumanAdminOnlyRequirement` | — | Admin profile operations |
...
The application service (`UserService`, `ProfileService`, `ContactFieldService`, etc.) lives in its section's `Humans.<Section>.Services` namespace and is `internal sealed` per HUM0034. It:
...
- **1 business service** lives in `Humans.Web/Infrastructure/Services/` (was `Humans.Infrastructure/Services/` until G5 lane 5b-6 deleted that project): `HumansMetricsService`. Its direct `DbContext` injection is gone — the 60-second gauge refresh resolves a scope via `IServiceScopeFactory` and reads through section read interfaces (`IUserServiceRead`, `ITeamServiceRead`, `IApplicationServiceRead`, `IMembershipCalculatorRead`, `ILegalDocumentSyncServiceRead`, `IGoogleSyncServiceRead`, …); the last residual repository resolve (`IGoogleSyncOutboxRepository`) is gone with it. What remains is the location, not a boundary crossing. All other files in that folder are connectors, stubs, trackers, or storage adapters that correctly stay in the Shell's Infrastructure folder. Migration progress by section:
  ...
  - **Profiles** — fully migrated 2026-04-20 in PR #235 — `ProfileService`, `ContactFieldService`, `UserEmailService`, `CommunicationPreferenceService` now live in `Humans.Users.Services` (the Profiles + Users merge; `ContactService` no longer exists). (`VolunteerHistoryService` was folded into the profile methods on `IUserRepository`; it no longer exists as a separate service.)
  - **User** — migrated 2026-04-21 in PR #243 — `UserService` now lives in `Humans.Users.Services`, goes through `IUserRepository` (`Humans.Users.Data.Repositories`), and invalidates the UserInfo cache on writes. A §15 caching decorator now owns the `UserInfo` read model (`CachingUserService`, Singleton, keyed-inner `UserService`, `TrackedCache<Guid, UserInfo>`). Its warm path calls the normal `IUserService.GetAllUserInfosAsync` read surface; there is no cache-only repository helper.
...
- **Cross-domain `.Include()` calls** are gone from the service layer everywhere — every remaining `.Include()` is inside a section's `Data/` folder and hydrates an aggregate-local nav (`Team.Members`, `Team.ChildTeams`, `Profile.Languages`, …). A cross-domain `.Include()` is now structurally impossible anyway: a section context maps only its own entities, so there is no foreign nav to name. Target: 0 everywhere, met.
...
- **Cross-domain navigation properties**. Target: stripped at the entity boundary, FK-only everywhere.
  - Still declared:
    - `User.UserEmails` stays — the `User.Email` override computes from it (per the #635 AC). It is no longer cross-domain at all: `User` and `UserEmail` are both `Humans.Users.Contracts` types on the same context. All other `User` navs were stripped in #635 (see Stripped, below).
    - Cross-section *navigation properties* are all gone (nobodies-collective/Humans#996 stripped the last 11, in Feedback, Teams and Campaigns), and so are the nav-less cross-section EF **FK constraints** — nobodies-collective/Humans#992 dropped all 54 (`HasOne<User>().WithMany().HasForeignKey(...)` and friends) across 38 configurations in one migration. Cross-section linkage is now a bare `Guid` column everywhere, and there are zero `[Obsolete("Cross-domain nav…")]` markers left in `src/`. HUM0024 (`CrossSectionEfJoinAnalyzer`) enforced this and was **retired** in nobodies-collective/Humans#1278: a section maps only its own tables in its own `<Section>DbContext`, so there is no shared EF model for a navigation to join across. The residual it described — a shared-project EF configuration mapping another section's entity — has nowhere left to occur now that `Humans.Infrastructure` is deleted; what survives is the review-time check that a *section* context does not map a foreign entity.
  - Stripped:
...
**External connectors (API bridge pattern).** External SDKs (Google, Stripe, SMTP/IMAP, Octokit, etc.) sit behind an interface so no consumer references the SDK assembly. The concrete implementation lives in the owning section's `Services/` folder (or a subfolder) and is the only code that imports the SDK namespaces; the SDK `PackageReference` sits on that one section's `.csproj`, which is what makes the bridge structural rather than conventional. Connectors own no database tables — side-effects that need persistence write through the owning section's repository (e.g., Stripe fee values land on `TicketOrder`, written by `ITicketRepository`).
...
- **Google Workspace** (pre-§15): resources are on Shared Drives only; all SDK access goes through the dedicated services listed in §13. Extracted connectors so far:
  ...
  - `IGoogleGroupMembershipClient`, `IGoogleGroupProvisioningClient`, `IGoogleDrivePermissionsClient`, `IGoogleDirectoryClient` (§15 Part 2a, issue #574) — SDK bridge surface consumed by the Application-layer `GoogleWorkspaceSyncService` (§15 Part 2b, issue #575). Real implementations and stubs (in-memory fakes, registered when no service-account credentials are configured) both live in `src/Sections/Humans.GoogleIntegration/Services/Workspace/`. `GoogleWorkspaceSyncService` never imports `Google.Apis.*`.
- **Email** (PR #266): `IEmailBodyComposer` and the transport seam live in `Humans.Email.Services`; `IImmediateOutboxProcessor` is implemented by the Shell's `HangfireImmediateOutboxProcessor` so Hangfire stays out of the section. The body-composer is SDK-free so section services can build messages without pulling MailKit in.
- **Ticket vendor** (PR #277): the port `ITicketVendorService` and `TicketVendorSettings` are Tickets-owned (`src/Sections/Humans.Tickets/Contracts/`); the concrete `TicketTailorService` / `StubTicketVendorService` live in the separate `src/Sections/Humans.TicketTailor/Services/`, the only project that references the vendor SDK.
