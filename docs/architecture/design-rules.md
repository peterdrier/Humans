...
- **Transitional §2c violations from #511 (User migration):**
  ...
  - `GoogleWorkspaceSyncService` → `ITeamService` injected via `IServiceProvider` lazy resolution to avoid the documented QA-outage cycle (`GoogleWorkspaceSyncService` → `TeamService` → `SystemTeamSync` → `GoogleWorkspaceSyncService`). Architectural fix requires breaking the `TeamService` → `ISystemTeamSync` dependency, which is out of #511 scope. (`TeamService` → `IUserEmailService` was fixed in the same PR by extracting `IAccountMergeRequestRepository` from `IAccountMergeService`, breaking the `TeamService` → `UserEmailService` → `AccountMergeService` → `TeamService` cycle.)
