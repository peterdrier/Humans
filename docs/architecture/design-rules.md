...
- **13 repositories** exist today (`ApplicationRepository`, `ProfileRepository`, `ContactFieldRepository`, `UserEmailRepository`, `CommunicationPreferenceRepository`, `UserRepository`, `CityPlanningRepository`, `AuditLogRepository`, `BudgetRepository`, `CampaignRepository`, `CampRepository`, `EmailOutboxRepository`, `FeedbackRepository`). Target: one per domain (~20 total).
