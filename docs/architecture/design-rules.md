...
- **Transitional §2c violations from #511 (User migration):**
  - `OnboardingService.ApproveVolunteerAsync`/`SuspendAsync`/`UnsuspendAsync` + `SuspendNonCompliantMembersJob` read/write `DbContext.Profiles` directly because calling `IProfileService.{ApproveVolunteer,Suspend,Unsuspend}Async` from OnboardingService would create a DI cycle (ProfileService already depends on IOnboardingService). Fixed when OnboardingService itself migrates.
  - `TeamService` → `IUserEmailService` and `GoogleWorkspaceSyncService` → `ITeamService` injected via `IServiceProvider` lazy resolution to avoid DI cycles (TeamService → UserEmailService → AccountMergeService → TeamService loop; GoogleWorkspaceSyncService → TeamService → SystemTeamSync → GoogleWorkspaceSyncService loop). Architectural fix is to extract the narrow query/create methods AccountMergeService exposes to UserEmailService into a separate service, breaking the concrete-class cycle. Out of #511 scope.
