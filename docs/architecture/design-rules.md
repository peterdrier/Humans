...
Repository **implementations** (the classes that talk to `DbContext`) live in the owning section project's `Data/`. `Humans.Infrastructure` was the shared home until G5 lane 5b-6 deleted it (nobodies-collective/Humans#866); only the platform context is left, in `Humans.Web/Data/`. (now `Humans.Base`)
...
5. **Stores are Infrastructure.** The interface lives in `Humans.Application.Interfaces.Stores/`, the implementation lives in the section's `Data/` folder. (now `Humans.Base`)
...
- **ViewModels** live in their section's `Models/` folder (or are inlined in controllers). Controllers map DTOs or entities to view models for Razor. The section-agnostic ones — the generic table models and `PagerViewModel` — are still under `Humans.UI.Models`, now in the base assembly (§1). (now `Humans.Base`)
