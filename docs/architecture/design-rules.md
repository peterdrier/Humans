...
| `ISectionHelp` | Guide/Glossary help content (Markdown, embedded resource) | `AccessMatrixViewComponent`'s modal and the agent's preloaded glossary corpus (`AgentPreloadAugmentor`, `AgentPromptAssembler`); entries carry an explicit `SectionHelpEntry.Order` |
