...
- **~24 services** still live in `Humans.Infrastructure/Services/` and inject `HumansDbContext` directly. Target: 0. Migration progress by section:
  ...
  - **Shifts (partial)** — `ShiftSignupService` migrated 2026-04-22 under umbrella #541 (sub-task #541b) — it now lives in `Humans.Application.Services.Shifts` and goes through `IShiftSignupRepository`. Option A (no decorator) — shift-signup reads are request-scoped and don't benefit from a dict cache. The repository also holds the within-section cross-service reads on `rotas` / `shifts` / `volunteer_event_profiles` / `general_availabilities` / `volunteer_tag_preferences` that `ShiftSignupService` needs; these move to `IShiftManagementRepository` / `IGeneralAvailabilityRepository` / etc. when #541a and the #541c surface-expansion follow-ups land. `ShiftManagementService` and `GeneralAvailabilityService` remain pending (#541a, #541c).
- **~20 cross-domain `.Include()` calls** across **~12 services**. Biggest offenders: `OnboardingService` (7), `GoogleWorkspaceSyncService` (4). Target: 0.
  - Includes fully removed from the service layer in these sections:
    ...
    - Shifts (ShiftSignupService only — `.Include(d => d.User)` / `.Include(s => s.ReviewedByUser)` deliberately preserved inside `IShiftSignupRepository.GetByShiftAsync` and `GetNoShowHistoryAsync` so the ShiftAdmin view and Profile view can keep reading `signup.User.DisplayName` / `signup.ReviewedByUser.DisplayName`; the in-memory stitching refactor lands with the wider User-entity nav cleanup. `ShiftManagementService` and `GeneralAvailabilityService` still in Infrastructure)
