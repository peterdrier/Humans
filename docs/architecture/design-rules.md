...
- **8 repositories** exist today (`ApplicationRepository`, `ProfileRepository`, `ContactFieldRepository`, `UserEmailRepository`, `CommunicationPreferenceRepository`, `UserRepository`, `CityPlanningRepository`, `AuditLogRepository`). Target: one per domain (~20 total).
