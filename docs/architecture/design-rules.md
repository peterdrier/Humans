...
- **1 business service** still lives in `Humans.Infrastructure/Services/` and injects `HumansDbContext` directly: `HumansMetricsService`. All other files in that folder are connectors, stubs, or renderers that correctly stay in Infrastructure. Target: 0. Migration progress by section:
  ...
  - **Tickets** — fully migrated 2026-04-24 under umbrella #545 (verification pass closed the remaining gaps). `TicketQueryService`, `TicketSyncService`, and `TicketingBudgetService` live in `Humans.Application.Services.Tickets`, going through `ITicketRepository` and `ITicketingBudgetRepository`. Cross-section reads route through sibling services (`IBudgetService`, `ICampaignService`, `IUserService`, `IUserEmailService`, `IProfileService`, `ITeamService`, `IShiftManagementService`). The Ticket Tailor API side of `TicketSyncService` is structurally separated via the `ITicketVendorService` connector (PR #277). `event_participations` write-path sits on `IUserRepository` / `IUserService` — kept there because the state (per-user, per-year attendance flag) is a User-section concern and the Ticket sync merely backfills it from Stripe-matched orders. Cross-domain navs (`TicketOrder.MatchedUser`, `TicketAttendee.MatchedUser`, `EventParticipation.User`) are `[Obsolete]`-marked per §6c; current readers use `MatchedUserId` FK + `IUserService` stitching, so no `#pragma warning disable CS0618` is needed outside the EF configurations (which reference the navs under scoped pragmas to keep FK + cascade behavior wired). **No caching decorator** — ticket reads power two admin dashboards and one guest-facing "you have tickets" probe; not hot-path enough to justify a decorator. Per-service architecture tests (`TicketQueryArchitectureTests`, `TicketSyncArchitectureTests`, `TicketingBudgetArchitectureTests`, `TicketVendorArchitectureTests`) pin namespace + no-DbContext-ctor + repository-dep invariants and enforce cross-section reads go through the owning service.
