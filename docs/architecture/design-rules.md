...
- **1 caching decorator** exists today (`CachingProfileService` — §15 pattern). Governance and User sections operate without one. Target: every migrated section that needs caching uses the §15 pattern. Not every section needs caching.
...
- **Cross-domain navigation properties** (`User.Profile`, `User.UserEmails`, `User.TeamMemberships`, `User.RoleAssignments`, `User.Applications`, `User.ConsentRecords`, `User.CommunicationPreferences`, `User.GetEffectiveEmail()`) are still declared on the `User` entity — PR #243 deferred the strip to an isolated follow-up PR because ~15 call sites need service-routing migration (and a new `IUserEmailService.GetNotificationEmailAsync` surface). `TeamMember.User`, `CampLead.User`, etc. are also still used freely. Profile-section (`Profile.User`, `UserEmail.User`, `CommunicationPreference.User`) is stripped. Target: stripped at the entity boundary, FK-only everywhere.
