...
**Browse shifts at `/Shifts`.** Filter by department, date range, period (Set-up / Event / Strike), or a specific day. Tag filters live above the list — click a tag like "Heavy lifting" or "Meeting new people" to narrow the view. Open the preferences panel to save tag preferences so matching shifts are highlighted with a star. Consecutive all-day Set-up and Strike shifts in the same rota are compressed into date ranges; click to expand individual days.
...
**Manage signups.** The admin page shows a "Signed Up" column with names (Event rotas) or avatars (Build/Strike rotas). Pending signups have their own table — approve, refuse (with an optional reason), or handle a date-range block at once. **Voluntell** to enroll a specific human directly (auto-confirmed; the assigned volunteer gets an in-app "you were assigned" notification, and the department's coordinators get a signup-change notification). **Voluntell range** does the same across a date range for build/strike rotas. **Remove** unassigns a confirmed signup — the confirmation prompt names the human, so you know exactly who you're removing. Past shifts open the same **Manage** panel as upcoming ones, so you can correct the rota after the fact: remove someone, **mark no-show**, or bail a whole date-range block, with no-showed and bailed humans listed as read-only history. No-show counts surface in volunteer profiles for coordinators.
