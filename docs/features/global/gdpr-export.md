...
| `AuditLog` | `AuditLogService` | Array of `{ Action, EntityType, EntityId, Description, OccurredAt, Role }` (Role is "Actor" or "Subject"). |
...
| `ExpenseReports` | `ExpenseReportService` | Single object `{ MaskedIban, Reports: [{ Id, Status, Note, PayeeName, PayeeIban (masked), Total, SubmittedAt, ApprovedAt, CreatedAt, Lines: [{ Id, Description, Amount, LineType, SortOrder, Attachment? }] }] }` — the user's expense reports including line items and attachment metadata, plus their masked profile IBAN; null when the user has neither reports nor an IBAN. Expense audit events are in the canonical `AuditLog` section. |
