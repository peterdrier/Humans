...
| `/Google/Resource/{id}` (`GoogleController`) | Board, Admin | GET: Per-resource sync audit log |
| `/Google/Human/{id}` (`GoogleController`) | HumanAdmin, Board, Admin | GET: Per-user sync audit log |
...
| Action | Link | Notes |
|--------|------|-------|
| Event settings | `/Settings/Admin` | The app-wide event values in `settings_event` — every section reads the calendar from here now; a blank id mints a new cycle (nobodies-collective/Humans#1631). `/Shifts/Settings` is knobs-only (`IsShiftBrowsingOpen`, caps, reminder lead time) |
