...
- Create form at `/Store/Admin/Catalog/New`; edit form at `/Store/Admin/Catalog/{id}/Edit`. Trim+validate name (≤200), description (≤2000), non-negative numerics, VAT rate 0–100, OrderableUntil must be a future date.
...
- Stripe's POST to `/Store/StripeWebhook` is verified via `EventUtility.ConstructEvent(body, signature, signingSecret, throwOnApiVersionMismatch: false)`. Only `checkout.session.completed` is handled; other event types return 200 no-op so Stripe doesn't retry.
