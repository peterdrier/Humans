...
| 27 | **Removing the `[Display(Name = …)]` attributes made the DataAnnotations validation text worse.** `[Required]`/`[StringLength]` on `Motivation` and `AdditionalInfo` interpolate the metadata display name, which is now the CLR name — "The Motivation field is required." Not a localization regression: neither the message template nor the old display names resolve against `SharedResource`, so this text was raw English in every supported culture before and after. The real defect underneath is older and larger — see the section ledger. | med | **section ledger** |
