...
| 2026-08-18 | Finance | First scheduled run (plan written this run): deep assessment + strike | Section doc rewritten off the pre-G5 controller (phantom Budget routes, phantom Tickets dependency, shipped read-split still listed as future work), same claims swept out of the code comments and the section index; the untested invariants pinned (Madrid date conversion, contact-list cache); InspectCode findings | peterdrier/Humans#1367 |
