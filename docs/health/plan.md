...
- [ ] **`GuideFiles.TryCanonical` shipped without the second-opinion gate.** Both reviewer
      subagents died; I worked their checklist myself (one concept; behaviour identical on
      null/empty/whitespace; no seam lost, all ten call sites passed the same set) but that is
      self-review. Judge it, or say revert — nothing depends on it.
