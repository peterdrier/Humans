...
| 7 | `docs/sections/Cantina.md` exists and matches reality | PARTIAL | **Correction:** `docs/sections/Cantina.md` DOES exist (a prior glob call with brace-expansion syntax returned a false negative — confirmed via `ls` and direct `Read`; it's a 96-line doc: on-site/arrival-day definitions, `MedicalConditions` exclusion invariant, architecture notes). It correctly documents the no-owned-tables shape and the cross-section read pattern (`IShiftManagementService`, `IUserServiceRead`). **But** it cites `tests/Humans.Application.Tests/Services/Cantina/CantinaRosterServiceTests.cs and CantinaAccessServiceTests.cs` as pinning the aggregation rules and access gate — `CantinaAccessServiceTests.cs` does **not exist** anywhere in the repo (the real files are `CantinaDailyRosterServiceTests.cs`/`CantinaRosterServiceTests.cs` under `Application.Tests/Services/Cantina/`, and `CantinaCsvWritersTests.cs`/`CantinaRosterAssemblerTests.cs` under `Web.Tests/Cantina/`). Minor doc drift on the test-file reference. |
...
2. **`docs/sections/Cantina.md` cites a non-existent test file (`CantinaAccessServiceTests.cs`)** — see G1.7. Fix: correct the "Architecture Test" reference to the real file names. No migration needed: **y**.
